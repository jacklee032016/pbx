...
void SipEvent::setSipMsg( const Sptr <SipMsg> sipMsg )
{
	assert( sipMsg != 0 );
	mySipMsg = sipMsg;
}

const Sptr <SipMsg> SipEvent::getSipMsg() const
{
	assert( mySipMsg != 0 );
	return ( mySipMsg );
}

void SipEvent::setSipMsgQueue(const Sptr <SipMsgQueue> sipRcv)
{
	assert( sipRcv != 0 );
...
const Sptr <SipMsgQueue> SipEvent::getSipMsgQueue() const
{
	assert( mySipMsgQueue != 0 );
	return ( mySipMsgQueue );
}

const Sptr <InviteMsg> SipEvent::getInvite() const
{
	// iterate through all msgs associated with this event looking for an INVITE, and return it if found
	Sptr <SipMsg> sipMsg;
...
const Sptr <SipCommand> SipEvent::getCommand() const
{
	// iterate through all msgs associated with this event looking for a command, and return it if found
	Sptr <SipMsg> sipMsg;
	...
	if ( transaction != 0 )
	{
		...
		for (i = transaction->begin(); i != transaction->end(); ++i)
		{
			...
			if ( command != 0 )
			{
				cpDebug(LOG_DEBUG, "Found command sent to %s:%d", command->getSendAddress().getIpName().c_str(),
...
const Sptr <SipCommand> SipEvent::getPendingCommand( Sptr <SipCommand> sipCommand ) const
{
	...
	// search through previous msgs, and return matching command if found; otherwise return 0
	...
	while ( iter != callLegMsgs.end() )
	{
		...
		else
		{
			...
			{
				if ( pendingCommand->getVia( 1 ) == sipCommand->getVia( 0 ) )
				{
...
const Sptr <SipCallLeg> SipEvent::getSipCallLeg() const
{
	assert( mySipCallLeg != 0 );
	return ( mySipCallLeg );
}

void SipEvent::removeCall( )
{
//	assert( myCallInfo != 0 );
...
void SipEvent::setSipStack(Sptr <SipTransceiver> sipStack)
{
	mySipStack = sipStack;
}
