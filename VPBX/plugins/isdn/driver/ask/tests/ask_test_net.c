/*
$Id: ask_test_net.c,v 1.1.1.1 2006/11/30 16:26:29 lizhijie Exp $
*/
/* test L3 DSS1 NET */
...
static char net_usage[] =
{
	"Call with %s [options] [filename]\r\n\r\n" \
	"       filename   filename.in  incoming data\r\n" \
	"                  filename.out outgoing data\r\n" \
	"                  data is sun audio 8khz 8bi for voice\r\n\r\n" \
	"\n     Valid options are:\r\n\r\n" \
	"  -?              Usage ; printout this information\r\n" \
	"  -c<n>           use card number n (default 1)\r\n" \
	"  -d <text>       Display text (default \"Test Display\")\r\n" \
	"  -m <number>     Called PN (default 789)\r\n" \
	"  -n <number>     Calling PN (default keine)\r\n" \
...
static int testnetVerifyOn=0xFF;
...
int testnet_play_msg(devinfo_t *di)
{
	unsigned char buf[PLAY_SIZE+AS_ISDN_HEADER_LEN], *p;
	...
	else if (testnetVerifyOn>3)
		fprintf(stdout,"play write ret=%d\n", ret);

	return(ret);
}

int testnet_send_data(devinfo_t *di)
{
...
int testnet_setup_bchannel(devinfo_t *di)
{
	AS_ISDN_pid_t pid;
	int ret;
	layer_info_t li;
...
int testnet_send_SETUP(devinfo_t *di, int SI, char *PNr)
{
...
int testnet_activate_bchan(devinfo_t *di)
{
...
int testnet_deactivate_bchan(devinfo_t *di)
{
	unsigned char buf[128];
	int ret;
...
int testnet_send_touchtone(devinfo_t *di, int tone)
{
	iframe_t frm;
	int tval, ret;

	if (testnetVerifyOn>1)
...
int testnet_read_mutiplexer(devinfo_t *di)
{
...
start_again:
	...
	if (di->flag & FLG_SEND_TONE)
	{
		...
		{
			di->val--;
			testnet_send_touchtone(di, tt_char[di->val]);
		}
		else
...
int testnet_do_connection(devinfo_t *di)
{
	...
	if (di->func == OUT_CALL)
	{
		di->flag |= FLG_CALL_ORGINATE;
		di->cr = 0x81;
		testnet_send_SETUP(di, di->si, di->phonenr);
	}
	...
	while ((ret = ask_isdn_read(di->device, buf, 1024, 3*TIMEOUT_10SEC)))
	{
		...
		if (ret >= 20)
		{
			if (((!(di->flag & FLG_CALL_ORGINATE)) && (buf[19] == MT_SETUP)) ||
				((di->flag & FLG_CALL_ORGINATE) && (buf[19] == MT_ALERTING)))
			{		
				if (!(di->flag & FLG_CALL_ORGINATE))
					di->cr = buf[18];
	 			idx = 20;
	...
	{/* setup a B-channel stack */
		...
		if (!(di->flag & FLG_CALL_ORGINATE))
		{
			...
			printf("Time %d:%d %d/%d/%d\n", ts->tm_hour, ts->tm_min, ts->tm_mday, ts->tm_mon+1, ts->tm_year);
...
int testnet_add_dlayer3(devinfo_t *di, int prot)
{
	layer_info_t li;
	stack_info_t si;
	interface_info_t ii;
	...
	li.object_id = -1;
	li.extentions = 0;
	li.pid.protocol[3] = prot;	/*L3 DSS1 NET */
	li.pid.layermask = ISDN_LAYER(3);
	li.st = di->d_stid;
...
int testnet_do_setup(devinfo_t *di)
{
	...
	ret = testnet_add_dlayer3(di, ISDN_PID_L3_DSS1NET);
	if (ret)
		return(ret);
	...
	ret = ask_isdn_read(di->device, buf, 1024, TIMEOUT_1SEC);
	if (testnetVerifyOn>3)
...
int main(int argc,char *argv[])
{
	...
	fprintf(stderr,"Test L3 DSS1 NET 1.0\n");
	strcpy(FileName, "test_file");
	memset(&AS_ISDN, 0, sizeof(AS_ISDN));
	AS_ISDN.cardnr = 1;
	AS_ISDN.func = 0;
	strcpy(AS_ISDN.display, "Test Display");
	strcpy(AS_ISDN.msn, "789");
	AS_ISDN.phonenr[0] = 0;
	
	if (argc<1)
	{
		fprintf(stderr,"Error: Not enough arguments please check\n");
		usage(argv[0], net_usage);
		exit(1);
	}
		...
		if (argv[aidx] && argv[aidx][0]=='-')
		{
			...
			switch (sw)
			{
				...
				case 'd':
					...
					if (aidx<=argc)
					{
						strcpy(AS_ISDN.display, &argv[aidx][idx]);
					}
					else
					{
				...
				case 'm':
					...
					if (aidx<=argc)
					{
						strcpy(AS_ISDN.msn, &argv[aidx][idx]);
					}
					else
					{
						fprintf(stderr," Switch %c without value\n",sw);
	...
	{
		printf("TestAS_ISDN cannot open AS_ISDN due to %s\n",	strerror(errno));
		return(1);
	}
	
	sprintf(FileNameOut,"%s.out",FileName);
	sprintf(FileName,"%s.in",FileName);
	printf("Test ISDN open in %s\n",FileName);
	...
	printf("Test ISDN files open\n");
	if (testnetVerifyOn>8)
		fprintf(stdout,"fileno %d/%d/%d\n",AS_ISDN.save, AS_ISDN.play, AS_ISDN.device);

	err = testnet_do_setup(&AS_ISDN);
	if (err)
		fprintf(stdout,"do_setup error %d\n", err);
	else
		testnet_do_connection(&AS_ISDN);	
