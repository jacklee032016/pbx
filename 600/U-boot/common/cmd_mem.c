 ...
 * $Id: cmd_mem.c,v 1.1.1.1 2006/11/29 09:09:44 lizhijie Exp $
 */
...
#if 0	/* chaned as following, lizhijie, 2005.06.15 */
	incr = 1;
	for (;;) 
	{
		if (ctrlc()) 
...
#endif
	incr = 1;
	printf("Assist Memory test : %ld KB --> %ld KB \r\n", (ulong)start/1024, (ulong)end/1024 );
//	printf("Assist Memory test : 0X%08lX(KB) --> 0X%08lX (%ld KB) \r\n", start, end, (end-start)/1024);

	for (i=0; i<sizeof(bitpattern)/sizeof(ulong); i++) 
	{
		ulong  end_addr = (ulong)end/4;
		...
		for (addr=start; addr<end_addr; addr++) 
		{
			*addr = pattern;
			readback = *addr;
			if (readback != pattern ) 
	...
	if(rcode==0)
	{
		printf("SDRAM Test in IXP Board for %d loop successful!\r\n", sizeof(bitpattern)/sizeof(ulong) );
		beep_ok();
	}
	else
	{
		printf("SDRAM Test in IXP Board for %d loop failed %d times!\r\n", sizeof(bitpattern)/sizeof(ulong), rcode );
		beep_fail();
	}
