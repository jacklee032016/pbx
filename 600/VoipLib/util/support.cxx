/*
 * $Id: support.cxx,v 1.3 2007/06/05 11:33:26 lizhijie Exp $
 */
...
Data convertToHex(const unsigned char* src, int len)
{
    ...
    cpDebug(LOG_DEBUG, "convertToHex, value is: %s", data.logData());
...
int convertToUnsigned(const Data& data, unsigned char* dest)
{
    ...
    cpDebug(LOG_DEBUG, "orig value string: %s", data.logData());

    cpDebug(LOG_DEBUG, "orig value hex:    %s", convertToHex(dest, len).logData());
    cpDebug(LOG_DEBUG, "convertToUnsigned, length is %d, value is : %s", len, dest);
    return len;
}
