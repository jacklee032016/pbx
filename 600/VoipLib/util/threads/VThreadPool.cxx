/*
 * $Id: VThreadPool.cxx,v 1.2 2007/03/01 20:04:33 lizhijie Exp $
 */
...
VThreadPool::addFunctor(const VFunctor& functor) throw (VThreadInitException&)
{
    ...
    if (shutdown_ || queueClosed_)
    {
        ...
        cpDebug(LOG_DEBUG, "Pool shutting down..");
...
VThreadPool::shutdown(bool finish)
{
    ...
    {
        cpDebug(LOG_DEBUG, "Failed to get the lock, still shutting down..");
	assert(0);
	return;
    }
...
VThreadPool::workerProc(void* args)
{
    ...
    cpDebug(LOG_DEBUG, "Creating worker thread (%d)", pthread_self());
    for (;;)
    {
        ...
        if (self.shutdown_)
        {
            ...
            cpDebug(LOG_DEBUG, "Thread (%d) exiting.." , pthread_self());
            pthread_exit(0);
        }
...
//            if (work)
//            {
                cpDebug(LOG_DEBUG, "Thread (%d) Doing work." , pthread_self());
                work.doWork();
//            }
        }
        catch (VException& e)
...
{
    cpDebug(LOG_DEBUG, "VThreadPool::~VThreadPool");
    shutdown();
}
