/*
 * $Id: NetworkConfig.cxx,v 1.4 2007/06/05 11:33:26 lizhijie Exp $
 */
...
int NetworkConfig::init()
{
	...
	while(myItr)
	{
		...
		cpDebug(LOG_DEBUG, "Family:%d", myItr->ai_family);
		cpDebug(LOG_DEBUG, "Address Len:%d" , myItr->ai_addrlen);
		cpDebug(LOG_DEBUG, "Protocol:%d" , myItr->ai_protocol);
		cpDebug(LOG_DEBUG, "Host Name:%s" , myItr->ai_canonname);
		myItr = myItr->ai_next;
	}
	...
	if((myFlg & 0x01) && (myFlg & 0x02))
	{
		...
		dualStack = true;
		cpDebug(LOG_INFO, "*** Dual-stack supports IPv4 and IPv6 ***" );
	}
	else if((myFlg & 0x01))
	{
		myAddrFamily = PF_INET;
		cpDebug(LOG_INFO, "*** IPv4 support only ***");
	}
	else if(myFlg & 0x02)
	{
		myAddrFamily = PF_INET6;
		cpDebug(LOG_INFO, "*** IPv6 support only ***" );
	}
