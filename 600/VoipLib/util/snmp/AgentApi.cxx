/*
 * $Id: AgentApi.cxx,v 1.3 2007/06/05 11:33:26 lizhijie Exp $
 */
...
AgentApi::~AgentApi()
{
    ...
    cpDebug(LOG_DEBUG, "entered agentApi destructor");
    shutdown();
    join();
}
...
AgentApi::sendResponse(int val, NetworkAddress *sender)
{
	try
	{
		...
		if (sender) 
		{
			cpDebug(LOG_DEBUG, "send integer response: %d to %s:%d", val, sender->getHostName().c_str(), sender->getPort());
...
AgentApi::sendResponse(unsigned long val, NetworkAddress *sender)
{
	try
	{
		...
		memset(message.parm2, 0, sizeof(message.parm2));
		memcpy(message.parm2, &val, sizeof(val));
		cpDebug(LOG_DEBUG, "send ulong response: %d to %s:%d", val, sender->getHostName().c_str(), sender->getPort());
		udpStack->transmitTo((char *)&message, sizeof(message), sender);
		return voSuccess;
	}
#ifdef PtW32CatchAll
...
AgentApi::sendResponse(string parameter, NetworkAddress *sender)
{
    try
    {
        ...
        cpDebug(LOG_DEBUG, "send string response: %s to %s:%d",
...
AgentApi::sendResponse(void *inData, NetworkAddress *sender)
{
	...
	memset(message.parm2, 0, sizeof(message.parm2));
        ...
        cpDebug(LOG_DEBUG, "send generic response to %s:%d",
...
AgentApi::thread()
{
    ...
    while (true)
    {
        try
        {
            ...
            if ( bytesRead )
            {
                cpDebug(LOG_DEBUG, "bytes read=%d", bytesRead);
