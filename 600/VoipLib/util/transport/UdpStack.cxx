/*
 * $Id: UdpStack.cxx,v 1.5 2007/06/05 11:33:26 lizhijie Exp $
 */
	...
	blockingFlg(true)
{
	...
	cpDebug (LOG_DEBUG_STACK, "UdpStack socketFd = %d", data->socketFd);
	...
	{
		if (setsockopt(data->socketFd, protoent->p_proto, SO_BSDCOMPAT,(char*)&buf1, len1) == -1)
		{
			fprintf(stderr, "setsockopt error SO_BSDCOMPAT :%s\n",	strerror(errno));
		}
#if 1
		if (setsockopt(data->socketFd, SOL_SOCKET, SO_RCVBUF,(int *)&rcvbufnew, rcvbufnewlen)	== -1)
		{
			fprintf(stderr, "setsockopt error SO_RCVBUF :%s\n",	strerror(errno));
		}
#endif
		if (getsockopt(data->socketFd, SOL_SOCKET, SO_RCVBUF, (int*)&rcvbuf, &rcvbuflen) == -1)
		{
			fprintf(stderr, "getsockopt error SO_RCVBUF :%s\n",	strerror(errno));
		}
		else
		{
			cpDebug(LOG_DEBUG, "SO_RCVBUF = %d, rcvbuflen  =%d" , rcvbuf , rcvbuflen);
		}
		
		if (getsockopt(data->socketFd, SOL_SOCKET, SO_SNDBUF, (int*)&sndbuf, &sndbuflen) == -1)
		{
			fprintf(stderr, "getsockopt error SO_SNDBUF :%s\n",	strerror(errno));
		}
		else
		{
			cpDebug(LOG_DEBUG, "SO_SNDBUF = %d, sndbuflen = %d" , sndbuf , sndbuflen);
		}
	}
	...
	if (isMulticast)
	{
		...
		if (setsockopt(data->socketFd, SOL_SOCKET, SO_REUSEADDR,(char*)&buf1, len1)== -1)
		{
		...
		if (setsockopt(data->socketFd, SOL_SOCKET, SO_REUSEPORT,(char*)&buf1, len1)== -1)
		{
			fprintf(stderr, "setsockopt error SO_REUSEPORT :%s",strerror(errno));
		}
	...
	switch (mode)
	{
		case inactive :
		{
			cpDebug(LOG_INFO, "Udp stack is inactive");
		...
		case sendonly :
		{
			...
			{
				cpDebug(LOG_DEBUG_STACK, "sendonly Udp stack, desHost is needed by using setDestination()");
			}
		}
		break;
		case recvonly :
		...
		case sendrecv :
		{
			...
			else
			{
				// only receive, do bind socket to local port
				doServer(minPort, maxPort);
				cpDebug(LOG_DEBUG_STACK, "sendrecv Udp stack, desHost is needed by using setDestination()");
			}
		}
		break;
		default :
	...
	if (logFlag)
	{
		...
		if (!in_log)
		{
			cerr << "Cannot open file " << logFileNameRcv.str() << endl;
		...
		if (!out_log)
		{
			cerr << "Cannot open file "<< logFileNameSnd.str() << endl;
...
void UdpStack::doServer ( int minPort, int maxPort )  // these are lcoal ports
{
    /*
    ...
    */
	...
	if (error) {
        ...
        cpDebug(LOG_DEBUG, "Udp bind() fd =%d, port=%s", data->socketFd, currport); //port = %d(h_order), %d(n_order)", data->socketFd, localPort, (data->localAddr).sin6_port);
	    ...
	    bError = false;
            ...
            {
                cpDebug(LOG_DEBUG, "(IPv6) Udp bound to fd = %d, port = %d",data->socketFd, localPort);
            ...
            {
                cpDebug(LOG_DEBUG, "(IPv4) Udp bound to fd = %d, port = %d",data->socketFd, localPort);
            }
        }
        freeaddrinfo(sa);
	if (portOk) break;
...
void UdpStack::doClient ( const NetworkAddress* desHost) // This is the destination port
{
	cpDebug(LOG_DEBUG_STACK, "UdpStack::doClient");
	cpDebug(LOG_DEBUG_STACK, "desHost = %s, desPort = %d", desHost->getIpName().c_str(), desHost->getPort());
...
void UdpStack::connectPorts()
{
	...
	// conenct to server
	if ((result = connect(data->socketFd, (struct sockaddr*) & (data->localAddr), sizeof(*data->localAddr))) != 0)
	{
		int err = errno;
		strstream errMsg;
...
void UdpStack::disconnectPorts()
{
	...
	if ((result = connect(data->socketFd, (struct sockaddr*) & dummyAddr, sizeof(dummyAddr))) != 0)
	{
		int err = errno;
		strstream errMsg;
		errMsg << "UdpStack<" << getLclName() << " " << getRmtName()
...
{
	cpDebug(LOG_DEBUG_STACK, "UdpStack::setLocal");
	cpDebug(LOG_DEBUG_STACK, "minPort = %d, maxPort = %d", minPort, maxPort);
	...
	{
		int newFd;
		newFd = socket(NetworkConfig::instance().getAddrFamily(), SOCK_DGRAM, IPPROTO_UDP);
...
void UdpStack::setDestination ( const NetworkAddress* host )
{
	if ((mode == recvonly) || (mode == inactive))
	{
		cpLog(LOG_ERR, "The UdpStack is recvonly or inactive.");
...
void UdpStack::setDestination ( const char* host, int port )
{
	assert(host);
...
{
	cpDebug(LOG_DEBUG_STACK, "getRxPort()");
	char service[6];
...
{
	cpDebug(LOG_DEBUG_STACK, "getTxPort()");
	char service[6];
...
NetworkAddress *UdpStack::getDestinationHost () const
{
	char host[256];
	char port[6];
...
int UdpStack::getMaxFD ( int prevMax )
{
...
int UdpStack::receive ( const char* buf, const int bufSize )
{
	if ((mode == sendonly) || (mode == inactive))
	{
	...
	int len = recv( data->socketFd, (char *)buf, bufSize, 0 /*flags */);
	if ( len < 0 )
	{
		int err = errno;
	...
	assert(0);
#endif
	//        cpDebug(LOG_DEBUG_STACK, "UdpStack::receive pkt len=0");
	}
	else if (len == 0)
	{
		// received no bytes
		cpDebug(LOG_DEBUG, "did not receive any data");
	}
	else
	{
...
int UdpStack::receiveFrom ( const char* buffer, const int bufSize, NetworkAddress* sender ) // returns bytes read
{
	if ((mode == sendonly) || (mode == inactive))
	{
    ...
    do
    {
        len = recvfrom( data->socketFd,  (char *)buffer,
                            bufSize,  0 /*flags */ , (sockaddr*) &xSrc, (socklen_t*) &srcLen);
    ...
    {
        len = recvfrom( data->socketFd,  (char *)buffer,  bufSize,
                        0 /*flags */ , (struct sockaddr*) &xSrc, (socklen_t*) &srcLen);
		if(len>=0)
		{
			char buf[512];
			char *ptr = NULL;
			struct sockaddr_in *addr = (struct sockaddr_in *)&xSrc;
			ptr = (char*)inet_ntop(AF_INET, &addr->sin_addr, buf, 511);
			
			cpDebug(LOG_INFO,"RX address is %s", ptr);
		}
    }
    else
    {
        ipV6 = true;
        int flags=0;
        len = recvfrom_flags( data->socketFd, (char *)buffer, bufSize,
                        &flags /*flags */ ,  (struct sockaddr*) &xSrc,  (socklen_t*) &srcLen,  &pktinfo);
    }
    ...
    else
    {
	    ...
	    if (err) {
            ...
            cpDebug(LOG_DEBUG, "***Received from:%s:%s", tmp.c_str(), port);
...
//#ifndef WIN32
    ...
    {
        cpDebug(LOG_INFO, "Interface (%s) index (%d) joining multicast group (%s)",
        ...
        {
           cpDebug(LOG_INFO, "Joined multi-cast group");
        }
    }
    else 
    {
...
#define CMSG_NXTHDR(m, c)                                               \
             ...
             cmptr = CMSG_NXTHDR(&msg, cmptr))
    {
        ...
        cpDebug(LOG_DEBUG, "unknown ancillary data, len = %d, level = %d, type = %d",
