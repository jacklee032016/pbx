/*
 * $Id: Connection.cxx,v 1.3 2007/03/03 18:38:56 lizhijie Exp $
 */
...
Connection::readn(void *dataRead, size_t nchar, int & bRead) throw (VNetworkException&)
{
    ...
    while (nleft > 0)
    {
        if ((nread = iread(ptr, nleft)) < 0)
        {
            ...
            {
                //cpDebug(LOG_DEBUG, "Received EINTR or EAGAIN, keep trying");
                nread = 0;
            }
            else
            {
...
Connection::writeData(void* data, size_t n) throw (VNetworkException&)
{
    ...
    //cpDebug(LOG_DEBUG, "Bytes written:%d" , nwritten);
}
...
Connection::close()
{
    cpDebug(LOG_DEBUG_STACK, "Closing connection %d", _connId);
    int err = 0;
    if(_connId > 2)
    {
        err = iclose();
...
Connection::isReadReady(int seconds, int mSecconds) const
{
    ...
    //cpDebug(LOG_DEBUG, "Checking connection %s", getDescription().c_str());
    FD_ZERO(&rfds);
    ...
    {
        cpDebug(LOG_DEBUG_STACK, "Data is ready on fd %d : %s", connId, getDescription().c_str());
        return true;
    }
    return false;
}
