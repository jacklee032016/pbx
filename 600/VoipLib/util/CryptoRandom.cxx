/*
 * $Id: CryptoRandom.cxx,v 1.2 2007/03/01 20:04:32 lizhijie Exp $
 */
...
int CryptoRandom::getCryptoRandom(unsigned char* buf, int num)
{
    ...
    if ( fp == 0 )
    {
       cpDebug(LOG_INFO, "CryptoRandom::unable to open /dev/random. Use pseudo random instead.");
    ...
    else
    {
       ...
       {
          cpDebug(LOG_INFO, "CryptoRandom::Error reading from /dev/random (%d/%d) ferror=%d. Use pseudo random instead.", len, num, ferror(fp));
          fclose(fp);
          return getPseudoRandom(buf, num);
       }
    }
