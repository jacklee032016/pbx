/*
 * $Id: DnsResolver.cxx,v 1.3 2007/06/05 11:33:26 lizhijie Exp $
 */
                         ...
                         myQType(qType)
{
    ...
    else                     // assume SRV record type
    {
        ...
        if (serviceType)
        {
            cpDebug(LOG_DEBUG, "serviceType = %s", serviceType);
            s = serviceType;
            s+= ".";
        }
        if (protocol)
        {
            cpDebug(LOG_DEBUG, "protocol = %s", protocol);
            p = protocol;
            p+= ".";
        }
        s += p;
        cpDebug(LOG_DEBUG, "s+p = %s", s.c_str());

        s += target;
        cpDebug(LOG_DEBUG, "s+p+target = %s", s.c_str());
        myQName.assign( s );
    }
    cpDebug(LOG_DEBUG, "myQName = %s", myQName.c_str());
...
{
    //  deallocate memory
    cpDebug(LOG_DEBUG, "entered DnsResolver destructor");
    clear();
}
...
void DnsResolver::dnsLookUp()
{
    ...
    {
        cpDebug(LOG_DEBUG, "no srv records, try finding A records:");
...
Sptr<DnsRecord> *  /* VECTOR_DNS::iterator */ DnsResolver::getRecord( int idx )
{
    ...
    {
       cpDebug(LOG_DEBUG, "getRecord() fails: bad index %d", idx);
       return myVDns.end();
    }
    return(&(myVDns[idx]));
}
...
DnsResolver::nsError( int error, const char* domain )
{
	switch(error)
	{
		case HOST_NOT_FOUND:
		{
			cpDebug(LOG_DEBUG, "Unknown domain: %s\n", domain);
		}
		break;   
		case NO_DATA:    // same as NO_ADDRESS
		{
			cpDebug(LOG_DEBUG, "No DNS records for %s\n", domain);
		}
		break;
		case NO_RECOVERY:
		{
			cpDebug(LOG_DEBUG, "non-recoverable error occurs during nameserver lookup for %s\n", domain);
		}
		break;
		case TRY_AGAIN:
		{
			cpDebug(LOG_DEBUG, "No response for DNS query\n");
		}
		break;
		default:
		{
			cpDebug(LOG_DEBUG, "Unexpected error=%d, domain=%s\n",error, domain);
		}
		break;
	}
}   // end of nsError()
...
DnsResolver::findRecords()
{
    ...
    {
        cpDebug(LOG_INFO, "ns_initparse: %s\n",strerror(errno));
        return false;
    };
...
DnsResolver::loadBalance( )
{
    ...
    while (i != myVDns.end())
    {
        ...
        if (startIdx == endIdx) 
        {
            cpDebug(LOG_DEBUG, "range of 1 found for priority=",  prio);
            i++;
        }
        else
        {
...
void DnsResolver::buildRecords(ns_msg handle, ns_sect section, int responseLen)
{
    ...
    cpDebug(LOG_DEBUG, "Response Length is: %d\n"
    ...
    if (myQType == ns_t_srv)
	{
        cpDebug(LOG_DEBUG, "building SRV records" );
        buildSRVRecords( handle, section, responseLen, numRecords );
    }
	else if (myQType == ns_t_a)
    {
        cpDebug(LOG_DEBUG, "building A records");
        buildARecords( handle, section, responseLen, numRecords );
    }
	else
	{
        cpDebug(LOG_DEBUG, "don't know how to build record type=%d", myQType );
	}
}  // buildRecords
                                  ...
                                  int responseLen, int numRecords)
{
    ...
    for(rrnum = 0; rrnum < numRecords; rrnum++)
    {
        ...
        {
            cpDebug(LOG_DEBUG, "ns_parserr: %s, skip rr #%d\n", 
		...
		if (rrt != ns_t_srv)
        {
            cpDebug(LOG_DEBUG, "skipping record number: %d, type: %d", 
							rrnum, rrt);
            continue;
        }
        ...
        {
            cpDebug(LOG_DEBUG,"Warning: Nameserver is not authoritative.\n");
        }
        ...
        cpDebug(LOG_DEBUG, "Target/address = %s/%s", name,
              rec->getAddress());
        ...
        // cpDebug(LOG_DEBUG, "Capacity of myVDns = %d", myVDns.capacity());
    ...
    myReady = true;
    cpDebug(LOG_DEBUG, "Number of DNS SRV records = %d\n", myNumDns);
}  // buildSRVRecords()
                               ...
                               int responseLen, int numRecords)
{
    ...
    for(rrnum = 0; rrnum < numRecords; rrnum++)
    {
		...
		if (rrt != ns_t_a)
        {
            cpDebug(LOG_DEBUG, "skipping record num: %d, type: %d", 
							rrnum, rrt);
            continue;
        }
        DnsRecord *rec = new DnsRecord;
        ...
        if (ns_parserr(&handle, section, rrnum, &rr))
        {
            cpDebug(LOG_INFO, "ns_parserr: %s\n", strerror(errno));
        }
        /* Check to see if answer is authoritive */
        if(!ns_msg_getflag(handle,ns_f_aa))
        	{
            cpDebug(LOG_INFO,"Warning: Nameserver is not authoritative.\n");
        	}
    ...
    myNumDns = myVDns.size();
	myReady = true;
    cpDebug(LOG_DEBUG, "Number of A records found = %d\n", myNumDns);
}  // buildARecords()
...
DnsResolver::sortByWeight( int startIdx, int endIdx )
{
    ...
    cpDebug(LOG_DEBUG, "sortByWeight(): startIdx=%d, endIdx=%d", 
           startIdx,endIdx );
   ...
   if (sumOfWeights == 0)
   {
       cpDebug(LOG_DEBUG,  "sum of weights is 0" );         
   ...
   for (int i = startIdx; i < endIdx; i++)
   {
       ...
       for (j = i; j <= endIdx; j++)
       {
           ...
           if ( randWeight < curWeight )
           {
	           ...
	           cpDebug(LOG_DEBUG_STACK, "randWeight=%d, curWeight=%d, i=%d, j=%d",
...
{
    cpDebug(LOG_INFO, "findPriorityRange()" );
    ...
    cpDebug(LOG_DEBUG, "findPriorityRange(): prio=%d, startIdx=%d",
           prio, startIdx );
