/*
 * $Id: RtpSession.cxx,v 1.3 2007/06/05 11:33:25 lizhijie Exp $
 */
                                      ...
                                      int jitterNew)
{
	...
	if (tran)
	{
		cpDebug(LOG_DEBUG_STACK, "RTP Tran Port: %d", tran->getUdpStack()->getDestinationPort());
	}
	if (recv)
	{
		cpDebug(LOG_DEBUG_STACK, "RTP Recv Port: %d", recv->getPort());
	}
	if (rtcpTran)
	{
		cpDebug(LOG_DEBUG_STACK, "RTCP Tran Port: %d", rtcpTran->getUdpStack()->getDestinationPort());
	}
	if (rtcpRecv)
	{
		cpDebug(LOG_DEBUG_STACK, "RTCP Recv Port: %d", rtcpRecv->getPort());
	}
}
...
int RtpSession::reserveRtcpPort(int rtcpLocalPort, int portRange)
{
    int port = 0;
