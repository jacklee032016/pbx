/*
 * $Id: rtpCodec.cxx,v 1.3 2007/06/05 11:33:25 lizhijie Exp $
 */
	...
	switch (fromType)
	{
		case rtpPayloadPCMU :
		{
			switch (toType)
			{
				...
				default :
				{
					cpDebug(LOG_DEBUG, "Unsupported converstion %d -> %d", fromType, toType);
					break;
				}
			}
		}
		...
		case rtpPayloadL16_8k_mono :
			{
				switch (toType)
				{
					...
					default :
					{
						cpDebug(LOG_DEBUG, "Unsupported conversion %d -> %d",	fromType, toType);
						break;
					}
				}
			}
...
char* findSilenceCodec( RtpPayloadType type, int len )
{
    for( int i = 0; i < numRtpCodecInfo; i++ )
    {
        ...
        {
            cpDebug(LOG_DEBUG_STACK, "Silence packet template found at index %d", i );
            return (char*)&rtpCodecInfo[i].silence;
        }
    }

    cpDebug(LOG_DEBUG_STACK, "Silence unknown for type:%d, len%d", type, len );
    return 0;
}
...
char* findSilenceCodecString( char* codecStringInput, int len )
{
    ...
    cpDebug(LOG_DEBUG_STACK, "Silence unknown for type:%s, len%d", codecStringInput, len );
    return 0;
}
