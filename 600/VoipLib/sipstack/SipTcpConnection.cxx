/*
 * $Id: SipTcpConnection.cxx,v 1.4 2007/06/05 06:08:54 lizhijie Exp $
 */
...
NTcpConnInfo::getStatusMsgConn(Sptr < SipMsg > msg)
{
	...
	{
		cpLog(LOG_WARNING, "could not find TCP connection for status msg (%s) reply", msg->encode().logData());
		return 0;
	}
}
...
void NTcpConnInfo::setConnNSenderIp(int fd, Sptr < Connection > conn, const Data& ip)
{
	LockHelper l(mutex);
        ...
        shutdownNow(false)
{
	...
	tcpConnInfo.setConnNSenderIp(fd, myConn, Vocal::theSystem.gethostAddress());
...
void SipTcpConnection_impl_::send(SipMsgContainer* msg, const Data& host,const Data& port)
{
	Data nhost;
	int nport=0;

#if 1
	/* Lizhijie, comment following to make TCP connection can send from both LAN and WAN 
	* this is just involved destination IP address 
	*/
	if(host.length())
	{
		nhost = host;
...
#endif		
	{
		...
		if(command.dynamicCast(msg->msg.in)!=0)
		{
			...
			if(sipDest != 0)
			{
				...
				cpDebug(LOG_DEBUG_STACK, "got host from request line: %s:%d", nhost.logData(), nport);
				if ( nport == 0 )
				{
					nport = SIP_PORT;
	...
	if(nhost.length())
	{
		...
		{
			cpLog(LOG_ERR, "Destination (%s) is not reachable, reason:%s.", nhost.logData(), e.getDescription().c_str());
		}
	}
...
void* SipTcpConnection_impl_::receiveThreadWrapper(void* p)
{
	return static_cast < SipTcpConnection_impl_* > (p)->receiveMain();
}


void* SipTcpConnection_impl_::processThreadWrapper(void* p)
{
	return static_cast < SipTcpConnection_impl_* > (p)->processMain();
}


void* SipTcpConnection_impl_::sendThreadWrapper(void* p)
{
	return static_cast < SipTcpConnection_impl_* > (p)->sendMain();
}

void* SipTcpConnection_impl_::sendMain()
{
	while (1)
	{
...
void* SipTcpConnection_impl_::processMain()
{
	while (1)
	{
...
Sptr <Connection> SipTcpConnection_impl_::createOrGetPersistentConnection(NetworkAddress nwaddr)
{
	//come here only if this is a command.
	...
	if(myDestinationMap.count(buf))
	{
		...
		{//close the connection , remove it from the map that would re-establish it
			cpDebug(LOG_ERR, "Stale connection from (%s), re-connecting..", buf);
	...
	try
	{
		...
		cpDebug(LOG_DEBUG_STACK, "Adding persistent connection %d", connection->getConnId());
...
void SipTcpConnection_impl_::prepareEvent(SipMsgContainer* sipMsg)
{
	...
	if (conn != 0)
	{
		...
		cpDebug(LOG_INFO, "sending TCP SIP msg:\n\n-> %s\n\n%s", conn->getDescription().c_str(), dataString.c_str());
...
int getContentLength(const string& str)
{
	LocalScopeAllocator lo;
...
#if 0	
	/* Li Zhijie, 2007.06.05 , change as following for bug when content-length is 
	* in the form of "content-length:   339"
	*/
...
#else
	string contentLen = str.substr(pos + 1, str.length() - (pos + 1));
#endif
	return (atoi(contentLen.c_str()));
}

bool isBlankLine(const string& str)
{
...
int Vocal::isFullMsg(const string& str)
{
	...
	if (foundBlank)
	{
		...
		cpDebug(LOG_INFO, "TCP SIP msg :remainingByte : %d, contentLength : %d, totalLength : %d", 
			remainingBytes, contentLen, str.length() );
...
void SipTcpConnection_impl_::processMsgsIfReady(int fd)
{
	Data data;
	string ipName;
	...
	cpDebug(LOG_DEBUG_STACK, "Raw Msg read %d: [%s]",bufLen, myBuf.c_str());
	
	while (bufLen != -1)
	{
		string msgBuf = myBuf.substr(0, bufLen);
		string remainingBuf = myBuf.substr(bufLen, myBuf.length() - bufLen);
		...
		if (sipMsg->msg.in != 0)
		{
			...
			cpDebug(LOG_DEBUG_STACK, "*** get SenderIP = %s***", ipName.c_str());
			sipMsg->msg.in->setReceivedIPName( ipName );
			cpDebug(LOG_INFO, "Received TCP Message :\n\n<- HOST[%s]\n\n%s",ipName.c_str(),  data.logData());
				...
				statusMsg->getStatusLine().getStatusCode() >= 200)
			{
				...
				{
					cpDebug(LOG_DEBUG_STACK, "Deleting non-persistent connection %d", fd);
...
void SipTcpConnection_impl_::readOnFdSet(fd_set* fdSet, TcpServerSocket* tcpStack)
{
	...
	for(list<int>::iterator itr = fdList.begin(); itr != fdList.end(); itr++)
	{
		int mapfd = (*itr);
...
void* SipTcpConnection_impl_::receiveMain()
{
	fd_set netFD;
	...
	while (1)
	{
		...
		else if (selret < 0)
		{
			...
			if ( errno != EINTR )
			{
				...
				cpLog(LOG_ALERT, "select() failed, reason: %s, %d", strerror(errno), errno);
			}
		}
		else if (selret == 0)
		{
...
void SipTcpConnection::send(SipMsgContainer* msg, const Data& host, const Data& port)
{
