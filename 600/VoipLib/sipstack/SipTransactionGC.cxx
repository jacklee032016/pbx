/*
 * $Id: SipTransactionGC.cxx,v 1.3 2007/06/04 17:59:33 lizhijie Exp $
 */
...
{
    cpDebug(DEBUG_NEW_STACK,"Garbage Collector thread running....");
    Fifo <int> x;
    x.addDelay(1, 1000);
    while(!doneFlag)
    {
...
SipTransactionGC::trash(SipMsgContainer* item)
{
	   ...
	   item->level3Ptr->msgs.request)
	{
		...
		cpDebug(DEBUG_NEW_STACK,"trashing at level-II:%s", level2Node->myKey.logData());
		delete level2Node;
	    }
	    cpDebug(DEBUG_NEW_STACK,"trashing at level-III:%s", level3Node->myKey.logData());
	    delete level3Node;
	}
    }
	
    cpDebug(DEBUG_NEW_STACK,"trashing item:[%s]",item->msg.out.c_str());
    delete item;
