/*
 * $Id: SipUdp_impl.cxx,v 1.4 2007/06/05 06:08:54 lizhijie Exp $
 */
...
{      
	...
	while (1)
	{
		...
		else if(recFifo->size() <= 10000 )
		{
			...
			*/
			...
			{// this message failed to decode
				cpDebug(LOG_DEBUG, "received message did not decode\n%s" , sipMsg->msg.out.logData());
		...
		{
			cpDebug(LOG_DEBUG,  "***Ignoring received msg ***:%d" , recFifo->size());
		}
...
void* SipUdp_impl::sendThreadWrapper(void *p) 
{
    return static_cast<SipUdp_impl*>(p)->sendMain();
}

    
int SipUdp_impl::udpSend(SipMsgContainer *sipMsg)
{
	// send it 
	int ret_status = 0;
	...
	try
	{
		...
		nAddr.getIpName().c_str(),nAddr.getPort(), sipMsg->msg.out.logData());
...
void* SipUdp_impl::rcvThreadWrapper(void *p) 
{
			...
			if (sipMsg != 0)   
			{
				...
				if (count)
				{
					...
					if(!randomLosePercent || ((random() % 100) >= randomLosePercent))
					{
						...
						if ( ret_status)
						{
							if  ((sipMsg != 0) && (sipMsg->msg.type != SIP_STATUS)) 
							{
								Data toBeEaten = sipMsg->msg.out;
								...
								retVal->msg.in=new StatusMsg((*commandMsg), ret_status);
				...
				else //i.e. count == 0
				{
					...
					if (retransmit->getCount())
					{
						...
						if ((sipMsg->msg.type != SIP_ACK) && (sipMsg->msg.type != SIP_STATUS)) 
						{
...
void SipUdp_impl::getHostPort(Sptr<SipMsg> sipMessage, Data* host, int* port)
{       
...
void SipUdp_impl::send(SipMsgContainer* msg, const Data& host, const Data& port)
{ 
	...
	if((msg->msg.in != 0) && 	(((msg->msg.in->getType() == SIP_STATUS) &&
		(msg->msg.in->getCSeq().getMethod() != "INVITE")) || (msg->msg.in->getType() == SIP_ACK)))
	{
		// clear msg.in
		msg->msg.in = 0;
	}
	...
	{
		cpDebug(LOG_DEBUG_STACK, "Msg is for retransmission:\n %s",  msg->msg.out.logData());
...
void SipUdp_impl::reTransOn()
{
	Udpretransmitoff = false;
}


void SipUdp_impl::reTransOff()
{
	Udpretransmitoff = true;
}


void SipUdp_impl::setRetransTime(int initial, int max)
{
...
void SipUdp_impl::setRandomLosePercent(int percent)
{
...
void SipUdp_impl::printSize()
{
	cpDebug(LOG_INFO, "recFifo: %d, sendFifo: %d", recFifo->size(),sendFifo.size());
}

Data SipUdp_impl::getDetails() const
{
	char buf[256];
	sprintf(buf, "receive fifo: %d    send fifo: %d", recFifo->size(),sendFifo.size());
	sprintf(buf, "\npkts received: %d   pkts sent: %d", udpStack.getPacketsReceived(),udpStack.getPacketsTransmitted());
	return buf;
}
