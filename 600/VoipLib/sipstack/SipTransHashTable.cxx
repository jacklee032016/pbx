/*
 * $Id: SipTransHashTable.cxx,v 1.3 2007/06/04 17:59:33 lizhijie Exp $
 */
...
\**************************************************************************/
	...
	if(currNode == node)
	{
	    *prevNodePtr = currNode->next;
	    cpDebug(DEBUG_NEW_STACK,"erasing top node[%s]", node->keyCopy.logData());
	    delete node;
	    break;
	}
	else
 ...
 */
    ...
    cpDebug(DEBUG_NEW_STACK,"%srehashing from %d (old size) to %d...%s",
	  "##########################################\n",
	  size,sizeHint,
	  "\n##########################################");
