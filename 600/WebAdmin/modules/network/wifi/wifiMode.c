/*
* $Id: wifiMode.c,v 1.4 2007/09/06 09:19:59 lizhijie Exp $
*/
...
char *__wifi_mac_mode_status(WIFI_INFO *info)
{
	char 	buf[8192*5];
	int 		length = 0;
	int 		i;
	...
	length += CGI_SPRINTF(buf, length, "\t<TR><TD align=\"right\"><strong>%s:</strong></TD><TD>\n\t<select name=\"%s\" style='width:120px;'>\n", 
		gettext(CGI_STR_WORK_MODE), WIFI_KEYWORD_MODE );
	for(i= 0; i< sizeof(_wifimacs)/sizeof(_wifimacs[0]); i++)
	{
		length += CGI_SPRINTF(buf,length, "\t\t<option value=\"%s\" %s>%s</option>\n", 
			_wifimacs[i].name, (strcasecmp(GET_VALUE(&info->globalCfgs, WIFI_KEYWORD_MODE), _wifimacs[i].name) )?"":"selected",  gettext(_wifimacs[i].description) );
	}
	length += CGI_SPRINTF(buf,length, "\t</select>\n</TD></TR>\r\n");

	length += CGI_SPRINTF(buf, length, "\t<TR><TD align=\"right\"><strong>%s:</strong></TD><TD>\n\t<select name=\"%s\" style='width:120px;'>\n", 
		gettext(CGI_STR_PHY_MODE), WIFI_KEYWORD_PHY_MODE );
	for(i= 0; i< sizeof(_wifiphys)/sizeof(_wifiphys[0]); i++)
	{
		int phy = atoi(GET_VALUE(&info->globalCfgs, WIFI_KEYWORD_PHY_MODE) );
		length += CGI_SPRINTF(buf,length, "\t\t<option value=\"%d\" %s>%s</option>\n", 
			_wifiphys[i].type, (phy==_wifiphys[i].type) ?"selected":"",  _wifiphys[i].description);
	}
	length += CGI_SPRINTF(buf,length, "\t</select>\n</TD></TR>\r\n");
...
char *__wifi_params_status(WIFI_INFO *info)
{
	char 	buf[8192*5];
	int 		length = 0;
	int 		j;
...
char *__wifi_result_info(WIFI_INFO *info)
{
	char 	buf[8192*5];
	int 		length = 0;
	int 		i, j;
	...
	for(j=0; j<sizeof(_wifiphys)/sizeof(_wifiphys[0]); j++)
	{
		if( i== _wifiphys[j].type )
		{
			sprintf(buffer, "%s", _wifiphys[j].description );
			isValidate = 1;
		}	
	}
	if(! isValidate)
...
int	wifi_Mode(WIFI_INFO *info)
{
	...
	cgi_wifi_save_config(info);

	if(!strcasecmp(action, CGI_ACTION_DEFAULT) )
	{/* mode save and display basic parameters */
		cgi_info_page(gettext("Basic Parameters of WiFi"), gettext("Basic parameters of WiFi are set here"), __wifi_params_status(info));
		return 0;
	}

	if(!strcasecmp(action, CGI_ACTION_ADD) )
	{/* mode save */
		cgi_info_page(gettext("Basic Parameters of WiFi"),  __wifi_result_info(info), "");
		return 0;
	}
