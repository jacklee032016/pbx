/*
* $Id: cgiStcCommand.c,v 1.1 2007/05/02 21:09:12 lizhijie Exp $
* read command from CGI and send it to serial port
*/
#include "_cgi_stc.h"

static void __feedback_delayed_signal_to_cgi(int pid, int opCode, int responseCode)
{
	sigset_t			tmpst;
	siginfo_t			tmpinfo;
...
void stc_handler_cgisig(int signo, siginfo_t *info,void * val)
{
	...
	switch(info->si_int)
	{
		case OP_PBX_WRITE_REQ:
			...
			__feedback_delayed_signal_to_cgi(info->si_pid, OP_PBX_WRITE_RPL, OP_PBX_WRITE_OK);

			break;
			
		case OP_PBX_READ_REQ:
			__feedback_delayed_signal_to_cgi(info->si_pid, OP_PBX_READ_RPL, OP_PBX_READ_OK);
			break;
