...
int read_dhcp_conf(char *filename,char *startname,char *dhcpmask,int u,llist l)
{
	...
	system("rm /tmp/tmp2.txt");
	readconfig("/tmp/tmp3.txt",ITEM_DBL,NULL, &l);
	writeconfig_no_override(CONF_DH,dhcp_dealline,&l);
	system("rm /tmp/tmp3.txt");
	return 0;
}	
...
//		n=check_ip_entries_error(l);
	...
	if(webType == WEB_TYPE_WWW)
	{
		...
		sprintf(title, "%s", gettext(CGI_STR_WAN_CONFIG) );
	...
	//system("/sbin/network-setup");
	//fork_cmd("/sbin/network-setup &");
	//lan_update_host_conf();
	fflush(stdout);
	
	kill_process("adsl-connect");
	kill_process("pppd");
	kill_process("udhcpc");

	/*activate new configuration */
//	system("/sbin/network-setup");
	fork_cmd("/sbin/network-setup > /dev/null ");
	
	cgi_syscfg_update_host_conf();

#if 0
	cgi_boa_modify_restart(webType);
#endif
...
int cgi_syscfg_write_nic_config(llist *l, syscfg_type_t type)
{
	...
	if(!strcasecmp(dev, ETH0) )
	{
...
#if 0
		FILE *fp;
		node *pnode, *qnode;// *rnode;
		pnode=cgi_locatename(l, "nameserver", 1);
		qnode=cgi_locatename( l, "nameserverdns",1);
			
		#if 1
		system("rm -f /etc/sys/ppp0");
		//rnode=cgi_locatename(l,"GATEWAY",1);
		//if(rnode)
		//{
		//	sprintf(cmd,"/sbin/route add -net 0.0.0.0 netmask 0.0.0.0 gw %s eth0",rnode->entry.value);
		//	add_default_route_to_script(cmd);
		//}
		#endif
		...
		if(!fp)
		{
			cgi_error_page(NULL, gettext(CGI_STR_ERROR), gettext("can't open file DNS Configuration File") );
			return 1;
		}
		...
		{
			fprintf(fp,"%s\t%s\n",pnode->entry.name,pnode->entry.value);
//			list_delete_node( l, pnode);
		}
		if(qnode)
		{
			fprintf(fp,"%s\t%s\n","nameserver",qnode->entry.value);
//			list_delete_node( l, qnode);
		}
		fclose(fp);
#endif		
	}
...
#define	CONF_HOSTNAME		"/etc/hosts"
	...
	sprintf(cmd,"ifconfig >/etc/tmp/ifconfig.txt");
	system(cmd);
	memset(cmd,0,strlen(cmd));
		
	memset(&wan,0,sizeof(interface_info) );
	memset(&lan,0,sizeof(interface_info) );
	...
	if(0 != strlen(lan.addr) )
		fprintf(fp,"%s\t\t%s\t\t%s.%s\n",lan.addr, hostname,  hostname,domain );
...
void cgi_syscfg_wan_activate_dhcp(llist *cgiVariables , syscfg_type_t type)
{
	...
	system(target);
//	system("/sbin/network-setup >/tmp/udhcpc.tmp");
	system("/sbin/wan.sh >/tmp/udhcpc.tmp");
...
void cgi_syscfg_wan_activate_pppoe(llist *cgiVariables )
{
	...
	sleep(2);
	 /*
		   ...
		   */
	CGI_SPRINTF(cmd, 0, "/usr/sbin/adsl-connect > %s &", PPPOE_LOG);
	system(cmd);
	//fork_cmd(cmd);	
