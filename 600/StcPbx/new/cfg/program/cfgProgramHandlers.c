/*
* $Id: cfgProgramHandlers.c,v 1.4 2007/09/13 20:13:09 lizhijie Exp $
*/
/*
...
{
#if 0
	if(callDevices[extIndex].status == PBX_STATE_BUSY)
	{
//		dtmfEncodeChannelAllocate(extIndex, extConfigs.phoneNumbers[extIndex].value, extConfigs.phoneNumbers[extIndex].length );
		CALL_EVENT_EMIT(extIndex, PBX_SIGNAL_RINGING);
		return PBX_STATE_CONTINUE;
	}
	if(callDevices[extIndex].status == PBX_STATE_IDLE )
	{
...
#endif	
		RECORDER_REC_START();
		CALL_TIMER_START(extIndex, RECORD_TIMER_LENGTH);
	}
	else
	{
		CALL_EVENT_EMIT(extIndex, PBX_EVENT_TIMEOUT);
...
/********** cfg itesm with dialed digits as '**1x' and gas only one parameter *********/
	...
	{/*Console Phone Number : '**26' */
		SET_BIT(2, 4)|SET_BIT(6, 0),
		CMD_CFG_CONSOLE_PHONENUMBER,	
		program_cfg_passwd,
	},

	{/*Console Phone Number : '**27' */
		SET_BIT(2, 4)|SET_BIT(7, 0),
		CMD_CFG_CONSOLE_EXT_INDEX,	
		program_cfg_global_options,
	},
