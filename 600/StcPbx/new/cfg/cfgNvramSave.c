/*
* $Id: cfgNvramSave.c,v 1.3 2007/09/13 20:13:00 lizhijie Exp $
*/
...
void nvram_save_int_value(INT8U cmd, INT8U value)
{
	switch(cmd)
	{
		...
		case CMD_CFG_IP_PREFIX:
			...
			break;
		case CMD_CFG_CONSOLE_PHONENUMBER:
			if(value<1 || value >DIAL_DIGIT_SPECIAL )
				value = DIAL_DIGIT_SPECIAL;
			baseConfigs.consolePhoneNumber.length = 1;
			baseConfigs.consolePhoneNumber.value[0] = (PBX_GET_LOW_NIBBLE(value)<<4);
			NVRAM_MODIFY_GLOBAL();
		...
		case CMD_CFG_CONSOLE_EXT_INDEX:
			if( IS_EXT_DEVICE(value-1) )
			{
				baseConfigs.extIndexOfConsole = value-1;
				NVRAM_MODIFY_GLOBAL();
			}
			break;
...
void nvram_save_2_int_value(INT8U cmd, INT8U index, INT8U value)
{
	...
	switch(cmd)
	{
		...
		case CMD_CFG_CONSOLE_PHONENUMBER:
			if( (value<0) || (value > DIAL_DIGIT_SPECIAL) )
				value = DIAL_DIGIT_SPECIAL;
			if( (index<0) || (index > DIAL_DIGIT_SPECIAL) )
				index = DIAL_DIGIT_SPECIAL;
			baseConfigs.consolePhoneNumber.length = 2;
			baseConfigs.consolePhoneNumber.value[0] = (PBX_GET_LOW_NIBBLE(index)<<4)|PBX_GET_LOW_NIBBLE(value);
			NVRAM_MODIFY_GLOBAL();
			break;
...
void nvram_save_values(INT8U cmd, INT8U xdata *value, INT8U length)
{
	switch(cmd)
	{
		...
		case CMD_CFG_CONSOLE_PHONENUMBER:
			if( (length>0) && (length <= 2*PHONE_NUMBER_BUF_SIZE) )
			{
				ipdigit_2_pbxdigit( baseConfigs.consolePhoneNumber.value, value, length);
				baseConfigs.consolePhoneNumber.length = length;
				NVRAM_MODIFY_GLOBAL();
			}
			break;
...
void nvram_save_values_4_program(INT8U cmd, INT8U xdata *value, INT8U length)
{
	...
	switch(cmd)
	{
		...
		case CMD_CFG_CONSOLE_PHONENUMBER:
			...
			{
				baseConfigs.consolePhoneNumber.length = length;
				pbx_x_str_copy( baseConfigs.consolePhoneNumber.value, value, (IS_ODD(length))?(length/2+1):length/2 );
				NVRAM_MODIFY_GLOBAL();
			}
			break;
