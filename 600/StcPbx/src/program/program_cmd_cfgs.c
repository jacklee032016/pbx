/*
* $Id: program_cmd_cfgs.c,v 1.9 2007/07/25 17:55:38 lizhijie Exp $
*/
...
//	DEVICE_TIMER_START(extIndex, 10*PBX_100MS);
	PBX_PROGRAM_MODE_CANCEL();
	
	if( (channel = decodingchannel_find(extIndex)) != PBX_VALUE_INVALIDATE)
	{
...
INT8U	program_cfg_2_params(INT8U extIndex,INT8U cmd,  phone_digits_t xdata *dialingDigits)
{
	...
	if(dialingDigits->length >= 6)
	{
		if(cmd == PROGRAM_CMD_OP_REPLACE )
		{
			...
			if(IS_EXT_DEVICE(index) && deviceStatus[index].status == STATE_RINGING )
			{
				DEVICE_EVENT_EMIT(index, PBX_SIGNAL_CANCEL);

				deviceStatus[extIndex].peerId = deviceStatus[index].peerId;
				deviceStatus[deviceStatus[extIndex].peerId].peerId = extIndex;
				deviceStatus[index].peerId = PBX_VALUE_INVALIDATE;

				if( pbx_voice_channel_connect(extIndex, deviceStatus[extIndex].peerId ))
				{
					DEVICE_EVENT_EMIT(deviceStatus[extIndex].peerId, PBX_SIGNAL_OK);
