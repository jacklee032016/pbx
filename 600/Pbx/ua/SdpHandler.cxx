/*
 * $Id: SdpHandler.cxx,v 1.3 2007/03/03 18:39:28 lizhijie Exp $
 */
...
void 
addSdpToMsg( SipMsg& msg, Data rtpRate, int rtpPort )
{
...
int
getRtpPacketSize ( SipSdp& remoteSdp )
{
    Data rtpPacketSize;
...
setStandardSdp(SipSdp& sipSdp, Data hostAddr, int port)
{
    ...
    setRtpPacketSize(sipSdp, UaConfiguration::instance()->getNetworkRtpRate());
}
...
{
    LocalScopeAllocator lo;
    Sptr < UaHardwareEvent > ringbackSignal 
    ...
    ringbackSignal->signalOrRequest.request.remotePort 
        = remoteSdp.getRtpPort();

    strcpy(ringbackSignal->signalOrRequest.request.remoteHost, 
           remoteSdp.getConnAddress().getData(lo));
   ...
   ringbackSignal->signalOrRequest.request.rtpPacketSize = getRtpPacketSize(remoteSdp);
    ...
    {
        ringbackSignal->signalOrRequest.request.rtpPacketSize 
            = UaConfiguration::instance()->getNetworkRtpRate();
    }
...
void startAudio( Sptr < SipSdp > localSdp, Sptr < SipSdp > remoteSdp, int deviceId )
{
    ...
    if(remoteSdp->getSdpDescriptor().isHold())
    {
        cpDebug(LOG_DEBUG, "Stopping audio (remote SDP == hold)");
    ...
    else
    {
        ...
        /**
         * Echo cancellation is always true, since there is no code to
         * handle SDP echo cancellation negotiation.
         */
        request->echoCancellation = true;
        
        // Set rtpPacketSize to default in ua.cfg file
        int rtpPacketSize = getRtpPacketSize(*localSdp);
        
        if(rtpPacketSize <= 0)
        {
            rtpPacketSize = UaConfiguration::instance()->getNetworkRtpRate();
        }
