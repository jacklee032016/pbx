/*
 * $Id: Registration.cxx,v 1.6 2007/03/21 18:12:39 lizhijie Exp $
 */
...
Registration::Registration(const RegisterMsg& srcMsg, unsigned int cseq)
        :
        seqNum(cseq),
        status(0),
        registerMsg(srcMsg)
...
SipContact Registration::findMyContact(const StatusMsg& msg) const
{
...
int Registration::updateRegistrationMsg(const StatusMsg& msg)
{
	...
	if ( status >= 300 && status <= 500 )
	{
		...
		{
			delay = DEFAULT_DELAY;
			cpLog( LOG_WARNING, "Register failed, Will try again in 60 seconds" );
		}
	}
	...
	if ( status == 401 || status == 407 )
	{
...
#else		
		Data user = msg.getTo().getUser().convertString();/* phone number for local dial plan */
#endif
		Data password = UaConfiguration::instance()->getPassword();
		...
		/* added this lines for complimant with authentication register request which is distinct with Branch in Via header, lzj 2007.03.06 */
...
//		sipvia.setHost(hip);
		...
		int cseq ( msg.getCSeq().getCSeqData().convertInt() );
		{
			if( cseq > 1 )
			{
				...
				// default delay to allow the user time to figure it out.

				cpDebug(LOG_ERR, "Authentication may have failed, check configuration info");
				delay = DEFAULT_DELAY;
			}
		}
		
		cpDebug( LOG_WARNING, "Will try Registration again with authentication information" );
	}

	return delay;
}

void Registration::setRegistrationMsg(const RegisterMsg& msg)
{
	registerMsg = msg;
...
Registration& Registration::operator = (const Registration& src)
{
	status = src.status;
	seqNum = src.seqNum;
	registerMsg = src.registerMsg;
...
{
	// Cast away const until someone fixes RegisterMsg's equality operator.
	Registration &  left = const_cast<Registration &>(*this);
	return ( left.registerMsg == right.registerMsg );
}

int Registration::getDelay()
{
