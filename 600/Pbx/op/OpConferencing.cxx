/*
 * $Id: OpConferencing.cxx,v 1.3 2007/03/16 19:33:56 lizhijie Exp $
 */
...
const char* const OpConferencing::name() const
{
	return "OpConferencing";
}

const Sptr <State> OpConferencing::process( const Sptr <SipProxyEvent> event )
{
    ...
    if ( status >= 200 )
    {
        ...
        if ( status == 200 )
        {
            ...
            try
            {
                Data urlStr = Data( string("sip:") + UaConfiguration::instance()->getConfServer() );
        ...
        else
        {
            ...
            cpDebug(LOG_DEBUG, "Transfer target Hold failed status %d", status );
