/*
 * $Id: OpXferTargetHoldOk.cxx,v 1.2 2007/03/02 04:19:28 lizhijie Exp $
 */
...
OpXferTargetHoldOk::OpXferTargetHoldOk()
{}
...
const char* const
OpXferTargetHoldOk::name() const
{
    return "OpXferTargetHoldOk";
}


///
const Sptr < State >
OpXferTargetHoldOk::process( const Sptr < SipProxyEvent > event )
{
    cpDebug(LOG_DEBUG, "OpXferTargetHoldOk operation" );
    ...
    if ( (int)(contact->getCSeqNum()) == cseq.getCSeqData().convertInt() )
    {
        if ( cseq.getMethod() == "INVITE" )
        {
		    ...
		    transferToUrl.dynamicCast( baseUrl );
		    assert( transferToUrl != 0 );
			...
			sipVia.setHost(theSystem.gethostAddress(&transferToUrl->getHost()));
