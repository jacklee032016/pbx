/*
 * $Id: as_dsp_utils.c,v 1.1.1.1 2006/11/29 09:50:50 lizhijie Exp $
*/
...
int as_dsp_get_audio_sample(as_dsp_engine *dsp, int fd )
{
	int len;
...
as_dsp_engine *as_dsp_init_dsp(int fd, int law )
{
#if 0
	float	f0,f1;
...
int spb,spb1;  /* these dont change, so they are okay as globals */
	...
	spb1 = 264;
#endif	
	dsp = (as_dsp_engine *)malloc(sizeof(as_dsp_engine) );
	if(!dsp)
	{
		printf("No critical memory %s", __FUNCTION__);
		exit(1);
	}
	memset(dsp, 0, sizeof(as_dsp_engine) );

	dsp->dtmf = as_dsp_init_dtmf_detect( law, DTMF_PARSE_LENGTH);
	assert(dsp->dtmf!=0);
...
void as_dsp_detsory_dsp(as_dsp_engine *dsp)
{
