/*
 * $Id: as_dsp_fsk_demodule.c,v 1.1.1.1 2006/11/29 09:50:50 lizhijie Exp $
*/
...
extern int as_dsp_get_audio_sample(as_dsp_engine *dsp, int fd );

int __get_audio_sample_from_config(as_dsp_engine *dsp, int fd)
{
//	int position;
	int res;
	int len = 0, left = 0;
...
static int as_fsk_get_audio_sample(as_dsp_engine *dsp, int fd, float *retval)
{
	as_fsk_decode_engine *engine = &(dsp->fsk_decoder);
...
static int as_fsk_demodulator(as_dsp_engine *dsp, int fd, float *retval)
{
	float x,xS,xM;
	as_fsk_decode_engine *engine = &(dsp->fsk_decoder);
...
static int get_bit_raw(as_dsp_engine *dsp , int fd)
{
...
int as_fsk_serie(as_dsp_engine *dsp, int fd)
{
	int a;
	float x1,x2;
	int i,j,n1,r;
