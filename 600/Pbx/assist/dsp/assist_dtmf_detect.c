/* 
 * $Id: assist_dtmf_detect.c,v 1.2 2007/03/16 20:40:29 lizhijie Exp $
...
#if __ARM_IXP__
			if (i >= 4 && (row_energy[best_row] + col_energy[best_col]) > 42.0*s->energy )
#else
			if (i >= 4 
...
//	int position;
	...
	if(length != dsp->length )
	{
		AS_MSG_LOG(AS_LOG_WARNING, "Data preparing to parsing is more longer(%d) than buf length in DSP engine(%d)\r\n", length, dsp->length);
		if(dsp->length > length)
			dsp->length = length;
	}
...
int assist_dsp_dtmf_detect_step(assist_dtmf_detector_t  *dsp, char *buf, int length)
{
	int i;
	int res;
	unsigned short line_samples[DTMF_DETECT_STEP_SIZE*2];

	assert(dsp!=0);
	assert(buf!=0);

	if(length > DTMF_DETECT_STEP_SIZE*2 )
	{
		length = DTMF_DETECT_STEP_SIZE*2;
	}
	
	for(i=0; i< length; i++)
	{
		line_samples[i] = FULLXLAW(buf[i], dsp->law);
	}	

	AS_MSG_DEBUG(AS_LOG_INFO,"\r\nlinesample length =%d", dsp->bufp);
	res = __as_dsp_dtmf_detect(&(dsp->state) , line_samples, length , 1);
...
assist_dtmf_detector_t *assist_dsp_dtmf_detect_init( as_law_type_t law, int sample_length)
{
	...
	if(!dsp)
	{
		AS_MSG_LOG(AS_LOG_EMERG, "No critical memory %s", __FUNCTION__);
		exit(1);
	}
