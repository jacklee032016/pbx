...
ixEthMiiPhyScan(BOOL phyPresent[], UINT32 maxPhyCount)
{
			      ...
			      &regval) == IX_ETH_ACC_SUCCESS)
	{
		...
		if ((ixEthMiiPhyId[i] == IX_ETH_MII_KS8995_PHY_ID)
		    ...
		    /*add wangwei 20051130*/
		    || (ixEthMiiPhyId[i] == IX_ETH_MII_LXT972_PHY_ID_bak)
		    || (ixEthMiiPhyId[i] == IX_ETH_MII_ADM6996_PHY_ID)
		    )
		{
		    /* supported phy */
		    phyPresent[i] = TRUE;
			/*add , wangwei , 2006.02.27*/
			if((ixEthMiiPhyId[i] == IX_ETH_MII_LXT972_PHY_ID)||(ixEthMiiPhyId[i] == IX_ETH_MII_LXT972_PHY_ID_bak))
				{
				ixEthAccMiiWriteRtn(i,0x14,0xd22);
				}
...
ixEthMiiPhyReset(UINT32 phyAddr)
{
	...
	if ((ixEthMiiPhyId[phyAddr] == IX_ETH_MII_LXT971_PHY_ID)	||
		...
		/*add wangwei 20051130*/
	   || (ixEthMiiPhyId[phyAddr] == IX_ETH_MII_LXT972_PHY_ID_bak)
		    || (ixEthMiiPhyId[phyAddr] == IX_ETH_MII_ADM6996_PHY_ID)
	    )
	{
           ...
           BOOL *autoneg)
{
    ...
    *autoneg = FALSE;
	...
	(ixEthMiiPhyId[phyAddr] != IX_ETH_MII_INVALID_PHY_ID))
	/*add wangwei 20051130*/
    	//if ((phyAddr < IXP425_ETH_ACC_MII_MAX_ADDR) &&(ixEthMiiPhyId[phyAddr] != IX_ETH_MII_INVALID_PHY_ID))
	if (phyAddr < IXP425_ETH_ACC_MII_MAX_ADDR)
    {
	if ((ixEthMiiPhyId[phyAddr] == IX_ETH_MII_LXT971_PHY_ID)	||
	    (ixEthMiiPhyId[phyAddr] == IX_ETH_MII_LXT972_PHY_ID)	||
