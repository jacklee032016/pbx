/* $Id: dchannel.c,v 1.1.1.1 2006/11/30 16:28:15 lizhijie Exp $
 *
...
int  mISDN_init_dch(dchannel_t *dch)
{
	if (!(dch->dlog = kmalloc(MAX_DLOG_SPACE, GFP_ATOMIC)))
	{
		printk(KERN_WARNING	"mISDN: No memory for dlog\n");
		return(-ENOMEM);
	}
	
	if (!(dch->tx_buf = kmalloc(MAX_DFRAME_LEN_L1, GFP_ATOMIC)))
	{
		printk(KERN_WARNING	"mISDN: No memory for dchannel tx_buf\n");
		kfree(dch->dlog);
		dch->dlog = NULL;
		return(-ENOMEM);
	}
...
int mISDN_free_dch(dchannel_t *dch)
{
#ifdef HAS_WORKQUEUE
	if (dch->work.pending)
