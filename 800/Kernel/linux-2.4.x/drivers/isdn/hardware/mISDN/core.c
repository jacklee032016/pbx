/* $Id: core.c,v 1.1.1.1 2006/11/30 16:28:15 lizhijie Exp $
 *
...
static char		*mISDN_core_revision = "$Revision: 1.1.1.1 $";
...
static int mISDNd(void *data)
{
	...
	unlock_kernel();
#endif
	printk( KERN_INFO "mISDNd: kernel daemon started\n");
...
static mISDNobject_t *find_object(int protocol)
{
	mISDNobject_t	*obj;
	int		err;
...
static int debugout(mISDNinstance_t *inst, logdata_t *log)
{
	char *p = tmpbuf;
...
static int get_hdevice(mISDNdevice_t **dev, int *typ)
{
	if (!dev)
		return(-EINVAL);
	if (!typ)
...
static int set_stack_req(mISDNstack_t *st, mISDN_pid_t *pid)
{
	struct sk_buff	*skb;
	mISDN_headext_t	*hhe;
	mISDN_pid_t	*npid;
