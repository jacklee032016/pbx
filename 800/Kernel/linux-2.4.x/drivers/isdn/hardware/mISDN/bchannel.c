/* $Id: bchannel.c,v 1.1.1.1 2006/11/30 16:28:15 lizhijie Exp $
 *
...
int mISDN_init_bch(bchannel_t *bch)
{
	int	devtyp = mISDN_RAW_DEVICE;

	if (!(bch->blog = kmalloc(MAX_BLOG_SPACE, GFP_ATOMIC)))
	{
		printk(KERN_WARNING	"mISDN: No memory for blog\n");
		return(-ENOMEM);
	}
	if (!(bch->rx_buf = kmalloc(MAX_DATA_MEM, GFP_ATOMIC)))
	{
		printk(KERN_WARNING	"mISDN: No memory for bchannel rx_buf\n");
		kfree(bch->blog);
		bch->blog = NULL;
		return (-ENOMEM);
	}
	...
	{
		printk(KERN_WARNING "mISDN: No memory for bchannel tx_buf\n");
		kfree(bch->blog);
		bch->blog = NULL;
		kfree(bch->rx_buf);
		bch->rx_buf = NULL;
	...
	if (!bch->dev)
	{
		...
		{
			printk(KERN_WARNING "mISDN: no raw device for bchannel\n");
		}
	}
	return(0);
}

int mISDN_free_bch(bchannel_t *bch)
{
#ifdef HAS_WORKQUEUE
	if (bch->work.pending)
	...
	{
		printk(KERN_WARNING "mISDN: del raw device error\n");
	} 
	else
		bch->dev = NULL;
	return(0);
