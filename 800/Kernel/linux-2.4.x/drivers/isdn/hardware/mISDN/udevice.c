/* $Id: udevice.c,v 1.1.1.1 2006/11/30 16:28:15 lizhijie Exp $
 *
...
static mISDNdevice_t *get_mISDNdevice4minor(int minor)
{
	mISDNdevice_t	*dev;
...
static int  mISDN_rdata(mISDNdevice_t *dev, struct sk_buff *skb)
{
	mISDN_head_t	*hp;
	u_long		flags;
...
static int error_answer(mISDNdevice_t *dev, struct sk_buff *skb, int err)
{
	mISDN_head_t	*hp;
...
static devicelayer_t *get_devlayer(mISDNdevice_t *dev, int addr)
{
	devicelayer_t *dl;
...
static int del_entity_req(mISDNdevice_t *dev, int entity)
{
...
static void dev_expire_timer(mISDNtimer_t *ht)
{
	struct sk_buff	*skb;
	mISDN_head_t	*hp;
...
static int wdata_frame(mISDNdevice_t *dev, struct sk_buff *skb)
{
	mISDN_head_t	*hp;
	mISDNif_t	*hif = NULL;
	devicelayer_t	*dl;
...
static int  mISDN_wdata_if(mISDNdevice_t *dev, struct sk_buff *skb)
{
...
static mISDNdevice_t *init_device(u_int minor)
{
	mISDNdevice_t	*dev;
	u_long		flags;
...
int free_device(mISDNdevice_t *dev)
{
...
static int mISDN_close(struct inode *ino, struct file *filep)
{
	...
	list_for_each_entry_safe(dev, nd, &mISDN_devicelist, list)
	{
		if (dev == filep->private_data)
		{
			...
			{
				test_and_clear_bit(FLG_mISDNPORT_OPEN,  &dev->rport.Flag);
			}
			if (filep->f_mode & FMODE_WRITE)
			{
				test_and_clear_bit(FLG_mISDNPORT_OPEN, &dev->wport.Flag);
			}
...
static __inline__ ssize_t do_mISDN_read(struct file *file, char *buf, size_t count, loff_t * off)
{
...
static ssize_t mISDN_read(struct file *file, char *buf, size_t count, loff_t * off)
{
	mISDNdevice_t	*dev = file->private_data;
	ssize_t		ret;
...
static loff_t  mISDN_llseek(struct file *file, loff_t offset, int orig)
{
	return -ESPIPE;
}

static __inline__ ssize_t  do_mISDN_write(struct file *file, const char *buf, size_t count, loff_t * off)
{
	mISDNdevice_t	*dev = file->private_data;
	size_t			len;
	u_long			flags;
...
static ssize_t mISDN_write(struct file *file, const char *buf, size_t count, loff_t * off)
{
	mISDNdevice_t	*dev = file->private_data;
	ssize_t		ret;
...
static unsigned int mISDN_poll(struct file *file, poll_table * wait)
{
	unsigned int	mask = POLLERR;
	mISDNdevice_t	*dev = file->private_data;
	mISDNport_t	*rport = (file->f_mode & FMODE_READ) ? &dev->rport : NULL;
	mISDNport_t	*wport = (file->f_mode & FMODE_WRITE) ? &dev->wport : NULL;
...
static int from_up_down(mISDNif_t *hif, struct sk_buff *skb)
{
...
static int set_if(devicelayer_t *dl, u_int prim, mISDNif_t *hif)
{
	int err = 0;
