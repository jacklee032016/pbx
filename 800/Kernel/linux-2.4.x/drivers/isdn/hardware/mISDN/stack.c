/* $Id: stack.c,v 1.1.1.1 2006/11/30 16:28:15 lizhijie Exp $
 *
...
mISDNstack_t *new_stack(mISDNstack_t *master, mISDNinstance_t *inst)
{
	mISDNstack_t *newst;
...
int do_for_all_layers(mISDNstack_t *st, u_int prim, void *arg)
{
	mISDNinstance_t *inst;
	mISDNlayer_t    *layer, *nl;
	int		cnt = 0;
...
int register_layer(mISDNstack_t *st, mISDNinstance_t *inst)
{
	...
	if (core_debug & DEBUG_CORE_FUNC)
		printk(KERN_DEBUG "%s:st(%p) inst(%p/%p) lmask(%x) id(%x)\n",
			__FUNCTION__, st, inst, inst->obj, inst->pid.layermask, inst->id);
	if (inst->id)
	{ /* allready registered */
		if (inst->st || !st)
		{
	...
	if (st)
	{
		if ((layer = getlayer4lay(st, inst->pid.layermask)))
		{
			if (layer->inst)
			{
				int_errtxt("stack %08x has layer %08x", st->id, layer->inst->id);
				return(-EBUSY);
			}
...
static int test_stack_protocol(mISDNstack_t *st, u_int l1prot, u_int l2prot, u_int l3prot)
{
...
int evaluate_stack_pids(mISDNstack_t *st, mISDN_pid_t *pid)
{
	int 		err;
	mISDN_pid_t	pidmask;
