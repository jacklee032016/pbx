 ...
 *
 * Revision 1.6  2006/04/19 12:31:48  lizhijie
 * no message
 *
 * Revision 1.5  2006/03/13 09:17:18  wangwei
 * no message
 *
 * Revision 1.4  2006/03/12 11:09:32  lizhijie
 * debug DTMF caller ID and tone play after ISR timer is modified
 *
 * Revision 1.3  2006/01/18 13:01:17  lizhijie
 * dd
 *
 * Revision 1.2  2006/01/17 03:19:42  lizhijie
 * a
 *
...
int as_fxs_ioctl(struct as_dev_chan *chan, unsigned int cmd, unsigned long data)
{
	...
	/*add , wangwei ,2006.03.13*/
		case AS_SWITCH_PHONE:			
			return as_realy_switch_phone(chan, data);
...
then the handler driver provided by span is not used 
*/
	...
	if (wc->modtype[chan->chanpos - 1] == MOD_TYPE_FXO) 
	{
		...
		switch(txsig) 
		{
			...
			case AS_TXSIG_OFFHOOK:
				wc->mod.fxo.wasringing[chan->chanpos - 1] = 0;	/* added for CAR detect, lizhijie, 2006.04.17 */
				wc->mod.fxo.offhook[chan->chanpos - 1] = 1;
				wc->mod.fxo.offhookTimer[chan->chanpos - 1] = AS_FXO_OFFHOOK_TIME;	/*added lizhijie,2006.01.16 */
...
//				as_fxs_setreg_nolock(wc, chan->chanpos - 1, 5, 0x9);
...
#if 1
				reg = as_fxs_getreg_nolock(wc, chan->chanpos - 1, 5);
				reg = reg & ~0x08;
				as_fxs_setreg_nolock(wc, chan->chanpos - 1, 5, reg);
...
#if	1//AS_WITH_JATE
				/* OFF-HOOK, battery is on */
				wc->mod.fxo.onhook[chan->chanpos - 1] = 0;
				wc->mod.fxo.battery[chan->chanpos - 1] = 1;
				wc->mod.fxo.battdebounce[chan->chanpos - 1] = BATT_DEBOUNCE;
				wc->mod.fxo.ohdebounce[chan->chanpos - 1] = OH_DEBOUNCE;
#endif
			...
			case AS_TXSIG_ONHOOK:
				...
				wc->mod.fxo.onhook[chan->chanpos - 1] = 1;
				wc->mod.fxo.offhook[chan->chanpos - 1] = 0;
				/* Don't accept a ring for another 1000 ms */
				wc->mod.fxo.ringdebounce[chan->chanpos - 1] = 1000*AS_TIME_UNIT;
#if	1//AS_WITH_JATE
				wc->mod.fxo.ohdebounce[chan->chanpos - 1] = OH_DEBOUNCE;
#endif
				break;
	...
	//			break;
					default:
						/* After ringoff on FXS device, Register 64 can not defined as 'AS_LINEFEED_FORWARD_ACTIVE'
						* otherwise, DTMF Caller ID can not be detected in phone, 
						* it must be AS_LINEFEED_FORWARD_ONHOOK, eg. transmit when in on-hook status
						*  in Li Zhijie, 2006.03.12
						*/
#if 0
						wc->mod.fxs.lasttxhook[chan->chanpos-1] = wc->mod.fxs.idletxhookstate[chan->chanpos-1];
	#if AS_SLIC_DEBUG
						printk("After RINGEROFF, Reg 64 of FXS device is '%d'\n", wc->mod.fxs.lasttxhook[chan->chanpos-1] );
	#endif						
#endif
//						wc->mod.fxs.lasttxhook[chan->chanpos-1] = AS_LINEFEED_FORWARD_ONHOOK;
						wc->mod.fxs.lasttxhook[chan->chanpos-1] = AS_LINEFEED_REVERSE_ON_HOOK;
						break;
				}
				break;
			case AS_TXSIG_START:
...
#else
//				as_fxs_setreg_nolock(wc, chan->chanpos - 1, 64, AS_LINEFEED_FORWARD_ACTIVE);
//				as_fxs_setreg_nolock(wc, chan->chanpos - 1, 64, AS_LINEFEED_REVERSE_ACTIVE);
//				udelay(100);
				wc->mod.fxs.lasttxhook[chan->chanpos-1] = AS_LINEFEED_RINGING;
#endif
				break;
...
#if AS_SLIC_DEBUG
		if (debug)
			printk("Setting FXS hook state to %d (%02x) on channel %d\n", txsig, reg, chan->channo );
#endif
