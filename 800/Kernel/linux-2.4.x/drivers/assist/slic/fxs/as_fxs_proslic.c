...
/* check some register in every ProSLIC card to determine whether this card is sane */
...
//	as_fxs_setreg(wc, card, 1, 0x20);
...
#if AS_WITH_JATE
	/* synthesis 600 ohm 2-wires impedance */
	as_fxs_setreg(wc, card, 10, 0x08 |0x00);
#else
	if (fxshonormode) 
	{
	...
	if (lowpower)/* on-hook line voltage */
