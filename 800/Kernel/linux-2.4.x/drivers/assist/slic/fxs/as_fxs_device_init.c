 ...
 * no message
 *
 * Revision 1.3  2006/02/16 08:32:51  wangwei
 * no message
 *
...
static int __as_fxs_channel_register(struct wcfxs *wc, as_channel_type_t type, int card_position)
{
	...
	{
			printk("Unable to register FXS/FXO channel %s with SPAN\n", chan->name );
			return -1;
	}

#if AS_SLIC_DEBUG
	printk("Registered FXS/FXO channel %s within SPAN\n", chan->name );
#endif
	wc->span = chan->span;
	...
	else 
	{
		...
		{/* line feed when startup */
			...
			if( wc->modtype[x] == MOD_TYPE_FXS ) 
				as_fxs_setreg( wc, x, 64, AS_LINEFEED_FORWARD_ACTIVE );
		}
