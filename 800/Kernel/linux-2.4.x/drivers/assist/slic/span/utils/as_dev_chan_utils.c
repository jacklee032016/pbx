 ...
 * AS800 Kernel
 *
 * Revision 1.9  2006/04/25 05:37:51  wangwei
 * no message
 *
 * Revision 1.8  2006/04/19 12:33:50  lizhijie
 * add CAR support for both FXS/FXO and debugs
 *
 * Revision 1.7  2006/04/19 12:33:08  lizhijie
 * no message
 *
 * Revision 1.6  2006/03/12 11:09:32  lizhijie
 * debug DTMF caller ID and tone play after ISR timer is modified
 *
 * Revision 1.5  2006/01/18 13:01:17  lizhijie
 * dd
 *
 ...
 * dd
 *
 * Revision 1.2  2006/01/14 11:05:35  lizhijie
 * dd
 *
...
/* wcfxs : this is the only case that should be happened */	
	...
	{
#if AS_SLIC_DEBUG
		printk("sethook on %d(position %d) channel with hooksig provided by low layer driver, timeout is %d\r\n", chan->channo,chan->chanpos, timeout );
#endif
		chan->txhooksig = txsig;
		chan->hooksig(chan, txsig);
		chan->otimer = timeout ;	/* all timers has been defined in AS_TIME_UNIT, except ringcandence and tone->samples */
		return;
	} 
...
int as_channel_hangup(struct as_dev_chan *chan)
{
	...
	if ( (chan->sig == AS_SIG_FXSLS) /* FXO device */
	...
	if (chan->span->flags & AS_FLAG_RBS) 
	{
		...
		else /* FXO device and FXS device when in on-hook state */
