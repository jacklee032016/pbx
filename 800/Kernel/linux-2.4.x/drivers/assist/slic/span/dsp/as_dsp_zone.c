...
static struct as_zone_tones_data *__as_zone_data_init(void )
{
	...
	{
		zone->ringcadence[x] = ringcadence.ringcadence[x]; //*AS_TIME_UNIT;	/* in 10 ms unit */
		zone->carcadence[x] = carcadence.ringcadence[x]; //*AS_TIME_UNIT;	/* in 10 ms unit */
	}
...
//		tone->tonesamples = tone_defs[x].samples;
		/* this value is write into register, so is must be real time, not associated with CHUNK_SIZE
		Please refer to Si3210 datasheet and as_gen_tones.c for more details, Lizhijie, 2006.03.12
		*/
		tone->tonesamples = tone_defs[x].samples ;
