 ...
 * AS800 Kernel
 *
 * Revision 1.5  2006/04/25 05:37:41  wangwei
 * no message
 *
...
/*case 3 */	
		...
		case AS_DIAL_OP_FOX_SEND_DP_10PPS:
		case AS_DIAL_OP_FOX_SEND_DP_20PPS:
			if(chan->dialing == 1)
			{
				rv = -ENODATA;
					break;
			...
			if(tdo.op == AS_DIAL_OP_FOX_SEND_DP_10PPS)
				chan->digitmode = AS_DIGIT_MODE_10PPS;
			else if(tdo.op == AS_DIAL_OP_FOX_SEND_DP_20PPS)
				chan->digitmode = AS_DIGIT_MODE_20PPS;
			else
			{
				rv = -ENODATA;
				break;
