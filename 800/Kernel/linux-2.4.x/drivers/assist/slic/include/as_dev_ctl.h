 ...
 * AS800 Kernel
 *
 * Revision 1.4  2006/04/25 05:36:46  wangwei
 * no message
 *
 * Revision 1.3  2006/04/19 12:33:50  lizhijie
 * add CAR support for both FXS/FXO and debugs
 *
 * Revision 1.2  2006/04/19 12:31:25  lizhijie
 * no message
 *
 ...
 *
 * Revision 1.3  2004/12/20 03:18:15  lizhijie
 ...
 * $Revision: 1.1.1.1 $
*/
#ifndef  __AS_DEV_CTL_H__
#define  __AS_DEV_CTL_H__
/* all ioctl command , parameters and data struct involved in ioctl system call */
...
#define AS_DIAL_OP_FOX_SEND_DP_10PPS  4
#define AS_DIAL_OP_FOX_SEND_DP_20PPS  5
...
#define	AS_RING		5
#define 	AS_RINGOFF  	6
#define	AS_RING_CAR	7	/* added CAR ring tx, lzj, 2006.04.17 */
...
#define AS_EVENT_CAR					18	/* CAR ring for NTT Caller ID,lizhijie,2006.04.17 */
...
#define AS_CTL_GET_BLOCKSIZE			_IOR (AS_CTL_CODE, 1, int)

/* * Set Transfer Block Size. */
#define AS_CTL_SET_BLOCKSIZE			_IOW (AS_CTL_CODE, 2, int)
 ...
 */
#define	AS_CTL_SYNC					_IOW (AS_CTL_CODE, 4, int)

/* * Get channel parameters */
#define AS_CTL_GET_PARAMS				_IOW (AS_CTL_CODE, 5, struct as_params)

#define AS_CTL_SET_PARAMS				_IOW (AS_CTL_CODE, 6, struct as_params)

/* * Set Hookswitch Status */
#define AS_CTL_HOOK						_IOW ( AS_CTL_CODE, 7, int)

/* * Get Signalling Event */
#define AS_CTL_GETEVENT				_IOR ( AS_CTL_CODE, 8, int)

/* * Set Maintenance Mode */
#define AS_CTL_MAINT					_IOW (AS_CTL_CODE, 11, struct zt_maintinfo)

/* * Get Channel audio gains */
#define AS_CTL_GETGAINS				_IOWR (AS_CTL_CODE, 16, struct as_gains)
...
#define	AS_CTL_SPANCONFIG				_IOW (AS_CTL_CODE, 18, struct as_lineconfig)

/* * Send a particular tone (see ZT_TONE_*) */
#define	AS_CTL_SENDTONE				_IOW (AS_CTL_CODE, 21, int)

/*
 * Set your region for tones (see ZT_TONE_ZONE_*)
 */
#define	AS_CTL_SETTONEZONE			_IOW (AS_CTL_CODE, 22, int)

/*
 * Retrieve current region for tones (see ZT_TONE_ZONE_*)
 */
#define	AS_CTL_GETTONEZONE			_IOR (AS_CTL_CODE, 23, int)

/*
 * Master unit only -- set default zone (see ZT_TONE_ZONE_*)
 */
#define	AS_CTL_DEFAULTZONE			_IOW (AS_CTL_CODE, 24, int)

/*
 * Load a tone zone from a as_tone_def_header, see below...
 */
#define AS_CTL_LOADZONE				_IOW (AS_CTL_CODE, 25, struct as_tone_def_header)

/*
 * Set buffer policy 
 */
#define AS_CTL_SET_BUFINFO				_IOW (AS_CTL_CODE, 27, struct as_bufferinfo)

/*
 * Get current buffer info
 */
#define AS_CTL_GET_BUFINFO				_IOR (AS_CTL_CODE, 28, struct as_bufferinfo)

/*
 * Get dialing parameters
 */
#define AS_CTL_GET_DIALPARAMS			_IOR (AS_CTL_CODE, 29, struct as_dialparams)

/*
 * Set dialing parameters
 */
#define AS_CTL_SET_DIALPARAMS			_IOW (AS_CTL_CODE, 30, struct as_dialparams)

/*
 * Append, replace, or cancel a dial string
 */
#define AS_CTL_SET_DTMF_STR			_IOW (AS_CTL_CODE, 31, struct as_dialoperation)

/*
 * Set a clear channel into audio mode
 */
#define AS_CTL_AUDIOMODE				_IOW (AS_CTL_CODE, 32, int)
 ...
 */
#define AS_CTL_ECHOCANCEL				_IOW (AS_CTL_CODE, 33, int)

/*
 * Return a channel's channel number (useful for the /dev/zap/pseudo type interfaces 
 */
#define AS_CTL_CHANNO					_IOR (AS_CTL_CODE, 34, int)
 ...
 */
#define AS_CTL_DIALING					_IOR (AS_CTL_CODE, 35, int)
 ...
 */
#define AS_CTL_SETLAW					_IOW (AS_CTL_CODE, 39, int)

/*
 * Temporarily set the channel to operate in linear mode when non-zero
 * or default law if 0
 */
#define AS_CTL_SETLINEAR				_IOW (AS_CTL_CODE, 40, int)


/*
 * Set the ring cadence for FXS interfaces
 */
#define AS_CTL_SETCADENCE				_IOW (AS_CTL_CODE, 42, struct as_ring_cadence)

/*
 * Display Channel Diagnostic Information on Console
 */
#define AS_CTL_CHAN_DIAG				_IOR (AS_CTL_CODE, 44, int) 


/*
 * Set timer expiration (in samples)
 */
#define AS_CTL_TIMERCONFIG				_IOW (AS_CTL_CODE, 47, int)


/*
 * Request echo training in some number of ms (with muting in the mean time)
 */
#define	AS_CTL_ECHOTRAIN				_IOW (AS_CTL_CODE, 50, int)

/*
 * Set on hook transfer for n number of ms -- implemnted by low level driver
 */
#define	AS_CTL_ONHOOKTRANSFER		_IOW (AS_CTL_CODE, 51, int)

/*
 * Queue Ping
 */
#define AS_CTL_TIMERPING 				_IOW (AS_CTL_CODE, 42, int) /* Should be 52, but works */

/*
 * Acknowledge ping
 */
#define AS_CTL_TIMERPONG 				_IOW (AS_CTL_CODE, 53, int)

/*
 * Set/get signalling freeze
 */
#define AS_CTL_SIGFREEZE 				_IOW (AS_CTL_CODE, 54, int)
#define AS_CTL_GETSIGFREEZE 			_IOR (AS_CTL_CODE, 55, int)
...
#define AS_CTL_SPAN_STARTUP			_IOW (AS_CTL_CODE, 99, int)
#define AS_CTL_SPAN_SHUTDOWN			_IOW (AS_CTL_CODE, 100, int)


#define AS_CTL_GET_DTMF_DETECT		_IOW (AS_CTL_CODE, 150, int )
#define AS_GET_CHANNEL_NUMBER		_IOW (AS_CTL_CODE, 151, int )
#define AS_GET_CHANNELS_STATES		_IOW (AS_CTL_CODE, 152, struct as_channel_state)
