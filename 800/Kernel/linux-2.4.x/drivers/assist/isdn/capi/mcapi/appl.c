/*
 * $Id: appl.c,v 1.1.1.1 2006/11/30 16:27:58 lizhijie Exp $
...
ApplicationSendMessage(Application_t *appl, struct sk_buff *skb)
{
	...
	switch (CAPICMD(CAPIMSG_COMMAND(skb->data), CAPIMSG_SUBCOMMAND(skb->data))) {
		...
		case CAPI_CONNECT_REQ:
			if (ControllerNewPlci(appl->contr, &plci, AS_ISDN_ID_ANY)) {
...
void applManufacturerReqAS_ISDN(Application_t *appl, _cmsg *cmsg, struct sk_buff *skb)
{
	AppPlci_t	*aplci;
	Ncci_t		*ncci;

	switch (cmsg->Class) {
		case AS_ISDN_MF_CLASS_HANDSET:
...
applManufacturerReq(Application_t *appl, struct sk_buff *skb)
{
	...
	capi_message2cmsg(cmsg, skb->data);
	switch (cmsg->ManuID) {
		case AS_ISDN_MANUFACTURER_ID:
			applManufacturerReqAS_ISDN(appl, cmsg, skb);
			break;
