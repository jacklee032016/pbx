/*
 * $Id: listen.c,v 1.1.1.1 2006/11/30 16:27:58 lizhijie Exp $
 */
...
listen_req_l_x(struct FsmInst *fi, int event, void *arg, int state)
{
	...
	if (as_isdn_fsm_event(&app->listen_m, EV_LISTEN_CONF, cmsg))
		cmsg_free(cmsg);
}
...
listen_conf_l_x_1(struct FsmInst *fi, int event, void *arg, int state)
{
	...
	if (cmsg->Info != CAPI_NOERROR) {
		...
		if (app->CIPmask == 0) {
			...
			fsm_change_state(fi, ST_LISTEN_L_0);
		} else {
			test_and_set_bit(APPL_STATE_LISTEN, &app->state);
			fsm_change_state(fi, ST_LISTEN_L_1);
		}
	}
...
listenSendMessage(Application_t *app, struct sk_buff *skb)
{
	...
	switch (CMSGCMD(cmsg)) {
		case CAPI_LISTEN_REQ:
			if (as_isdn_fsm_event(&app->listen_m, EV_LISTEN_REQ, cmsg))
				cmsg_free(cmsg);
			break;
		default:
			int_error();
...
void init_listen(void)
{
	...
	as_isdn_fsm_new(&listen_fsm, fn_listen_list, FN_LISTEN_COUNT);
}

void free_listen(void)
{
	as_isdn_fsm_free(&listen_fsm);
}
