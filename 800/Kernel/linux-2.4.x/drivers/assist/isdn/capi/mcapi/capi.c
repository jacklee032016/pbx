/*
 * $Id: capi.c,v 1.1.1.1 2006/11/30 16:27:58 lizhijie Exp $
 */
...
static char *capi_revision = "$Revision: 1.1.1.1 $";

static int debug = 0;
static as_isdn_object_t capi_obj;

static char MName[] = "AS_ISDN Capi 2.0";
...
kmem_cache_t	*AS_ISDN_cmsg_cp;
kmem_cache_t	*AS_ISDN_AppPlci_cp;
kmem_cache_t	*AS_ISDN_ncci_cp;
kmem_cache_t	*AS_ISDN_sspc_cp;

#ifdef AS_ISDN_KMEM_DEBUG
static struct list_head AS_ISDN_kmem_garbage = LIST_HEAD_INIT(AS_ISDN_kmem_garbage);

_cmsg *
_kd_cmsg_alloc(char *fn, int line)
{
	_kd_cmsg_t	*ki = kmem_cache_alloc(AS_ISDN_cmsg_cp, GFP_ATOMIC);
	...
	ki->kdi.line = line;
	ki->kdi.file = fn;
	list_add_tail(&ki->kdi.head, &AS_ISDN_kmem_garbage);
	return(&ki->cm);
}
...
cmsg_free(_cmsg *cm)
{
	...
	kdi = KDB_GET_KDI(cm);
	list_del(&kdi->head);
	kmem_cache_free(AS_ISDN_cmsg_cp, kdi);
}

AppPlci_t *
_kd_AppPlci_alloc(char *fn, int line)
{
	_kd_AppPlci_t	*ki = kmem_cache_alloc(AS_ISDN_AppPlci_cp, GFP_ATOMIC);
...
AppPlci_free(AppPlci_t *ap)
{
	...
	kdi = KDB_GET_KDI(ap);
	list_del(&kdi->head);
	kmem_cache_free(AS_ISDN_AppPlci_cp, kdi);
}

Ncci_t *
_kd_ncci_alloc(char *fn, int line)
{
	_kd_Ncci_t	*ki = kmem_cache_alloc(AS_ISDN_ncci_cp, GFP_ATOMIC);
...
ncci_free(Ncci_t *ni)
{
	...
	kdi = KDB_GET_KDI(ni);
	list_del(&kdi->head);
	kmem_cache_free(AS_ISDN_ncci_cp, kdi);
}

SSProcess_t *
_kd_SSProcess_alloc(char *fn, int line)
{
	_kd_SSProcess_t	*ki = kmem_cache_alloc(AS_ISDN_sspc_cp, GFP_ATOMIC);
...
SSProcess_free(SSProcess_t *sp)
{
	...
	kdi = KDB_GET_KDI(sp);
	list_del(&kdi->head);
	kmem_cache_free(AS_ISDN_sspc_cp, kdi);
}
...
free_garbage(void)
{
	...
	list_for_each_safe(item, next, &AS_ISDN_kmem_garbage) {
		kda = (_kd_all_t *)item;
		...
		switch(kda->kdi.typ) {
			case KM_DBG_TYP_CM:
				...
				kmem_cache_free(AS_ISDN_cmsg_cp, item);
				break;
			case KM_DBG_TYP_AP:
				printk(KERN_DEBUG "AppPlci: PLCI(%x) m.state(%x) appl(%p)\n",
					...
					kda->a.ap.appl);
				kmem_cache_free(AS_ISDN_AppPlci_cp, item);
				break;
			case KM_DBG_TYP_NI:
				printk(KERN_DEBUG "Ncci: NCCI(%x) state(%lx) m.state(%x) aplci(%p)\n",
					...
					kda->a.ni.AppPlci);
				kmem_cache_free(AS_ISDN_ncci_cp, item);
				break;
			case KM_DBG_TYP_SP:
				printk(KERN_DEBUG "SSPc: addr(%x) id(%x) apid(%x) func(%x)\n",
					...
					kda->a.sp.ApplId,
					kda->a.sp.Function);
				kmem_cache_free(AS_ISDN_sspc_cp, item);
				break;
			default:
...
#endif
	if (AS_ISDN_cmsg_cp) {
		kmem_cache_destroy(AS_ISDN_cmsg_cp);
		AS_ISDN_cmsg_cp = NULL;
	}
	if (AS_ISDN_AppPlci_cp) {
		kmem_cache_destroy(AS_ISDN_AppPlci_cp);
		AS_ISDN_AppPlci_cp = NULL;
	}
	if (AS_ISDN_ncci_cp) {
		kmem_cache_destroy(AS_ISDN_ncci_cp);
		AS_ISDN_ncci_cp = NULL;
	}
	if (AS_ISDN_sspc_cp) {
		kmem_cache_destroy(AS_ISDN_sspc_cp);
		AS_ISDN_sspc_cp = NULL;
	}
}
...
static int CapiNew(void)
{
	...
	AS_ISDN_ncci_cp = NULL;
	AS_ISDN_sspc_cp = NULL;
	AS_ISDN_cmsg_cp = kmem_cache_create("AS_ISDN_cmesg",
#ifdef AS_ISDN_KMEM_DEBUG
				sizeof(_kd_cmsg_t),
#else
				sizeof(_cmsg),
	...
	AS_ISDN_AppPlci_cp = kmem_cache_create("AS_ISDN_AppPlci",
#ifdef AS_ISDN_KMEM_DEBUG
	...
	AS_ISDN_ncci_cp = kmem_cache_create("AS_ISDN_Ncci",
#ifdef AS_ISDN_KMEM_DEBUG
				sizeof(_kd_Ncci_t),
#else
				sizeof(Ncci_t),
	...
	AS_ISDN_sspc_cp = kmem_cache_create("AS_ISDN_SSProc",
#ifdef AS_ISDN_KMEM_DEBUG
...
#ifdef OLDCAPI_DRIVER_INTERFACE
	cdrv_if = attach_capi_driver(&AS_ISDN_driver);
	if (!cdrv_if) {
		CapiCachesFree();
		printk(KERN_ERR "AS_ISDN: failed to attach capi_driver\n");
		return -EIO;
	}
#endif
	init_listen();
...
capi20_manager(void *data, u_int prim, void *arg) {
	...
	switch(prim) {
		...
		if (&ctrl->inst == inst)
			return(AS_ISDN_SetIF(inst, arg, prim, NULL, ControllerL3L4, ctrl));
		else
			return(AppPlciAS_ISDN_SetIF(inst->data, prim, arg));
	    case MGR_DISCONNECT | REQUEST:
	    case MGR_DISCONNECT | INDICATION:
		return(AS_ISDN_DisConnectIF(inst, arg));
	    case MGR_RELEASE | INDICATION:
		if (CAPI_DBG_INFO & debug)
			printk(KERN_ERR  "release_capi20 id %x\n", ctrl->inst.st->id);
...
int Capi20Init(void)
{
	...
	if ((err = CapiNew()))
		return(err);
	if ((err = as_isdn_register(&capi_obj))) {
		printk(KERN_ERR "Can't register %s error(%d)\n", MName, err);
#ifdef OLDCAPI_DRIVER_INTERFACE
		detach_capi_driver(&AS_ISDN_driver);
#endif
		CapiCachesFree();
		free_listen();
		free_AppPlci();
...
static void Capi20cleanup(void)
{
	...
	if ((err = as_isdn_unregister(&capi_obj))) {
		printk(KERN_ERR "Can't unregister CAPI20 error(%d)\n", err);
	}
	if (!list_empty(&capi_obj.ilist)) {
		printk(KERN_WARNING "AS_ISDN controller list not empty\n");
