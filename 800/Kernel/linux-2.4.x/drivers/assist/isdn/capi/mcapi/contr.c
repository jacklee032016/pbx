/*
 * $Id: contr.c,v 1.1.1.1 2006/11/30 16:27:58 lizhijie Exp $
 */
...
{
	as_isdn_instance_t		*inst = &contr->inst;
	...
	if (contr->entity != AS_ISDN_ENTITY_NONE)
...
ControllerRun(Controller_t *contr)
{
	...
	if (contr->inst.st && contr->inst.st->mgr)
		sprintf(contr->ctrl->manu, "AS_ISDN CAPI controller %s", contr->inst.st->mgr->name);
	else
		sprintf(contr->ctrl->manu, "AS_ISDN CAPI");
	...
	if (contr->nr_bc) {
		...
		memset(&pidmask, 0, sizeof(AS_ISDN_pid_t));
...
read_proc(char *page, char **start, off_t off, int count, int *eof, struct capi_ctr *ctrl)
{
       ...
       len += sprintf(page+len, "AS_ISDN_read_proc\n");
       if (off+count >= len)
          *eof = 1;
       if (len < off)
           return 0;
...
struct capi_driver AS_ISDN_driver = {
	"AS_ISDN",
	"0.01",
	LoadFirmware,
	ResetController,
...
ControllerNewPlci(Controller_t *contr, Plci_t  **plci_p, u_int l3id)
{
	...
	*plci_p = plci;
	if (l3id == AS_ISDN_ID_ANY) {
		if (contr->entity == AS_ISDN_ENTITY_NONE) {
			printk(KERN_ERR "AS_ISDN %s: no ENTITY id\n",
				__FUNCTION__);
		...
		if (plci) {
			printk(KERN_WARNING "AS_ISDN %s: PLCI(%x) allready has l3id(%x)\n",
...
int
ControllerL3L4(as_isdn_if_t *hif, struct sk_buff *skb)
{
	Controller_t	*contr;
	Plci_t		*plci;
	int		ret = -EINVAL;
	...
	if (hh->prim == (CC_NEW_CR | INDICATION)) {
		...
		if(!ret)
			dev_kfree_skb(skb);
	} else if (hh->dinfo == AS_ISDN_ID_DUMMY) {
...
int
ControllerConstr(Controller_t **contr_p, as_isdn_stack_t *st, AS_ISDN_pid_t *pid, as_isdn_object_t *ocapi)
{
	...
	contr->next_id = 1;
	memcpy(&contr->inst.pid, pid, sizeof(AS_ISDN_pid_t));
#ifndef OLDCAPI_DRIVER_INTERFACE
	...
	AS_ISDN_init_instance(&contr->inst, ocapi, contr);
	if (!AS_ISDN_SetHandledPID(ocapi, &contr->inst.pid)) {
		int_error();
		ControllerDestr(contr);
		return(-ENOPROTOOPT);
	}
	list_for_each_entry(cst, head, list) {
		...
		plink->st = cst;
		plink->inst.st = cst;
		AS_ISDN_init_instance(&plink->inst, ocapi, plink);
	...
	contr->entity = AS_ISDN_ENTITY_NONE;
...
#ifdef OLDCAPI_DRIVER_INTERFACE
	{
		...
		sprintf(tmp, "AS_ISDN%d", st->id);
		contr->ctrl = cdrv_if->attach_ctr(&AS_ISDN_driver, tmp, contr);
		if (!contr->ctrl)
			retval = -ENODEV;
	}
#else
	contr->ctrl->owner = THIS_MODULE;
	sprintf(contr->ctrl->name, "AS_ISDN%d", st->id);
	contr->ctrl->driver_name = "AS_ISDN";
