/*
 * $Id: supp_serv.c,v 1.1.1.1 2006/11/30 16:27:58 lizhijie Exp $
 */
...
// FIXME	err = ControllerL4L3(contr, prim, contr->addr | DUMMY_CR_FLAG, skb);
	err = ControllerL4L3(spc->contr, prim, AS_ISDN_ID_DUMMY, skb);
	if (err)
		dev_kfree_skb(skb);
	return(err);
}
...
GetSupportedServices(FacReqParm_t *facReqParm, FacConfParm_t *facConfParm)
{
	...
	facConfParm->u.GetSupportedServices.SupportedServices = AS_ISDNSupportedServices;
	return CapiSuccess;
}
static int
FacListen(Application_t *appl, FacReqParm_t *facReqParm, FacConfParm_t *facConfParm)
{
	if (facReqParm->u.Listen.NotificationMask &~ AS_ISDNSupportedServices) {
...
{
	SSProcess_t	*sspc;
	struct sk_buff	*skb = AS_ISDN_alloc_l3msg(260, MT_FACILITY);
	__u8		*p;
	...
	AS_ISDN_AddIE(skb, IE_FACILITY, sspc->buf);
...
SupplementaryFacilityReq(Application_t *appl, _cmsg *cmsg)
{
	...
	switch (facReqParm.Function) {
		...
		case 0x0004: // Suspend
			...
			break;
		case 0x0005: // Resume
			ret = ControllerNewPlci(appl->contr, &plci, AS_ISDN_ID_ANY);
			if (ret) {
