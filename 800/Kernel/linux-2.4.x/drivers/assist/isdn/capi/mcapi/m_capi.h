/* $Id: m_capi.h,v 1.1.1.1 2006/11/30 16:27:58 lizhijie Exp $
 *
 * Rewritten CAPI Layer (Layer4 in AS_ISDN)
 * 
...
#ifndef __AS_ISDN_CAPI_H__
#define __AS_ISDN_CAPI_H__
...
extern struct capi_driver AS_ISDN_driver;
#endif
...
int		ControllerConstr(Controller_t **, as_isdn_stack_t *, AS_ISDN_pid_t *, as_isdn_object_t *);
...
int		ControllerL3L4(as_isdn_if_t *, struct sk_buff *);
...
int	AppPlciAS_ISDN_SetIF(AppPlci_t *, u_int, void *);
...
int	ncci_l3l4(Ncci_t *, as_isdn_head_t *, struct sk_buff *);
void	ncciGetCmsg(Ncci_t *, _cmsg *);
int	ncci_l3l4_direct(Ncci_t *, as_isdn_head_t *, struct sk_buff *);
...
#define AS_ISDNSupportedServices		(SuppServiceCF | SuppServiceTP)
...
// ---------------------------------------------------------------------------
// AS_ISDN kmem cache managment functions
// ---------------------------------------------------------------------------

/* kmem caches */
extern kmem_cache_t	*AS_ISDN_cmsg_cp;
extern kmem_cache_t	*AS_ISDN_AppPlci_cp;
extern kmem_cache_t	*AS_ISDN_ncci_cp;
extern kmem_cache_t	*AS_ISDN_sspc_cp;
...
#else /* ! AS_ISDN_KMEM_DEBUG */

static __inline__ _cmsg *cmsg_alloc(void)
{
	return(kmem_cache_alloc(AS_ISDN_cmsg_cp, GFP_ATOMIC));
}

static __inline__ void cmsg_free(_cmsg *cm)
{
	kmem_cache_free(AS_ISDN_cmsg_cp, cm);
}

static __inline__ AppPlci_t *AppPlci_alloc(void)
{
	return(kmem_cache_alloc(AS_ISDN_AppPlci_cp, GFP_ATOMIC));
}

static __inline__ void AppPlci_free(AppPlci_t *ap)
{
	kmem_cache_free(AS_ISDN_AppPlci_cp, ap);
}

static __inline__ Ncci_t *ncci_alloc(void)
{
	return(kmem_cache_alloc(AS_ISDN_ncci_cp, GFP_ATOMIC));
}

static __inline__ void ncci_free(Ncci_t *ni)
{
	kmem_cache_free(AS_ISDN_ncci_cp, ni);
}

static __inline__ SSProcess_t *SSProcess_alloc(void)
{
	return(kmem_cache_alloc(AS_ISDN_sspc_cp, GFP_ATOMIC));
}

static __inline__ void SSProcess_free(SSProcess_t *sp)
{
	kmem_cache_free(AS_ISDN_sspc_cp, sp);
}

#endif /* AS_ISDN_KMEM_DEBUG */
