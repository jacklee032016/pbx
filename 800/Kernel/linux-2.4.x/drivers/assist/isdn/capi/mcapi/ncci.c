/*
 * $Id: ncci.c,v 1.1.1.1 2006/11/30 16:27:58 lizhijie Exp $
 */
...
log_skbdata(struct sk_buff *skb)
{
	...
	AS_ISDN_QuickHex(t, skb->data, skb->len);
	printk(KERN_ERR  "%s\n", logbuf);
}
...
ncci_connect_b3_req(struct FsmInst *fi, int event, void *arg)
{
	...
	fsm_change_state(fi, ST_NCCI_N_0_1);
	...
	ncci_debug(fi, "ncci_connect_b3_req NCCI %x cmsg->Info(%x)",
		ncci->addr, cmsg->Info);
	if (as_isdn_fsm_event(fi, EV_NC_CONNECT_B3_CONF, cmsg))
		cmsg_free(cmsg);
}
...
{
	// from DL_ESTABLISH
	fsm_change_state(fi, ST_NCCI_N_1);
...
ncci_connect_b3_conf(struct FsmInst *fi, int event, void *arg)
{
	...
	if (cmsg->Info == 0) {
		...
		fsm_change_state(fi, ST_NCCI_N_0);
		Send2Application(fi->userdata, cmsg);
...
ncci_disconnect_b3_req(struct FsmInst *fi, int event, void *arg)
{
	...
	if (ncci->appl) { //FIXME
		...
		capi_cmsg_answer(cmsg);
		cmsg->Info = Info;
		if (as_isdn_fsm_event(fi, EV_NC_DISCONNECT_B3_CONF, cmsg))
			cmsg_free(cmsg);
	} else {
		cmsg_free(cmsg);
		fsm_change_state(fi, ST_NCCI_N_4);
...
ncci_disconnect_b3_ind(struct FsmInst *fi, int event, void *arg)
{
	...
	fsm_change_state(fi, ST_NCCI_N_5);
	if (ncci->appl) { // FIXME
...
ncci_manufacturer_req(struct FsmInst *fi, int event, void *arg)
{
	...
	capi_cmsg_answer(cmsg);
	if (cmsg->Class == AS_ISDN_MF_CLASS_HANDSET) { // Handset
		switch(cmsg->Function) {
			case AS_ISDN_MF_HANDSET_ENABLE:
				err = ncciL4L3(ncci, PH_CONTROL | REQUEST, HW_POTS_ON, 0, NULL, NULL);
				if (err)
					mcp.Info = CapiFacilityNotSupported;
				break;
			case AS_ISDN_MF_HANDSET_DISABLE:
				err = ncciL4L3(ncci, PH_CONTROL | REQUEST, HW_POTS_OFF, 0, NULL, NULL);
				if (err)
					mcp.Info = CapiFacilityNotSupported;
				break;
			case AS_ISDN_MF_HANDSET_SETMICVOLUME:
			case AS_ISDN_MF_HANDSET_SETSPKVOLUME:
				if (!mrp || mrp->len != 2) {
					mcp.Info = CapiIllMessageParmCoding;
					break;
				}
				op = (cmsg->Function == AS_ISDN_MF_HANDSET_SETSPKVOLUME) ?
				...
				break;
			/* not handled yet */
			case AS_ISDN_MF_HANDSET_GETMICVOLUME:
			case AS_ISDN_MF_HANDSET_GETSPKVOLUME:
			default:
				mcp.Info = CapiFacilityNotSupported;
				break;
		}
...
ncci_connect_b3_active_ind(struct FsmInst *fi, int event, void *arg)
{
	...
	fsm_change_state(fi, ST_NCCI_N_ACT);
...
ncci_n0_dl_establish_ind_conf(struct FsmInst *fi, int event, void *arg)
{
	...
	if (as_isdn_fsm_event(&ncci->ncci_m, EV_NC_CONNECT_B3_IND, cmsg))
		cmsg_free(cmsg);
}
...
ncci_dl_establish_conf(struct FsmInst *fi, int event, void *arg)
{
	...
	CMSG_ALLOC(cmsg);
	ncciCmsgHeader(ncci, cmsg, CAPI_CONNECT_B3_ACTIVE, CAPI_IND);
	if (as_isdn_fsm_event(&ncci->ncci_m, EV_NC_CONNECT_B3_ACTIVE_IND, cmsg))
		cmsg_free(cmsg);
}
...
ncci_dl_release_ind_conf(struct FsmInst *fi, int event, void *arg)
{
	...
	CMSG_ALLOC(cmsg);
	ncciCmsgHeader(ncci, cmsg, CAPI_DISCONNECT_B3, CAPI_IND);
	if (as_isdn_fsm_event(&ncci->ncci_m, EV_NC_DISCONNECT_B3_IND, cmsg))
		cmsg_free(cmsg);
}
...
{
	fsm_change_state(fi, ST_NCCI_N_3);
	SKB2Application(fi->userdata, arg);
}
...
ncciD_disconnect_b3_conf(struct FsmInst *fi, int event, void *arg)
{
	...
	if (test_bit(NCCI_STATE_APPLRELEASED, &ncci->state))
		return;
	if (info != 0)
		fsm_change_state(fi, ncci->savedstate);
...
#if AS_ISDN_DEBUG_FSM	
	if (ncci->ncci_m.debug)
		printk(KERN_ERR  "%s: ncci(%p) NCCI(%x) debug (%x/%x)\n",
...
ncciApplRelease(Ncci_t *ncci)
{
	...
	as_isdn_fsm_event(&ncci->ncci_m, EV_AP_RELEASE, NULL);
}

void
ncciDelAppPlci(Ncci_t *ncci)
{
	printk(KERN_ERR  "%s: ncci(%p) NCCI(%x)\n",
...
ncciReleaseLink(Ncci_t *ncci)
{
	...
	as_isdn_fsm_event(&ncci->ncci_m, EV_NC_LINKDOWN, NULL);
}
...
ncciDataInd(Ncci_t *ncci, int pr, struct sk_buff *skb)
{
	...
	if (i == CAPI_MAXDATAWINDOW) {
		...
		printk(KERN_ERR  "%s: frame %d dropped\n", __FUNCTION__, skb->len);
		dev_kfree_skb(skb);
		return;
	}
...
ncciDataReq(Ncci_t *ncci, struct sk_buff *skb)
{
	...
	AS_ISDN_HEAD_DINFO(skb) = ControllerNextId(ncci->contr);
	ncci->xmit_skb_handles[i].PktId = AS_ISDN_HEAD_DINFO(skb);
	...
	err = ncciL4L3(ncci, DL_DATA | REQUEST, AS_ISDN_HEAD_DINFO(skb), 0, NULL, skb);
	if (!err)
		return(CAPI_NOERROR);
	...
	if (i == -1) {
		for (i = 0; i < ncci->window; i++) {
			if (ncci->xmit_skb_handles[i].PktId == AS_ISDN_HEAD_DINFO(skb))
				break;
		}
		if (i == ncci->window)
			int_error();
...
ncciDataConf(Ncci_t *ncci, int pr, struct sk_buff *skb)
{
	...
	if (test_bit(NCCI_STATE_FCTRL, &ncci->state)) {
		if (skb_queue_len(&ncci->squeue)) {
			skb = skb_dequeue(&ncci->squeue);
			if (ncciL4L3(ncci, DL_DATA | REQUEST, AS_ISDN_HEAD_DINFO(skb),
				0, NULL, skb)) {
				int_error();
				dev_kfree_skb(skb);
			}
...
ncci_l4l3_direct(Ncci_t *ncci, struct sk_buff *skb) {
	...
	switch (hh->prim) {
		...
		case CAPI_CONNECT_B3_REQ:
			ret = as_isdn_fsm_event(&ncci->ncci_m, EV_AP_CONNECT_B3_REQ, skb);
			break;
		case CAPI_CONNECT_B3_RESP:
			ret = as_isdn_fsm_event(&ncci->ncci_m, EV_AP_CONNECT_B3_RESP, skb);
			break;
		case CAPI_CONNECT_B3_ACTIVE_RESP:
			ret = as_isdn_fsm_event(&ncci->ncci_m, EV_AP_CONNECT_B3_ACTIVE_RESP, skb);
			break;
		case CAPI_DISCONNECT_B3_REQ:
			ret = as_isdn_fsm_event(&ncci->ncci_m, EV_AP_DISCONNECT_B3_REQ, skb);
			break;
		case CAPI_DISCONNECT_B3_RESP:
			ret = as_isdn_fsm_event(&ncci->ncci_m, EV_AP_DISCONNECT_B3_RESP, skb);
			break;
		default:
			int_error();
			ret = -1;
...
ncciGetCmsg(Ncci_t *ncci, _cmsg *cmsg)
{
	...
	switch (CMSGCMD(cmsg)) {
		case CAPI_CONNECT_B3_REQ:
			retval = as_isdn_fsm_event(&ncci->ncci_m, EV_AP_CONNECT_B3_REQ, cmsg);
			break;
		case CAPI_CONNECT_B3_RESP:
			retval = as_isdn_fsm_event(&ncci->ncci_m, EV_AP_CONNECT_B3_RESP, cmsg);
			break;
		case CAPI_CONNECT_B3_ACTIVE_RESP:
			retval = as_isdn_fsm_event(&ncci->ncci_m, EV_AP_CONNECT_B3_ACTIVE_RESP, cmsg);
			break;
		case CAPI_DISCONNECT_B3_REQ:
			retval = as_isdn_fsm_event(&ncci->ncci_m, EV_AP_DISCONNECT_B3_REQ, cmsg);
			break;
		case CAPI_DISCONNECT_B3_RESP:
			retval = as_isdn_fsm_event(&ncci->ncci_m, EV_AP_DISCONNECT_B3_RESP, cmsg);
			break;
		case CAPI_FACILITY_REQ:
			retval = as_isdn_fsm_event(&ncci->ncci_m, EV_AP_FACILITY_REQ, cmsg);
			break;
		case CAPI_MANUFACTURER_REQ:
			retval = as_isdn_fsm_event(&ncci->ncci_m, EV_AP_MANUFACTURER_REQ, cmsg);
			break;
		default:
			int_error();
			retval = -1;
...
int
ncci_l3l4_direct(Ncci_t *ncci, as_isdn_head_t *hh, struct sk_buff *skb)
{
	__u16		msgnr;
	int		event;
	...
	if (as_isdn_fsm_event(&ncci->ncci_m, event, skb))
		dev_kfree_skb(skb);
	return(0);
}

int
ncci_l3l4(Ncci_t *ncci, as_isdn_head_t *hh, struct sk_buff *skb)
{
	capidebug(CAPI_DBG_NCCI_L3, "%s: NCCI %x prim(%x) dinfo (%x) skb(%p) s(%x)",
		__FUNCTION__, ncci->addr, hh->prim, hh->dinfo, skb, ncci->state);
	switch (hh->prim) {
		...
		case DL_ESTABLISH | INDICATION:
			as_isdn_fsm_event(&ncci->ncci_m, EV_DL_ESTABLISH_IND, skb);
			break;
		case DL_ESTABLISH | CONFIRM:
			as_isdn_fsm_event(&ncci->ncci_m, EV_DL_ESTABLISH_CONF, skb);
			break;
		case DL_RELEASE | INDICATION:
			as_isdn_fsm_event(&ncci->ncci_m, EV_DL_RELEASE_IND, skb);
			break;
		case DL_RELEASE | CONFIRM:
			as_isdn_fsm_event(&ncci->ncci_m, EV_DL_RELEASE_CONF, skb);
			break;
...
init_ncci(void)
{
	...
	as_isdn_fsm_new(&ncci_fsm, fn_ncci_list, FN_NCCI_COUNT);
	...
	as_isdn_fsm_new(&ncciD_fsm, fn_ncciD_list, FN_NCCID_COUNT);
}

void
free_ncci(void)
{
	as_isdn_fsm_free(&ncci_fsm);
	as_isdn_fsm_free(&ncciD_fsm);
}
