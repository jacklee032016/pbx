/*
 * $Id: app_plci.c,v 1.1.1.1 2006/11/30 16:27:58 lizhijie Exp $
 */
...
__u16 CIPValue2setup(__u16 CIPValue, struct sk_buff *skb)
{
	switch (CIPValue) {
		case 16:
			AS_ISDN_AddvarIE(skb, BEARER_31AUDIO_64K_ALAW);
			AS_ISDN_AddvarIE(skb, HLC_TELEPHONY);
			break;
		case 17:
			AS_ISDN_AddvarIE(skb, BEARER_31AUDIO_64K_ALAW);
			AS_ISDN_AddvarIE(skb, HLC_FACSIMILE);
			break;
		case 1:
			AS_ISDN_AddvarIE(skb, BEARER_SPEECH_64K_ALAW);
			break;
		case 2:
			AS_ISDN_AddvarIE(skb, BEARER_UNRES_DIGITAL_64K);
			break;
		case 3:
			AS_ISDN_AddvarIE(skb, BEARER_RES_DIGITAL_64K);
			break;
		case 4:
			AS_ISDN_AddvarIE(skb, BEARER_31AUDIO_64K_ALAW);
			break;
...
__u16 cmsg2setup_req(_cmsg *cmsg, struct sk_buff *skb)
{
	if (CIPValue2setup(cmsg->CIPValue, skb))
		goto err;
	AS_ISDN_AddIE(skb, IE_CALLING_PN, cmsg->CallingPartyNumber);
	AS_ISDN_AddIE(skb, IE_CALLING_SUB, cmsg->CallingPartySubaddress);
	AS_ISDN_AddIE(skb, IE_CALLED_PN, cmsg->CalledPartyNumber);
	AS_ISDN_AddIE(skb, IE_CALLED_SUB, cmsg->CalledPartySubaddress);
	AS_ISDN_AddIE(skb, IE_BEARER, cmsg->BC);
	AS_ISDN_AddIE(skb, IE_LLC, cmsg->LLC);
	AS_ISDN_AddIE(skb, IE_HLC, cmsg->HLC);
	return 0;
 err:
	return CapiIllMessageParmCoding;
}
...
{
	AS_ISDN_AddIE(skb, IE_KEYPAD, cmsg->Keypadfacility);
	AS_ISDN_AddIE(skb, IE_CALLED_PN, cmsg->CalledPartyNumber);
	return 0;
}
...
{
	AS_ISDN_AddIE(skb, IE_USER_USER, cmsg->Useruserdata);
	return 0;
}
...
plci_connect_req(struct FsmInst *fi, int event, void *arg)
{
	...
	fsm_change_state(fi, ST_PLCI_P_0_1);
	test_and_set_bit(PLCI_STATE_OUTGOING, &plci->state);

	skb = AS_ISDN_alloc_l3msg(260, MT_SETUP);
...
answer:
	...
	as_isdn_fsm_event(fi, EV_PI_CONNECT_CONF, cmsg);
}
...
plci_connect_conf(struct FsmInst *fi, int event, void *arg)
{
	...
	if (cmsg->Info == 0) {
		...
		fsm_change_state(fi, ST_PLCI_P_1);
	} else {
		Send2Application(aplci, cmsg);
		fsm_change_state(fi, ST_PLCI_P_0);
		AppPlciDestr(aplci);
	}
}
...
{
	fsm_change_state(fi, ST_PLCI_P_2);
...
static void plci_resume_req(struct FsmInst *fi, int event, void *arg)
{
	...
	fsm_change_state(fi, ST_PLCI_P_RES);
	plciNewCrReq(plci);
...
plci_alert_req(struct FsmInst *fi, int event, void *arg)
{
	...
	if (test_and_set_bit(PLCI_STATE_ALERTING, &plci->state)) {
		...
		struct sk_buff	*skb = AS_ISDN_alloc_l3msg(10, MT_ALERTING);
		if (!skb) {
			int_error();
			goto answer;
		}
...
plci_connect_resp(struct FsmInst *fi, int event, void *arg)
{
	...
	if (cmsg->Reject == 0) { // accept
		...
		fsm_change_state(fi, ST_PLCI_P_4);
		cmsg_free(cmsg);
		return;
	}
	// ignore, reject 
	...
	if (plci->nAppl == 1) {
		...
		skb = AS_ISDN_alloc_l3msg(10, MT_DISCONNECT);
		if (!skb) {
			plciL4L3(plci, prim, NULL);
		} else {
			AS_ISDN_AddIE(skb, IE_CAUSE, cause);
	...
	if (as_isdn_fsm_event(&aplci->plci_m, EV_PI_DISCONNECT_IND, cmsg))
		cmsg_free(cmsg);
}
...
plci_connect_active_ind(struct FsmInst *fi, int event, void *arg)
{
	...
	fsm_change_state(fi, ST_PLCI_P_ACT);
	AppPlciLinkUp(aplci);
	if (test_bit(PLCI_STATE_STACKREADY, &aplci->plci->state))
...
static void plci_disconnect_req(struct FsmInst *fi, int event, void *arg)
{
	...
	fsm_change_state(fi, ST_PLCI_P_5);
...
{
	fsm_change_state(fi, ST_PLCI_P_6);
	Send2Application(fi->userdata, arg);
}
...
plci_cc_setup_conf(struct FsmInst *fi, int event, void *arg)
{
	...
	if (aplci->channel == -1) {/* no valid channel set */
		as_isdn_fsm_event(fi, EV_PI_CHANNEL_ERR, NULL);
		return;
	}
	CMSG_ALLOC(cmsg);
	...
	if (as_isdn_fsm_event(fi, EV_PI_CONNECT_ACTIVE_IND, cmsg))
		cmsg_free(cmsg);
}
...
plci_channel_err(struct FsmInst *fi, int event, void *arg)
{
	...
	skb = AS_ISDN_alloc_l3msg(10, MT_RELEASE_COMPLETE);
	if (skb) {
		cause[0] = 2;
		cause[1] = 0x80;
...
{ 
	...
	if (as_isdn_fsm_event(&aplci->plci_m, EV_PI_CONNECT_IND, cmsg))
		cmsg_free(cmsg);
}
...
plci_cc_setup_compl_ind(struct FsmInst *fi, int event, void *arg)
{
	...
	CMSG_ALLOC(cmsg);
	AppPlciCmsgHeader(aplci, cmsg, CAPI_CONNECT_ACTIVE, CAPI_IND);
	if (as_isdn_fsm_event(&aplci->plci_m, EV_PI_CONNECT_ACTIVE_IND, cmsg))
		cmsg_free(cmsg);
}
...
AppPlci_suspend_reply(AppPlci_t *aplci, __u16 SuppServiceReason)
{
	...
	if (SuppServiceReason == CapiSuccess)
		as_isdn_fsm_event(&aplci->plci_m, EV_PI_SUSPEND_CONF, NULL);
}
...
plci_cc_resume_conf(struct FsmInst *fi, int event, void *arg)
{
	...
	cmsg->FacilitySelector = 0x0003;
	cmsg->FacilityIndicationParameter = tmp;
	if (as_isdn_fsm_event(&aplci->plci_m, EV_PI_RESUME_CONF, cmsg))
		cmsg_free(cmsg);
}
...
plci_info_req_overlap(struct FsmInst *fi, int event, void *arg)
{
	...
	skb = AS_ISDN_alloc_l3msg(100, MT_INFORMATION);
	if (skb) {
...
void AppPlciDestr(AppPlci_t *aplci)
{
	...
	if (aplci->plci) {
		...
		if (aplci->plci_m.state != ST_PLCI_P_0) {
			struct sk_buff	*skb = AS_ISDN_alloc_l3msg(10, MT_RELEASE_COMPLETE);
...
AppPlciRelease(AppPlci_t *aplci)
{
	...
	as_isdn_fsm_event(&aplci->plci_m, EV_AP_RELEASE, NULL);
}
...
AppPlciLinkUp(AppPlci_t *aplci)
{
	...
	memset(&aplci->link->inst.pid, 0, sizeof(AS_ISDN_pid_t));
...
static int
PL_l3l4(as_isdn_if_t *hif, struct sk_buff *skb)
{
	AppPlci_t		*aplci;
	Ncci_t 			*ncci;
	int			ret = -EINVAL;
...
static int
PL_l3l4mux(as_isdn_if_t *hif, struct sk_buff *skb)
{
	AppPlci_t	*aplci;
	Ncci_t 		*ncci;
	int		ret = -EINVAL;
...
int
AppPlciAS_ISDN_SetIF(AppPlci_t *aplci, u_int prim, void *arg)
{
	int ret;

	if (aplci->Bprotocol.B3 == 0) // transparent
		ret = AS_ISDN_SetIF(&aplci->link->inst, arg, prim, NULL, PL_l3l4, aplci);
	else
		ret = AS_ISDN_SetIF(&aplci->link->inst, arg, prim, NULL, PL_l3l4mux, aplci);
	if (ret)
		return(ret);
...
AppPlci_l3l4(AppPlci_t *aplci, int pr, void *arg)
{
	...
	switch (pr) {
		case CC_SETUP | INDICATION:
			...
			as_isdn_fsm_event(&aplci->plci_m, EV_L3_SETUP_IND, arg); 
			break;
		case CC_TIMEOUT | INDICATION:
			as_isdn_fsm_event(&aplci->plci_m, EV_L3_SETUP_CONF_ERR, arg); 
			break;
		case CC_CONNECT | INDICATION:
			...
			as_isdn_fsm_event(&aplci->plci_m, EV_L3_SETUP_CONF, arg); 
			break;
		case CC_CONNECT_ACKNOWLEDGE | INDICATION:
			...
			as_isdn_fsm_event(&aplci->plci_m, EV_L3_SETUP_COMPL_IND, arg); 
			break;
		case CC_DISCONNECT | INDICATION:
			if (qi) {
		  	...
		  	as_isdn_fsm_event(&aplci->plci_m, EV_L3_DISCONNECT_IND, arg); 
			break;
		case CC_RELEASE | INDICATION:
			if (qi) {
		        ...
		        as_isdn_fsm_event(&aplci->plci_m, EV_L3_RELEASE_IND, arg); 
			break;
		...
		case CC_RELEASE_CR | INDICATION:
			as_isdn_fsm_event(&aplci->plci_m, EV_L3_RELEASE_PROC_IND, arg); 
			break;
		...
		case CC_SUSPEND_ACKNOWLEDGE | INDICATION:
			as_isdn_fsm_event(&aplci->plci_m, EV_L3_SUSPEND_CONF, arg); 
			break;
		case CC_SUSPEND_REJECT | INDICATION:
			as_isdn_fsm_event(&aplci->plci_m, EV_L3_SUSPEND_ERR, arg); 
			break;
		case CC_RESUME_ACKNOWLEDGE | INDICATION:
			as_isdn_fsm_event(&aplci->plci_m, EV_L3_RESUME_CONF, arg); 
			break;
		case CC_RESUME_REJECT | INDICATION:
			as_isdn_fsm_event(&aplci->plci_m, EV_L3_RESUME_ERR, arg); 
			break;
		case CC_NOTIFY | INDICATION:
			as_isdn_fsm_event(&aplci->plci_m, EV_L3_NOTIFY_IND, arg); 
			break;
		case PH_CONTROL | INDICATION:
			/* TOUCH TONE */
			as_isdn_fsm_event(&aplci->plci_m, EV_PH_CONTROL_IND, arg);
			break;
		default:
...
AppPlciGetCmsg(AppPlci_t *aplci, _cmsg *cmsg)
{
	...
	switch (CMSGCMD(cmsg)) {
		case CAPI_INFO_REQ:
			retval = as_isdn_fsm_event(&aplci->plci_m, EV_AP_INFO_REQ, cmsg);
			break;
		case CAPI_ALERT_REQ:
			retval = as_isdn_fsm_event(&aplci->plci_m, EV_AP_ALERT_REQ, cmsg);
			break;
		case CAPI_CONNECT_REQ:
			retval = as_isdn_fsm_event(&aplci->plci_m, EV_AP_CONNECT_REQ, cmsg);
			break;
		case CAPI_CONNECT_RESP:
			retval = as_isdn_fsm_event(&aplci->plci_m, EV_AP_CONNECT_RESP, cmsg);
			break;
		case CAPI_DISCONNECT_REQ:
			retval = as_isdn_fsm_event(&aplci->plci_m, EV_AP_DISCONNECT_REQ, cmsg);
			break;
		case CAPI_DISCONNECT_RESP:
			retval = as_isdn_fsm_event(&aplci->plci_m, EV_AP_DISCONNECT_RESP, cmsg);
			break;
		case CAPI_CONNECT_ACTIVE_RESP:
			retval = as_isdn_fsm_event(&aplci->plci_m, EV_AP_CONNECT_ACTIVE_RESP, cmsg);
			break;
		case CAPI_SELECT_B_PROTOCOL_REQ:
			retval = as_isdn_fsm_event(&aplci->plci_m, EV_AP_SELECT_B_PROTOCOL_REQ, cmsg);
			break;
		default:
			int_error();
			retval = -1;
...
AppPlciFacSuspendReq(AppPlci_t *aplci, FacReqParm_t *facReqParm, FacConfParm_t *facConfParm)
{
	...
	if (CallIdentity && CallIdentity[0] > 8) 
		return CapiIllMessageParmCoding;
	skb = AS_ISDN_alloc_l3msg(20, MT_SUSPEND);
	if (!skb) {
		int_error();
		return CapiIllMessageParmCoding;
	}
	if (CallIdentity && CallIdentity[0])
		AS_ISDN_AddIE(skb, IE_CALL_ID, CallIdentity);

	if (as_isdn_fsm_event(&aplci->plci_m, EV_AP_SUSPEND_REQ, skb)) {
		// no routine
...
AppPlciFacResumeReq(AppPlci_t *aplci, FacReqParm_t *facReqParm, FacConfParm_t *facConfParm)
{
	...
	skb = AS_ISDN_alloc_l3msg(20, MT_RESUME);
	if (!skb) {
		int_error();
		AppPlciDestr(aplci);
		return CapiIllMessageParmCoding;
	...
	if (CallIdentity && CallIdentity[0])
		AS_ISDN_AddIE(skb, IE_CALL_ID, CallIdentity);
	if (as_isdn_fsm_event(&aplci->plci_m, EV_AP_RESUME_REQ, skb))
		kfree(skb);
...
AppPlciClearOtherApps(AppPlci_t *aplci)
{
	...
	list_for_each_safe(item, next, &aplci->plci->AppPlcis) {
		...
		if (o_aplci != aplci) {
			...
			as_isdn_fsm_event(&o_aplci->plci_m, EV_PI_DISCONNECT_IND, cm);
		}
	} 
}
...
AppPlciInfoIndIE(AppPlci_t *aplci, unsigned char ie, __u32 mask, Q931_info_t *qi)
{
	...
	if (ie & 0x80) { /* single octett */
		...
		if (AS_ISDN_l3_ie2pos(ie) < 0)
			return;
		ies += AS_ISDN_l3_ie2pos(ie);
		if (!*ies)
			return;
		iep = (u_char *)qi;
...
void init_AppPlci(void)
{
	...
	as_isdn_fsm_new(&plci_fsm, fn_plci_list, FN_PLCI_COUNT);
}


void free_AppPlci(void)
{
	as_isdn_fsm_free(&plci_fsm);
}
