/*
 * $Id: kcapi.c,v 1.1.1.1 2006/11/30 16:27:58 lizhijie Exp $
 */
				...
				__u16 appl, struct sk_buff *skb)
{
	...
	if (showctl & 2) {
		if (showctl & 1) {
			printk(KERN_ERR  "kcapi: got [0x%lx] id#%d %s len=%u\n",
			       (unsigned long) card->cnr,
			       CAPIMSG_APPID(skb->data),
			       capi_cmd2str(cmd, subcmd),
			       CAPIMSG_LEN(skb->data));
		} else {
			printk(KERN_ERR  "kcapi: got [0x%lx] %s\n",
					(unsigned long) card->cnr,
...
{
	if (!card->blocked) {
		printk(KERN_ERR  "kcapi: card %d suspend\n", CARDNR(card));
		card->blocked = 1;
	}
}
...
{
	if (card->blocked) {
		printk(KERN_ERR  "kcapi: card %d resume\n", CARDNR(card));
		card->blocked = 0;
	}
}
...
static __u16 capi_put_message(__u16 applid, struct sk_buff *skb)
{
	...
	if (showctl & 2) {
		if (showctl & 1) {
			printk(KERN_ERR  "kcapi: put [0x%lx] id#%d %s len=%u\n",
			       (unsigned long) contr,
			       CAPIMSG_APPID(skb->data),
			       capi_cmd2str(cmd, subcmd),
			       CAPIMSG_LEN(skb->data));
		} else {
			printk(KERN_ERR  "kcapi: put [0x%lx] %s\n",
					(unsigned long) contr,
					capi_message2str(skb->data));
		}
...
static int old_capi_manufacturer(unsigned int cmd, void *data)
{
	...
	case AVMB1_LOAD_AND_CONFIG:
		...
		if (card->driver->load_firmware == 0) {
			printk(KERN_ERR  "kcapi: load: driver \%s\" has no load function\n", card->driver->name);
			return -ESRCH;
		}

		if (ldef.t4file.len <= 0) {
			printk(KERN_ERR  "kcapi: load: invalid parameter: length of t4file is %d ?\n", ldef.t4file.len);
			return -EINVAL;
		}
		if (ldef.t4file.data == 0) {
			printk(KERN_ERR  "kcapi: load: invalid parameter: dataptr is 0\n");
			return -EINVAL;
		}
