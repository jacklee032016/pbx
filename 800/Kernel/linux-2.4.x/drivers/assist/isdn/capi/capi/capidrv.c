/*
 * $Id: capidrv.c,v 1.1.1.1 2006/11/30 16:27:58 lizhijie Exp $
...
static void listen_change_state(capidrv_contr * card, int event)
{
	...
	while (p->event) {
		if (card->state == p->actstate && p->event == event) {
			if (debugmode)
				printk(KERN_ERR  "capidrv-%d: listen_change_state %d -> %d\n",
...
static void plci_change_state(capidrv_contr * card, capidrv_plci * plci, int event)
{
	...
	while (p->event) {
		if (plci->state == p->actstate && p->event == event) {
			if (debugmode)
				printk(KERN_ERR  "capidrv-%d: plci_change_state:0x%x %d -> %d\n",
...
static void ncci_change_state(capidrv_contr * card, capidrv_ncci * ncci, int event)
{
	...
	while (p->event) {
		if (ncci->state == p->actstate && p->event == event) {
			if (debugmode)
				printk(KERN_ERR  "capidrv-%d: ncci_change_state:0x%x %d -> %d\n",
...
static void handle_controller(_cmsg * cmsg)
{
	...
	case CAPI_LISTEN_CONF:	/* Controller */
		if (debugmode)
			printk(KERN_ERR  "capidrv-%d: listenconf Info=0x%4x (%s) cipmask=0x%x\n",
...
static void capidrv_signal(__u16 applid, void *dummy)
{
	...
	while ((*capifuncs->capi_get_message) (global.appid, &skb) == CAPI_NOERROR) {
		...
		if (debugmode > 2)
			printk(KERN_ERR  "capidrv_signal: applid=%d %s\n",
			     ...
			     int send, int level2, __u8 *data, __u16 len)
{
    	...
    	if (!len) {
		printk(KERN_ERR  "capidrv-%d: avmb1_q931_data: len == %d\n",
				card->contrnr, len);
		return;
	}
...
static int capidrv_ioctl(isdn_ctrl * c, capidrv_contr * card)
{
	...
	case 1:
		...
		printk(KERN_ERR  "capidrv-%d: debugmode=%d\n",
				card->contrnr, debugmode);
		return 0;
	default:
		printk(KERN_ERR  "capidrv-%d: capidrv_ioctl(%ld) called ??\n",
				card->contrnr, c->arg);
		return -EINVAL;
	}
	return -EINVAL;
...
static int capidrv_command(isdn_ctrl * c, capidrv_contr * card)
{
	...
	case ISDN_CMD_DIAL:{
			...
			if (debugmode)
				printk(KERN_ERR  "capidrv-%d: ISDN_CMD_DIAL(ch=%ld,\"%s,%d,%d,%s\")\n",
					card->contrnr,
					c->arg,
				        c->parm.setup.phone,
				        c->parm.setup.si1,
			...
			if (isleasedline) {
				...
				called[0] = 0;
			        if (debugmode)
					printk(KERN_ERR  "capidrv-%d: connecting leased line\n", card->contrnr);
			} else {
	...
	case ISDN_CMD_ACCEPTD:
		...
		bchan = &card->bchans[c->arg % card->nbchan];
		if (debugmode)
			printk(KERN_ERR  "capidrv-%d: ISDN_CMD_ACCEPTD(ch=%ld) l2=%d l3=%d\n",
			       card->contrnr,
	...
	case ISDN_CMD_ACCEPTB:
		if (debugmode)
			printk(KERN_ERR  "capidrv-%d: ISDN_CMD_ACCEPTB(ch=%ld)\n",
			       card->contrnr,
			       c->arg);
		return -ENOSYS;
	...
	case ISDN_CMD_HANGUP:
		if (debugmode)
			printk(KERN_ERR  "capidrv-%d: ISDN_CMD_HANGUP(ch=%ld)\n",
			       card->contrnr,
			       c->arg);
		bchan = &card->bchans[c->arg % card->nbchan];
		...
		if (bchan->disconnecting) {
			if (debugmode)
				printk(KERN_ERR  "capidrv-%d: chan %ld already disconnecting ...\n",
				       card->contrnr,
				       c->arg);
			return 0;
		}
...
/* ready */
	...
	case ISDN_CMD_SETL2:
		if (debugmode)
			printk(KERN_ERR  "capidrv-%d: set L2 on chan %ld to %ld\n",
			       card->contrnr,
			       (c->arg & 0xff), (c->arg >> 8));
	...
	case ISDN_CMD_SETL3:
		if (debugmode)
			printk(KERN_ERR  "capidrv-%d: set L3 on chan %ld to %ld\n",
			       card->contrnr,
			       (c->arg & 0xff), (c->arg >> 8));
		bchan = &card->bchans[(c->arg & 0xff) % card->nbchan];
		bchan->l3 = (c->arg >> 8);
	...
	case ISDN_CMD_SETEAZ:
		if (debugmode)
			printk(KERN_ERR  "capidrv-%d: set EAZ \"%s\" on chan %ld\n",
			       card->contrnr,
			       c->parm.num, c->arg);
		bchan = &card->bchans[c->arg % card->nbchan];
	...
	case ISDN_CMD_CLREAZ:
		if (debugmode)
			printk(KERN_ERR  "capidrv-%d: clearing EAZ on chan %ld\n",
					card->contrnr, c->arg);
		bchan = &card->bchans[c->arg % card->nbchan];
		bchan->msn[0] = 0;
		return 0;
	...
	case ISDN_CMD_LOCK:
		if (debugmode > 1)
			printk(KERN_ERR  "capidrv-%d: ISDN_CMD_LOCK (%ld)\n", card->contrnr, c->arg);
		MOD_INC_USE_COUNT;
		break;

	case ISDN_CMD_UNLOCK:
		if (debugmode > 1)
			printk(KERN_ERR  "capidrv-%d: ISDN_CMD_UNLOCK (%ld)\n",
					card->contrnr, c->arg);
		MOD_DEC_USE_COUNT;
		break;
...
/* never called */
	case ISDN_CMD_GETL2:
		if (debugmode)
			printk(KERN_ERR  "capidrv-%d: ISDN_CMD_GETL2\n",
					card->contrnr);
		return -ENODEV;
	case ISDN_CMD_GETL3:
		if (debugmode)
			printk(KERN_ERR  "capidrv-%d: ISDN_CMD_GETL3\n",
					card->contrnr);
		return -ENODEV;
	case ISDN_CMD_GETEAZ:
		if (debugmode)
			printk(KERN_ERR  "capidrv-%d: ISDN_CMD_GETEAZ\n",
					card->contrnr);
		return -ENODEV;
	case ISDN_CMD_SETSIL:
		if (debugmode)
			printk(KERN_ERR  "capidrv-%d: ISDN_CMD_SETSIL\n",
					card->contrnr);
		return -ENODEV;
	case ISDN_CMD_GETSIL:
		if (debugmode)
			printk(KERN_ERR  "capidrv-%d: ISDN_CMD_GETSIL\n",
					card->contrnr);
		return -ENODEV;
	default:
...
static int if_sendbuf(int id, int channel, int doack, struct sk_buff *skb)
{
	...
	if (debugmode > 1)
		printk(KERN_ERR  "capidrv-%d: sendbuf len=%d skb=%p doack=%d\n",
	...
	if (skb_headroom(skb) < msglen) {
		...
		printk(KERN_ERR  "capidrv-%d: only %d bytes headroom, need %d\n",
...
static int capidrv_delcontr(__u16 contr)
{
	...
	if (debugmode)
		printk(KERN_ERR  "capidrv-%d: id=%d unloading\n",
	...
	while (card->nbchan) {
		...
		cmd.arg = card->nbchan-1;
	        cmd.parm.num[0] = 0;
		if (debugmode)
			printk(KERN_ERR  "capidrv-%d: id=%d disable chan=%ld\n",
	...
	if (debugmode)
		printk(KERN_ERR  "capidrv-%d: id=%d isdn unload\n",
					card->contrnr, card->myid);
	...
	if (debugmode)
		printk(KERN_ERR  "capidrv-%d: id=%d remove contr from list\n",
					card->contrnr, card->myid);
