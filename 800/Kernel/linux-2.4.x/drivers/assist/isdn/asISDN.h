/* $Id: asISDN.h,v 1.1.1.1 2006/11/30 16:27:58 lizhijie Exp $
 *
 */
 ...
 *
 */
#define	AS_ISDN_MAJOR_VERSION	1
#define	AS_ISDN_MINOR_VERSION	0
#define	AS_ISDN_VERSION		((AS_ISDN_MAJOR_VERSION<<16) | AS_ISDN_MINOR_VERSION)

/* global compile macro */
#define AS_ISDN_DEBUG_FSM				0

/* Layer 1 compile macro */
#define AS_ISDN_DEBUG_L1_DATA			0
#define AS_ISDN_DEBUG_L1_CTRL			0
#define AS_ISDN_DEBUG_L1_INIT			0

/* Layer 2 compile macro */
#define AS_ISDN_DEBUG_L2_DATA			0
#define AS_ISDN_DEBUG_L2_CTRL			0
#define AS_ISDN_DEBUG_L2_INIT			0
#define AS_ISDN_DEBUG_L2_TEI			0
#define AS_ISDN_DEBUG_L2_STATE		0

/* Layer 3 compile macro */
#define AS_ISDN_DEBUG_L3_DATA			0
#define AS_ISDN_DEBUG_L3_CTRL			0
#define AS_ISDN_DEBUG_L3_INIT			0
#define AS_ISDN_DEBUG_L3_STATE		0

/* Core compile macro */
#define AS_ISDN_DEBUG_CORE_DATA		0
#define AS_ISDN_DEBUG_CORE_CTRL		0
#define AS_ISDN_DEBUG_CORE_INIT		0
#define AS_ISDN_DEBUG_CORE_STATE		0
#define AS_ISDN_DEBUG_CORE_BCHAN		0


#define	AS_ISDN_MAX_EVENTSIZE			64	/* 64 events max in buffer */
...
// jolly patch start: use to request and confirm link status via DINFO
#define DL_STATUS			0x020300
// jolly patch stop
#define MDL_UNITDATA		0x121200
#define MDL_ASSIGN			0x022100
#define MDL_REMOVE			0x022000
#define MDL_ERROR			0x023000
/* following MDL primitives are not defined in INS specs */
#define MDL_INFORMATION	0x024000
#define MDL_STATUS			0x028100
#define MDL_FINDTEI			0x028200

// jolly patch start: use to send and receive management informations
/* layer 2 status info */
#define SDL_ESTAB			0x0002 
#define SDL_REL				0x0003 

// jolly patch stop
/* layer 3 : Call Control(CC)*/
...
#define ISDN_PID_L1_B_64TRANS			0x41000002		/* B Channel 64k bps Transient */
...
/* for Japan INS network */
#define ISDN_PID_L3_INS_USER			0x03000002
#define ISDN_PID_L3_INS_NET			0x03000110
...
#define AS_ISDN_CORE_DEVICE			127
#define AS_ISDN_RAW_DEVICE			128

/* status of AS_ISDNdevice */
#define FLG_AS_ISDNPORT_BUSY			1
#define FLG_AS_ISDNPORT_ENABLED		2
#define FLG_AS_ISDNPORT_BLOCK			3	/* blocked :can not be used */
#define FLG_AS_ISDNPORT_OPEN			4
#define FLG_AS_ISDNPORT_ONEFRAME		5
...
#define AS_ISDN_MAX_ENTITY				2048
#define AS_ISDN_ENTITY_NONE			0x0000ffff
#define AS_ISDN_ID_ENTITYMASK			0xffff0000
#define AS_ISDN_ID_LOCALMASK			0x0000FFFF
#define AS_ISDN_ID_ANY					0xffffffff
#define AS_ISDN_ID_NONE				0xfffffffe
#define AS_ISDN_ID_DUMMY				0xffff0001
#define AS_ISDN_ID_GLOBAL				0xffff0002

#define MAX_LAYER_NR					7
#define ISDN_LAYER(n)					(1<<n)
#define LAYER_OUTRANGE(layer)			((layer<0) || (layer>MAX_LAYER_NR))
#define AS_ISDN_MAX_IDLEN				16
...
/* modify as 512, lizhijie,2005.12.18 */
//#define DEFAULT_RAW_POR_QUEUE_LENGTH			110
#define DEFAULT_RAW_POR_QUEUE_LENGTH			10
#define DEFAULT_CORE_PORT_QUEUE_LENGTH			256
//#define DEFAULT_PORT_QUEUELEN			10
...
/* below is data structure exchanged between user space and kernel space */
typedef struct _AS_ISDN_head
{
...
#define AS_ISDN_HEADER_LEN	sizeof(as_isdn_head_t)
...
typedef struct _AS_ISDN_pid
{
	int		protocol[MAX_LAYER_NR +1];
	u_char	*param[MAX_LAYER_NR +1];	/* param offset in pbuf for every layer */
	__u16	global;						/* as caller, it is 1 */
	int		layermask;
	int		maxplen;
	u_char	*pbuf;						/* keep param for every layer */
} AS_ISDN_pid_t;

/* stack parameter */
typedef struct _AS_ISDN_stPara
{
	int	maxdatalen;
	int	up_headerlen;
	int	down_headerlen;
...
{
	u_int			id;						/* Stack ID for D channal */
	AS_ISDN_pid_t		pid;
	AS_ISDN_stPara_t	para;
	u_int			extentions;
	u_int			mgr;
	int				instcnt;					/* instance count */
	int				inst[MAX_LAYER_NR +1];	/* instance ID */
	int				childcnt;				/* B channel Stack is the child of D channel Stack */
	u_int			child[2]; 				/* Stack ID for 2 B channels, this is correct handled for PRI see get_stack_info() */
} stack_info_t;

typedef struct _layer_info
{
	char			name[AS_ISDN_MAX_IDLEN];
	int			object_id;
	int			extentions;
	u_int		id;
	u_int		st;
...
/* keep info about Layer3 msg, so it is info about Q931  */
typedef struct _Q931_info
{
	...
	/* from here, all field keep the position info of IE in L3 message */
	...
	/* from CAUSE to USERUSERr, there are 32 IEs */
	
	u16	more_data __attribute__((packed));			/* just offset of more_data in skb, not offered in INS */
	u16	sending_complete __attribute__((packed));		/* just offset of more_data in skb, not offered in INS */
	u16	congestion_level __attribute__((packed));		/* just offset of more_data in skb, not offered in INS */
	u16	fill1 __attribute__((packed));					/* for IEs in codeset 6, lizhijie, 2006.06.12 */
} Q931_info_t;
...
typedef struct _AS_ISDNobject			as_isdn_object_t;
typedef struct _AS_ISDNinstance		as_isdn_instance_t;
typedef struct _AS_ISDNlayer			as_isdn_layer_t;
typedef struct _AS_ISDNstack			as_isdn_stack_t;
...
#define AS_ISDN_HEAD_P(s)		((as_isdn_head_t *)&s->cb[0])
#define AS_ISDN_HEAD_PRIM(s)	((as_isdn_head_t *)&s->cb[0])->prim
#define AS_ISDN_HEAD_DINFO(s)	((as_isdn_head_t *)&s->cb[0])->dinfo

typedef struct _AS_ISDN_headext
{
	u_int	addr __attribute__((packed));
	u_int	prim __attribute__((packed));
	int	dinfo  __attribute__((packed));
...
/* from sk_buf */
#define AS_ISDN_HEADEXT_P(s) ((AS_ISDN_headext_t *)&s->cb[0])

/* Basic struct of a AS_ISDN component */
struct _AS_ISDNobject
{
	...
	AS_ISDN_pid_t		DPROTO;	/* D Channel Protocol */
	AS_ISDN_pid_t		BPROTO;		/* B Channel Protocol */
	ctrl_func_t		*own_ctrl;
	ctrl_func_t		*ctrl;
	struct list_head	ilist;		/* private data in every AS_ISDNobject, such as layer1_t in isdnl1 */
	struct module	*owner;
};

/* the interface between two AS_ISDNinstances 
* this object can only exist in AS_ISDNinstance or AS_ISDNport
*/
struct _AS_ISDNif
{
	if_func_t			*func;
...
/* a instance of a AS_ISDNobject */
struct _AS_ISDNinstance
{
	struct list_head	list;
	char				name[AS_ISDN_MAX_IDLEN];
	int				extentions;
	...
	void				*data;		/* data or structure used in AS_ISDNobject in this instance, such as dtmf_t */
	as_isdn_if_t		up;
	as_isdn_if_t		down;
	lock_func_t		*lock;
...
struct _AS_ISDNstack
{
	struct list_head	list;		/* stack link which this one belone to */
	u_int			id;		/* 3 types : system stack IDm clone stack ID, child stack ID */
	u_int			extentions;
	AS_ISDN_pid_t		pid;
	AS_ISDN_stPara_t	para;
	struct list_head	layerlist;
	as_isdn_instance_t	*mgr;
	struct list_head	childlist;
};

/* lowlevel read/write struct for the AS_ISDNdevice */
struct _AS_ISDNport
{
...
/* the user interface to handle /dev/AS_ISDN */
struct _AS_ISDNdevice
{
	struct list_head		list;
	int					minor;
	...
	/* upper field must be same as as_isdn_rawdev_t */
	
	struct list_head		layerlist;	/* point to devicelayer_t */
	struct list_head		stacklist;	/* point to devicestack_t */
...
extern 	int	AS_ISDN_bprotocol2pid(void *, AS_ISDN_pid_t *);
extern 	int	AS_ISDN_SetIF(as_isdn_instance_t *, as_isdn_if_t *, u_int, void *, void *, void *);
extern 	int	AS_ISDN_ConnectIF(as_isdn_instance_t *, as_isdn_instance_t *);
extern 	int	AS_ISDN_DisConnectIF(as_isdn_instance_t *, as_isdn_if_t *);

/* global register/unregister functions */

extern 	int	as_isdn_register(as_isdn_object_t *obj);
extern 	int	as_isdn_unregister(as_isdn_object_t *obj);
...
#define TRACE()  printk(KERN_ERR "kernel TRACE at %s->%s:%d!\n", __FILE__, __FUNCTION__, __LINE__)
