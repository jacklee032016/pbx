/*
 * $Id: tei.c,v 1.1.1.1 2006/11/30 16:27:58 lizhijie Exp $
 */
#include "layer2.h"
#include "helper.h"
#if 0
...
enum
{
	...
	EV_REMOVE,
	EV_VERIFY,
	EV_VERIFY_NT,		/* NT device rx VERIFY and reply check request */
	EV_T202,
};
...
//		tm->tei_m.printdebug(&tm->tei_m, "net tei assign request without tei");
	...
	printk(KERN_ERR"net assign request ri %d teim %d\n", tm->ri, *dp);
...
/* NT rx VERIFY(7) from user side and reply with CHECK REQUEST, added by lizhijie,2006.06.12  */
static void tei_id_verify_2_check_request(struct FsmInst *fi, int event, void *arg)
{
	teimgr_t *tm = fi->userdata;
	u_char *dp = arg;
	int tei;
...
//		fsm_del_timer(&tm->t202, 4);
		fsm_change_state(&tm->tei_m, ST_TEI_NOP);
		/* ID check request : N-->U */
//		put_tei_msg(tm, ID_CHK_REQ, random_ri(), tm->l2->tei);
//		put_tei_msg(tm, ID_CHK_REQ, tm->ri, tm->l2->tei);
...
#if 0
		else if (mt == ID_VERIFY )//|| mt== ID_CHK_RES )
		{
			printk(KERN_ERR "TEI VERIFT requested send by user side\n");
			as_isdn_fsm_event(&tm->tei_m, EV_VERIFY_NT, dp);
		}	
#endif
...
//			tm->tei_m.printdebug(&tm->tei_m, "tei handler wrong mt %x", mt);
			printk(KERN_ERR "tei handler wrong mt %x\n", mt);
			return(ret);
		}
...
static struct FsmNode TeiFnList[] =
{
	...
	{ST_TEI_NOP, 		EV_VERIFY_NT, 	tei_id_verify_2_check_request},	/* added for Net side VERIFY ,lzj 2006.06.12  */
