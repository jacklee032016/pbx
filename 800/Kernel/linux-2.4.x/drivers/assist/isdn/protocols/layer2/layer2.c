/*
 * $Id: layer2.c,v 1.1.1.1 2006/11/30 16:27:58 lizhijie Exp $
 */
...
static int asl2_debug = 0;//0xFFFFFFFF;
static as_isdn_object_t isdnl2;

static struct Fsm l2fsm = {NULL, 0, 0, NULL, NULL};

/* refer to specs vol.1 page 235 */
enum
{
...
inline u_int l2headersize(layer2_t *l2, int ui)
{
	return (((test_bit(FLG_MOD128, &l2->flag) && (!ui)) ? 2 : 1) +
		(test_bit(FLG_LAPD, &l2->flag) ? 2 : 1));
}

inline u_int l2addrsize(layer2_t *l2)
{
	return (test_bit(FLG_LAPD, &l2->flag) ? 2 : 1);
}

static int l2_newid(layer2_t *l2)
{
	u_long	flags;
	int	id;
...
static int l2up(layer2_t *l2, u_int prim, int dinfo, struct sk_buff *skb)
{
	return(if_newhead(&l2->inst.up, prim, dinfo, skb));
}

static int l2up_create(layer2_t *l2, u_int prim, int dinfo, int len, void *arg)
{
	return(if_link(&l2->inst.up, prim, dinfo, len, arg, 0));
}

/* transfer skb with l2 down interface */
static int l2down_skb(layer2_t *l2, struct sk_buff *skb)
{
	as_isdn_if_t *down = &l2->inst.down;
	int ret = -ENXIO;

	if (down->func)
	{
#if AS_ISDN_DEBUG_L2_DATA
		printk(KERN_ERR  "send out in DOWN interface of %s\n", l2->inst.name);
#endif
	...
	{
		printk(KERN_ERR  "No DOWN interface of %s\n", l2->inst.name);
	}
#endif

#if AS_ISDN_DEBUG_L2_DATA
	if (ret && l2->debug)
		printk(KERN_ERR  "l2down_skb: ret(%d)\n", ret);
#endif
	return(ret);
}

static int l2down_raw(layer2_t *l2, struct sk_buff *skb)
{
	...
	if (hh->prim == PH_DATA_REQ)
	{
		...
		l2->down_id = AS_ISDN_HEAD_DINFO(skb);
	}
	return(l2down_skb(l2, skb));
}

static int l2down(layer2_t *l2, u_int prim, int dinfo, struct sk_buff *skb)
{
	AS_ISDN_sethead(prim, dinfo, skb);
	return(l2down_raw(l2, skb));
}

static int l2down_create(layer2_t *l2, u_int prim, int dinfo, int len, void *arg)
{
	struct sk_buff	*skb;
	int		err;
...
static int l2_chain_down(as_isdn_if_t *hif, struct sk_buff *skb)
{
	if (!hif || !hif->fdata)
		return(-EINVAL);
...
static int ph_data_confirm(as_isdn_if_t *up, as_isdn_head_t *hh, struct sk_buff *skb)
{
	layer2_t *l2 = up->fdata;
	struct sk_buff *nskb = skb; 
	as_isdn_if_t *next = up->clone;
	int ret = -EAGAIN;
	...
	if (test_bit(FLG_L1_BUSY, &l2->flag))
	{
		if (hh->dinfo == l2->down_id)
		{
			...
			{
				l2->down_id = AS_ISDN_HEAD_DINFO(nskb);
				if (l2down_skb(l2, nskb))
				{
					dev_kfree_skb(nskb);
					l2->down_id = AS_ISDN_ID_NONE;
				}
			}
			else
				l2->down_id = AS_ISDN_ID_NONE;
			...
			if (l2->down_id == AS_ISDN_ID_NONE)
			{
				test_and_clear_bit(FLG_L1_BUSY, &l2->flag);
				as_isdn_fsm_event(&l2->l2m, EV_L2_ACK_PULL, NULL);
			}
		}
	}
...
static int l2mgr(layer2_t *l2, u_int prim, void *arg)
{
	long c = (long)arg;

#if AS_ISDN_DEBUG_L2_CTRL
	printk(KERN_WARNING "l2mgr: prim %x %c\n", prim, (char)c);
#endif

	if (test_bit(FLG_LAPD, &l2->flag) &&!test_bit(FLG_FIXED_TEI, &l2->flag))
	{
		struct sk_buff  *skb;
		switch(c)
		{
...
static void set_peer_busy(layer2_t *l2)
{
...
static void clear_peer_busy(layer2_t *l2)
{
	if (test_and_clear_bit(FLG_PEER_BUSY, &l2->flag))
		test_and_clear_bit(FLG_L2BLOCK, &l2->flag);
}

static void InitWin(layer2_t *l2)
{
	int i;
...
static int freewin(layer2_t *l2)
{
	int i, cnt = 0;
...
static void ReleaseWin(layer2_t *l2)
{
	int cnt;
...
inline unsigned int cansend(layer2_t *l2)
{
	unsigned int p1;
...
inline void clear_exception(layer2_t *l2)
{
...
/* set address filed in layer2 frame header */
static int sethdraddr(layer2_t *l2, u_char *header, int rsp)
{
	u_char *ptr = header;
	int crbit = rsp;
	...
	if (test_bit(FLG_LAPD, &l2->flag))
	{
		if (test_bit(FLG_LAPD_NET, &l2->flag))
			crbit = !crbit;
		*ptr++ = (l2->sapi << 2) | (crbit ? 2 : 0);		/* support user side and network side both */
		*ptr++ = (l2->tei << 1) | 1;
		return (2);
	}
	else
	{/* not LAPD, for example LAPB , address file is only one byte */
...
inline static void enqueue_super(layer2_t *l2, struct sk_buff *skb)
{
	if (l2down(l2, PH_DATA | REQUEST, l2_newid(l2), skb))
		dev_kfree_skb(skb);
}

inline static void enqueue_ui(layer2_t *l2, struct sk_buff *skb)
{
	if (l2down(l2, PH_DATA | REQUEST, AS_ISDN_HEAD_DINFO(skb), skb))
		dev_kfree_skb(skb);
}
...
inline int IsUA(u_char * data)
{
	return ((data[0] & 0xef) == UA);
}

inline int IsDM(u_char * data)
{
	return ((data[0] & 0xef) == DM);
}

inline int IsDISC(u_char * data)
{
	return ((data[0] & 0xef) == DISC);
}

inline int IsRR(u_char * data, layer2_t *l2)
{
	if (test_bit(FLG_MOD128, &l2->flag))
		return (data[0] == RR);
	else
...
/* what is Supervisory Frame for I Frame:ACK|reqyest retransmission|temporary suspension of I frame */
inline int IsSFrame(u_char * data, layer2_t *l2)
{
...
inline int IsSABME(u_char * data, layer2_t *l2)
{
...
inline int IsREJ(u_char * data, layer2_t *l2)
{
	return (test_bit(FLG_MOD128, &l2->flag) ? data[0] == REJ : (data[0] & 0xf) == REJ);
}

inline int IsFRMR(u_char * data)
{
	return ((data[0] & 0xef) == FRMR);
}

inline int IsRNR(u_char * data, layer2_t *l2)
{
	return (test_bit(FLG_MOD128, &l2->flag) ? data[0] == RNR : (data[0] & 0xf) == RNR);
}

int iframe_error(layer2_t *l2, struct sk_buff *skb)
{
...
int super_error(layer2_t *l2, struct sk_buff *skb)
{
...
int unnum_error(layer2_t *l2, struct sk_buff *skb, int wantrsp)
{
...
int UI_error(layer2_t *l2, struct sk_buff *skb)
{
	int rsp ;
...
//	TRACE();
	...
	{
		printk(KERN_ERR "fatal error : skb is null\n");
		return 'O';
	}
//	TRACE();
	if (test_bit(FLG_ORIG, &l2->flag))
...
int FRMR_error(layer2_t *l2, struct sk_buff *skb)
{
...
//		else if (l2->debug)
		{
//			l2m_debug(&l2->l2m, "FRMR information %2x %2x %2x %2x %2x", datap[0], datap[1], datap[2], datap[3], datap[4]);
			printk(KERN_WARNING "FRMR information %2x %2x %2x %2x %2x\n", datap[0], datap[1], datap[2], datap[3], datap[4]);
		}	
	} 
	else
	{
...
//		else if (l2->debug)
		{
//			l2m_debug(&l2->l2m, "FRMR information %2x %2x %2x", datap[0], datap[1], datap[2]);
			printk(KERN_WARNING "FRMR information %2x %2x %2x", datap[0], datap[1], datap[2]);
		}
	}
	return 0;
}

/* check 'nr' is validate frame number just send out */
static unsigned int legalnr(layer2_t *l2, unsigned int nr)
{
	if(test_bit(FLG_MOD128, &l2->flag))
...
static void setva(layer2_t *l2, unsigned int nr)
{
	u_long		flags;
	struct sk_buff	*skb;
	...
	{
		if (l2up(l2, DL_DATA | CONFIRM, AS_ISDN_HEAD_DINFO(skb), skb))
			dev_kfree_skb(skb);
	}
}

static void send_uframe(layer2_t *l2, struct sk_buff *skb, u_char cmd, u_char cr)
{
...
inline u_char get_PollFlag(layer2_t *l2, struct sk_buff * skb)
{
	return (skb->data[l2addrsize(l2)] & 0x10);
}

inline u_char get_PollFlagFree(layer2_t *l2, struct sk_buff *skb)
{
	u_char PF;
...
inline void start_t200(layer2_t *l2, int i)
{
	fsm_add_timer(&l2->t200, l2->T200, EV_L2_T200, NULL, i);
	test_and_set_bit(FLG_T200_RUN, &l2->flag);
}

inline void restart_t200(layer2_t *l2, int i)
{
	fsm_restart_timer(&l2->t200, l2->T200, EV_L2_T200, NULL, i);
	test_and_set_bit(FLG_T200_RUN, &l2->flag);
}

inline void stop_t200(layer2_t *l2, int i)
{
	if(test_and_clear_bit(FLG_T200_RUN, &l2->flag))
		fsm_del_timer(&l2->t200, i);
}

inline void st5_dl_release_l2l3(layer2_t *l2)
{
	int pr;
...
inline void lapb_dl_release_l2l3(layer2_t *l2, int f)
{
...
static void establishlink(struct FsmInst *fi)
{
	...
	fsm_del_timer(&l2->t203, 1);
	restart_t200(l2, 1);
	test_and_clear_bit(FLG_PEND_REL, &l2->flag);
	freewin(l2);
	fsm_change_state(fi, ST_L2_5);
}
 ...
 */
 	/* modify by lizhijie, 2005.09.19 */
//	TRACE();
 	ret = l2up(l2, DL_UNITDATA | INDICATION, AS_ISDN_HEAD_DINFO(skb), skb);
//	TRACE();
	if (ret)
	{
//		TRACE();
...
static void l2_start_multi(struct FsmInst *fi, int event, void *arg)
{
	...
	fsm_change_state(fi, ST_L2_7);
	fsm_add_timer(&l2->t203, l2->T203, EV_L2_T203, NULL, 3);
	skb_trim(skb, 0);
...
static void l2_restart_multi(struct FsmInst *fi, int event, void *arg)
{
	...
	fsm_change_state(fi, ST_L2_7);
	stop_t200(l2, 3);
	fsm_restart_timer(&l2->t203, l2->T203, EV_L2_T203, NULL, 3);

	if (est)
		l2up_create(l2, DL_ESTABLISH | INDICATION, 0, 0, NULL);

	if (skb_queue_len(&l2->i_queue) && cansend(l2))
		as_isdn_fsm_event(fi, EV_L2_ACK_PULL, NULL);
}
...
static void l2_stop_multi(struct FsmInst *fi, int event, void *arg)
{
	...
	fsm_change_state(fi, ST_L2_4);
	fsm_del_timer(&l2->t203, 3);
	stop_t200(l2, 4);
...
static void l2_connected(struct FsmInst *fi, int event, void *arg)
{
	...
	spin_unlock_irqrestore(&l2->lock, flags);
	fsm_change_state(fi, ST_L2_7);
	fsm_add_timer(&l2->t203, l2->T203, EV_L2_T203, NULL, 4);
	if (pr != -1)
		l2up_create(l2, pr, 0, 0, NULL);
...
void enquiry_cr(layer2_t *l2, u_char typ, u_char cr, u_char pf)
{
	struct sk_buff *skb;
	u_char tmp[MAX_HEADER_LEN];
	int i;
...
inline void enquiry_response(layer2_t *l2)
{
...
inline void transmit_enquiry(layer2_t *l2)
{
	if (test_bit(FLG_OWN_BUSY, &l2->flag))
...
static void invoke_retransmission(layer2_t *l2, unsigned int nr)
{
	u_int	p1;
	u_long	flags;
...
static void l2_st7_got_super(struct FsmInst *fi, int event, void *arg)
{
	...
	if (legalnr(l2, nr))
	{
		if (typ == REJ) {
			...
			if (fsm_add_timer(&l2->t203, l2->T203,	EV_L2_T203, NULL, 6))
			{
//				l2m_debug(&l2->l2m, "Restart T203 ST7 REJ");
				printk(KERN_WARNING "Restart T203 ST7 REJ\n");
			}	
		}
		else if ((nr == l2->vs) && (typ == RR))
		{
			setva(l2, nr);
			stop_t200(l2, 11);
			fsm_restart_timer(&l2->t203, l2->T203, EV_L2_T203, NULL, 7);
		}
		else if ((l2->va != nr) || (typ == RNR))
		{
			setva(l2, nr);
			if (typ != RR)
				fsm_del_timer(&l2->t203, 9);
			restart_t200(l2, 12);
		}
...
static void l2_got_iframe(struct FsmInst *fi, int event, void *arg)
{
	...
	else
	{
		...
		if (l2->vr == ns)
		{
			...
			if (l2up(l2, DL_DATA | INDICATION, AS_ISDN_HEAD_DINFO(skb), skb))
				dev_kfree_skb(skb);
		}
		else
		{
...
static void l2_st5_tout_200(struct FsmInst *fi, int event, void *arg)
{
	...
	if (test_bit(FLG_LAPD, &l2->flag) &&	test_bit(FLG_DCHAN_BUSY, &l2->flag)) 
	{
		fsm_add_timer(&l2->t200, l2->T200, EV_L2_T200, NULL, 9);
	}
	...
	else
	{
		l2->rc++;
		fsm_add_timer(&l2->t200, l2->T200, EV_L2_T200, NULL, 9);
		send_uframe(l2, NULL, (test_bit(FLG_MOD128, &l2->flag) ? SABME : SABM) | 0x10, CMD);
	}
}
...
static void l2_st7_tout_203(struct FsmInst *fi, int event, void *arg)
{
	...
	if (test_bit(FLG_LAPD, &l2->flag) && test_bit(FLG_DCHAN_BUSY, &l2->flag))
	{
		fsm_add_timer(&l2->t203, l2->T203, EV_L2_T203, NULL, 9);
		return;
	}
	fsm_change_state(fi, ST_L2_8);
	transmit_enquiry(l2);
...
static void l2_pull_iqueue(struct FsmInst *fi, int event, void *arg)
{
	...
	{
		printk(KERN_WARNING "isdnl2 pull_iqueue skb header(%d/%d) too short\n", i, p1);
		oskb = nskb;
	...
	l2down(l2, PH_DATA_REQ, AS_ISDN_HEAD_DINFO(skb), nskb);
	test_and_clear_bit(FLG_ACK_PEND, &l2->flag);
	if (!test_and_set_bit(FLG_T200_RUN, &l2->flag))
	{
		fsm_del_timer(&l2->t203, 13);
		fsm_add_timer(&l2->t200, l2->T200, EV_L2_T200, NULL, 11);
	}
}
...
static void l2_st8_got_super(struct FsmInst *fi, int event, void *arg)
{
	...
	if (rsp && PollFlag)
	{
		if (legalnr(l2, nr))
		{
			...
			{
				stop_t200(l2, 16);
				fsm_add_timer(&l2->t203, l2->T203,  EV_L2_T203, NULL, 5);
				setva(l2, nr);
			}
...
static int ph_data_indication(layer2_t *l2, as_isdn_head_t *hh, struct sk_buff *skb)
{
...
//	TRACE();
	...
	{
//		TRACE();
		as_isdn_fsm_event(&l2->l2m, EV_L2_FRAME_ERROR, (void *) 'N');
		return(ret);
	}
	...
	if (test_bit(FLG_LAPD, &l2->flag))
	{
		...
		if ((psapi & 1) || !(ptei & 1))
		{
			printk(KERN_WARNING "l2 D-channel frame wrong EA0/EA1(SAPI :0x%x : TEI 0x%x)\n", psapi, ptei );
			return(ret);
		}
...
//		TRACE();
		if(!(c = iframe_error(l2, skb)))
			ret = as_isdn_fsm_event(&l2->l2m, EV_L2_I, skb);
	}
	else if (IsSFrame(datap, l2))
	{	/* S-Frame */
//		TRACE();
		if(!(c = super_error(l2, skb)))
			ret = as_isdn_fsm_event(&l2->l2m, EV_L2_SUPER, skb);
	}
	else if (IsUI(datap))
	{
//		TRACE();
		...
		{
			ret = as_isdn_fsm_event(&l2->l2m, EV_L2_UI, skb);
		}
	}
...
//		TRACE();
		if(!(c = unnum_error(l2, skb, CMD)))
			ret = as_isdn_fsm_event(&l2->l2m, EV_L2_SABME, skb);
	}
	else if (IsUA(datap))
	{
//		TRACE();
		if(!(c = unnum_error(l2, skb, RSP)))
			ret = as_isdn_fsm_event(&l2->l2m, EV_L2_UA, skb);
	}
	else if (IsDISC(datap))
	{
//		TRACE();
		if(!(c = unnum_error(l2, skb, CMD)))
			ret = as_isdn_fsm_event(&l2->l2m, EV_L2_DISC, skb);
	}
	else if (IsDM(datap))
	{
//		TRACE();
		if(!(c = unnum_error(l2, skb, RSP)))
			ret = as_isdn_fsm_event(&l2->l2m, EV_L2_DM, skb);
	}
	else if (IsFRMR(datap))
	{
//		TRACE();
		if(!(c = FRMR_error(l2, skb)))
			ret = as_isdn_fsm_event(&l2->l2m, EV_L2_FRMR, skb);
	}
	else
	{
//		TRACE();
...
//		TRACE();
		...
		as_isdn_fsm_event(&l2->l2m, EV_L2_FRAME_ERROR, (void *)(long)c);
	}
//	TRACE();
...
/* function associated with down interface of layer2_t */
static int l2from_down(as_isdn_if_t *hif, struct sk_buff *askb)
{
	layer2_t	*l2;
	int 		ret = -EINVAL;
	struct sk_buff	*cskb = askb;
	as_isdn_if_t	*next;
	as_isdn_head_t	*hh, sh;

//	TRACE();
	if (!hif || !askb)
		return(-EINVAL);
	l2 = hif->fdata;
	hh = AS_ISDN_HEAD_P(askb);
	next = hif->clone;
#if AS_ISDN_DEBUG_L2_DATA
...
//	TRACE();
	...
	/* why request primitive is handled here ???*/
...
//			TRACE();
		...
		case (PH_ACTIVATE | INDICATION):
			...
			if (test_and_clear_bit(FLG_ESTAB_PEND, &l2->flag))
				ret = as_isdn_fsm_event(&l2->l2m, EV_L2_DL_ESTABLISH_REQ, cskb);
			break;

		case (PH_DEACTIVATE | INDICATION):
		case (PH_DEACTIVATE | CONFIRM):
			test_and_clear_bit(FLG_L1_ACTIV, &l2->flag);
			ret = as_isdn_fsm_event(&l2->l2m, EV_L1_DEACTIVATE, cskb);
			if (ret)
				dev_kfree_skb(cskb);
			ret = 0;
			break;
		...
		default:
			if (l2->debug)
			{
//				l2m_debug(&l2->l2m, "l2 unknown pr %x address %x dinfo %x", hh->prim, hh->addr, hh->dinfo);
				printk( KERN_WARNING "l2 unknown pr %x address %x dinfo %x\n", hh->prim, hh->addr, hh->dinfo);
			}	
			ret = -EINVAL;
			break;
	}
...
//	TRACE();
#if 0	
	/* commented by lizhijie, 2005.09.19
	skb is freed in D channel BH */
	if (ret)
	{
		dev_kfree_skb(cskb);
		ret = 0;
...
/* upper interface function in the up interface of layer2_t */
static int l2from_up(as_isdn_if_t *hif, struct sk_buff *skb)
{
	layer2_t	*l2;
	as_isdn_head_t	*hh;
	int		ret = -EINVAL;
...
//	TRACE();
			ret = as_isdn_fsm_event(&l2->l2m, EV_L2_DL_DATA, skb);
			break;
		case (DL_UNITDATA | REQUEST):
//	TRACE();
			ret = as_isdn_fsm_event(&l2->l2m, EV_L2_DL_UNITDATA, skb);
			break;
...
//	TRACE();
			if (test_bit(FLG_L1_ACTIV, &l2->flag))
			{
				...
				{
//	TRACE();
					ret = as_isdn_fsm_event(&l2->l2m, EV_L2_DL_ESTABLISH_REQ, skb);
				}
			}
			else
			{
...
//	TRACE();
		...
		case (DL_RELEASE | REQUEST):
			if (test_bit(FLG_LAPB, &l2->flag))
				l2down_create(l2, PH_DEACTIVATE | REQUEST, 0, 0, NULL);
//	TRACE();
			ret = as_isdn_fsm_event(&l2->l2m, EV_L2_DL_RELEASE_REQ, skb);
			break;
		case (MDL_ASSIGN | REQUEST):
//	TRACE();
			ret = as_isdn_fsm_event(&l2->l2m, EV_L2_MDL_ASSIGN, skb);
			break;
		case (MDL_REMOVE | REQUEST):
//	TRACE();
			ret = as_isdn_fsm_event(&l2->l2m, EV_L2_MDL_REMOVE, skb);
			break;
		case (MDL_ERROR | RESPONSE):
//	TRACE();
			ret = as_isdn_fsm_event(&l2->l2m, EV_L2_MDL_ERROR, skb);
		case (MDL_STATUS | REQUEST):
//	TRACE();
			l2up_create(l2, MDL_STATUS | CONFIRM, hh->dinfo, 1,	(void *)l2->tei);
			break;
		default:
			if (l2->debug)
			{
//				l2m_debug(&l2->l2m, "l2 unknown pr %04x address %x dinfo %x\n", hh->prim, hh->addr, hh->dinfo );
				printk(KERN_WARNING "l2 unknown pr %04x address %x dinfo %x\n", hh->prim, hh->addr, hh->dinfo );
			}	
	}
//	TRACE();
	return(ret);
...
/* command interface for L2 TEI manager */
int tei_l2(layer2_t *l2, struct sk_buff *skb)
{
	as_isdn_head_t	*hh;
	int		ret = -EINVAL;
...
static void release_l2(layer2_t *l2)
{
	as_isdn_instance_t  *inst = &l2->inst;

	fsm_del_timer(&l2->t200, 21);
	fsm_del_timer(&l2->t203, 16);
	discard_queue(&l2->i_queue);
	discard_queue(&l2->ui_queue);
	...
	if (l2->cloneif)
	{
#if AS_ISDN_DEBUG_L2_INIT
		printk(KERN_ERR  "%s: cloneif(%p) owner(%p) peer(%p)\n", __FUNCTION__, l2->cloneif, l2->cloneif->owner, l2->cloneif->peer);
#endif
		...
		if (l2->cloneif->owner && (l2->cloneif->owner->up.clone == l2->cloneif))
	...
	if (l2->entity != AS_ISDN_ENTITY_NONE)
		isdnl2.ctrl(inst, MGR_DELENTITY | REQUEST, (void *)l2->entity);
	kfree(l2);
}

static int new_l2(as_isdn_stack_t *st, AS_ISDN_pid_t *pid, layer2_t **newl2)
{
	layer2_t *nl2;
	int err;
	u_char *p;
	...
	{
		printk(KERN_ERR "ISDNL2-MGR : kmalloc layer2 failed\n");
		return(-ENOMEM);
	}
	memset(nl2, 0, sizeof(layer2_t));

	spin_lock_init(&nl2->lock);
	nl2->debug = asl2_debug;
	nl2->next_id = 1;
	nl2->down_id = AS_ISDN_ID_NONE;
	AS_ISDN_init_instance(&nl2->inst, &isdnl2, nl2);
	nl2->inst.extentions = EXT_INST_CLONE;
	memcpy(&nl2->inst.pid, pid, sizeof(AS_ISDN_pid_t));
	if (!AS_ISDN_SetHandledPID(&isdnl2, &nl2->inst.pid))
	{
		int_error();
		return(-ENOPROTOOPT);
	}
	...
	switch(pid->protocol[2] & ~ISDN_PID_FEATURE_MASK)
	{
		case ISDN_PID_L2_LAPD_NET:
			...
			test_and_set_bit(FLG_MOD128, &nl2->flag);	/* modulo 128 is need of INS Net */
			nl2->sapi = 0;
			nl2->tei = 88;		/* TEI is preallocated in network side */
			/* default system parameter in INS. Refer specs. vol.1. Page 230 */
		...
		case ISDN_PID_L2_LAPD:
			...
			nl2->sapi = 0;	/* SAPI for call control precedure */
			nl2->tei = -1; 	/* automtic TEI assignment when TEI =-1 */
			if (pid->protocol[2] & ISDN_PID_L2_DF_PTP)
			{/* P2P, so no automatic TEI assigned is needed. Here MDL deliver TEI for user side terminal 
			  refer specs, vol.1, page 202. clause 5.3.2 */
		...
		case ISDN_PID_L2_B_X75SLP:
			...
			break;
		default:
			printk(KERN_ERR "ISDNL2-MGR : layer2 create failed prt %x\n", pid->protocol[2]);
			kfree(nl2);
			return(-ENOPROTOOPT);
	}
	...
	InitWin(nl2);
	nl2->l2m.fsm = &l2fsm;
	if (test_bit(FLG_LAPB, &nl2->flag) ||	test_bit(FLG_PTP, &nl2->flag) ||
		test_bit(FLG_LAPD_NET, &nl2->flag))
		nl2->l2m.state = ST_L2_4;	/* TEI assigned : No TEI assignement procedure needed */
	else
		nl2->l2m.state = ST_L2_1;	/* TEI unassigned */
#if AS_ISDN_DEBUG_FSM
	nl2->l2m.debug = asl2_debug;
#endif

	nl2->l2m.userdata = nl2;
	nl2->l2m.userint = 0;
#if AS_ISDN_DEBUG_FSM
	nl2->l2m.printdebug = NULL;//l2m_debug;
#endif

	fsm_init_timer(&nl2->l2m, &nl2->t200);
	fsm_init_timer(&nl2->l2m, &nl2->t203);
	
	list_add_tail(&nl2->list, &isdnl2.ilist);
	err = isdnl2.ctrl(&nl2->inst, MGR_NEWENTITY | REQUEST, NULL);
	if (err)
	{
		printk(KERN_WARNING "ISDNL2-MGR : MGR_NEWENTITY REQUEST failed err(%x)\n", err);
	}
	err = isdnl2.ctrl(st, MGR_REGLAYER | INDICATION, &nl2->inst);
	if (err)
	{
		fsm_del_timer(&nl2->t200, 0);
		fsm_del_timer(&nl2->t203, 0);
		list_del(&nl2->list);
		kfree(nl2);
		nl2 = NULL;
	}
...
static int clone_l2(layer2_t *l2, as_isdn_instance_t **new_ip)
{
	int err;
	layer2_t	*nl2;
	as_isdn_if_t	*nif;
	...
	if (!new_ip)
		return(-EINVAL);
	st = (as_isdn_stack_t *)*new_ip;
	if (!st)
		return(-EINVAL);
	if (!l2->inst.down.peer)
		return(-EINVAL);

	if (!(nif = kmalloc(sizeof(as_isdn_if_t), GFP_ATOMIC)))
	{
		printk(KERN_ERR "ISDNL2-MGR : clone l2 no if mem\n");
		return(-ENOMEM);
	}
	...
	{
		kfree(nif);
		printk(KERN_ERR "ISDNL2-MGR : clone l2 failed err(%d)\n", err);
		return(err);
	}
	memset(nif, 0, sizeof(as_isdn_if_t));
	nl2->cloneif = nif;
...
/* return status of l2, called by MGR_STATUS_REQ */
static int l2_status(layer2_t *l2, status_info_l2_t *si)
{
...
MODULE_LICENSE("GPL");
#endif
MODULE_PARM(asl2_debug, "1i");
#endif

static int l2_manager(void *data, u_int prim, void *arg)
{
...
#if AS_ISDN_DEBUG_L2_CTRL
	if (asl2_debug & 0x1000)
		printk(KERN_ERR  "ISDNL2-MGR : data:%p(inst:%s) prim:%x arg:%p--%s\n", data, inst->name, prim, arg, inst->name);
#endif
...
#if AS_ISDN_DEBUG_L2_DATA
		if (asl2_debug & 0x1)
			printk(KERN_WARNING "ISDNL2-MGR : prim(%x) l2 no instance\n", prim);
#endif
		return(err);
	}
	...
	switch(prim)
	{
		...
		case MGR_ADDSTPARA | INDICATION:
			if (((AS_ISDN_stPara_t *)arg)->maxdatalen)
				l2l->maxlen = ((AS_ISDN_stPara_t *)arg)->maxdatalen;
		...
		case MGR_SETIF | INDICATION:
			return(AS_ISDN_SetIF(inst, arg, prim, l2from_up, l2from_down, l2l));
		case MGR_ADDIF | REQUEST:
			if (arg)
			{
				as_isdn_if_t *hif = arg;
				if (hif->stat & IF_UP)
				{
					hif->fdata = l2l;
		...
		default:
			if (asl2_debug & 0x1)
				printk(KERN_WARNING "ISDNL2-MGR : prim %x not handled\n", prim);
		return(-EINVAL);
	}
	return(0);
}
...
int Isdnl2_Init(void)
{
	...
	printk(KERN_ERR AS_VERSION_INFO( "L2 Driver") );
#ifdef MODULE
	isdnl2.owner = THIS_MODULE;
#endif
	isdnl2.name = "ISDNL2";
	isdnl2.DPROTO.protocol[2] = ISDN_PID_L2_LAPD |ISDN_PID_L2_LAPD_NET |ISDN_PID_L2_DF_PTP;
	...
	as_isdn_fsm_new(&l2fsm, L2FnList, L2_FN_COUNT);
	
	TEIInit();
	
	if ((err = as_isdn_register(&isdnl2)))
	{
		printk(KERN_ERR "Can't register %s error(%d)\n", isdnl2.name, err);
		as_isdn_fsm_free(&l2fsm);
	}
	return(err);
}
...
void Isdnl2_cleanup(void)
{
	...
	if ((err = as_isdn_unregister(&isdnl2)))
	{
		printk(KERN_ERR "Can't unregister ISDN layer 2 %s error(%d)\n", isdnl2.name, err);
	}
	if(!list_empty(&isdnl2.ilist))
	{
		printk(KERN_WARNING "AS_ISDNl2 l2 list not empty\n");
	...
	printk(KERN_INFO "Release " AS_VERSION_INFO( "L2 Driver") );
}
