/*
 * $Id: dsp_blowfish.c,v 1.1.1.1 2006/11/30 16:27:58 lizhijie Exp $
 * Blowfish encryption/decryption for AS_ISDN_dsp.
 */
		...
		*data++ = bf_data_out[k++];
		...
		if ((cs&0x7) != (((cs2>>5)&4) | ((cs1>>6)&2) | (cs0 >> 7)))
		{
			if (dsp_debug & DEBUG_DSP_BLOWFISH)
				printk(KERN_ERR  "DSP BLOWFISH: received corrupt frame, checksumme is not correct\n");
			continue;
		} 
		/* decrypt */
		yr ^= P[17];
