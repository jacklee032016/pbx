/*
 * $Id: dsp_tones.c,v 1.1.1.1 2006/11/30 16:27:58 lizhijie Exp $ 
 */
	...
	while(len) {
		...
		while(42) {
			...
			if (dsp_debug & DEBUG_DSP_TONE)
				printk(KERN_ERR  "%s: reaching next sequence (index=%d)\n", __FUNCTION__, index);
			count -= pat->seq[index];
			index++;
		}
...
dsp_tone(dsp_t *dsp, int tone)
{
	...
	if (dsp_debug & DEBUG_DSP_TONE)
		printk(KERN_ERR  "%s: now starting tone %d (index=%d)\n", __FUNCTION__, tone, 0);
	tonet->tone = tone;
	tonet->pattern = pat;
	tonet->index = 0;
	tonet->count = 0;
