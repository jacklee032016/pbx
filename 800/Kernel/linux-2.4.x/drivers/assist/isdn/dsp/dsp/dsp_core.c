/* $Id: dsp_core.c,v 1.1.1.1 2006/11/30 16:27:58 lizhijie Exp $
 * This module does:
...
as_isdn_object_t dsp_obj;
AS_ISDN_HWlock_t dsp_lock;
 ...
 */
static int
dsp_control_req(dsp_t *dsp, as_isdn_head_t *hh, struct sk_buff *skb)
{
	struct		sk_buff *nskb;
	int ret = 0;
	int cont;
	...
	switch (cont) {
		...
		case CMX_CONF_JOIN: /* join / update conference */
			...
			if (*((u32 *)data) == 0)
				goto conf_split;
			if (dsp_debug & DEBUG_DSP_CORE)
				printk(KERN_ERR  "%s: join conference %d\n", __FUNCTION__, *((u32 *)data));
		...
		case CMX_CONF_SPLIT: /* remove from conference */
			conf_split:
			if (dsp_debug & DEBUG_DSP_CORE)
				printk(KERN_ERR  "%s: release conference\n", __FUNCTION__);
		...
		case TONE_PATT_ON: /* play tone */
			...
			if (dsp_debug & DEBUG_DSP_CORE)
				printk(KERN_ERR  "%s: turn tone 0x%x on\n", __FUNCTION__, *((int *)skb->data));
		...
		case TONE_PATT_OFF: /* stop tone */
			if (dsp_debug & DEBUG_DSP_CORE)
				printk(KERN_ERR  "%s: turn tone off\n", __FUNCTION__);
			dsp_tone(dsp, 0);
			dsp_cmx_hardware(dsp->conf, dsp);
		...
		case VOL_CHANGE_TX: /* change volume */
			...
			if (dsp_debug & DEBUG_DSP_CORE)
				printk(KERN_ERR  "%s: change tx volume to %d\n", __FUNCTION__, dsp->tx_volume);
			dsp_cmx_hardware(dsp->conf, dsp);
			break;
		case VOL_CHANGE_RX: /* change volume */
			...
			if (dsp_debug & DEBUG_DSP_CORE)
				printk(KERN_ERR  "%s: change rx volume to %d\n", __FUNCTION__, dsp->tx_volume);
			dsp_cmx_hardware(dsp->conf, dsp);
			break;
		case CMX_ECHO_ON: /* enable echo */
			dsp->echo = 1; /* soft echo */
			if (dsp_debug & DEBUG_DSP_CORE)
				printk(KERN_ERR  "%s: enable cmx-echo\n", __FUNCTION__);
			dsp_cmx_hardware(dsp->conf, dsp);
			if (dsp_debug & DEBUG_DSP_CMX)
				dsp_cmx_debug(dsp);
			break;
		case CMX_ECHO_OFF: /* disable echo */
			dsp->echo = 0;
			if (dsp_debug & DEBUG_DSP_CORE)
				printk(KERN_ERR  "%s: disable cmx-echo\n", __FUNCTION__);
			dsp_cmx_hardware(dsp->conf, dsp);
			if (dsp_debug & DEBUG_DSP_CMX)
				dsp_cmx_debug(dsp);
			break;
		case CMX_RECEIVE_ON: /* enable receive to user space */
			if (dsp_debug & DEBUG_DSP_CORE)
				printk(KERN_ERR  "%s: enable receive to user space\n", __FUNCTION__);
			dsp->rx_disabled = 0;
			dsp_cmx_hardware(dsp->conf, dsp);
			break;
		case CMX_RECEIVE_OFF: /* disable receive to user space */
			if (dsp_debug & DEBUG_DSP_CORE)
				printk(KERN_ERR  "%s: disable receive to user space\n", __FUNCTION__);
			dsp->rx_disabled = 1;
			dsp_cmx_hardware(dsp->conf, dsp);
			break;
		case CMX_MIX_ON: /* enable mixing of transmit data with conference members */
			if (dsp_debug & DEBUG_DSP_CORE)
				printk(KERN_ERR  "%s: enable mixing of tx-data with conf mebers\n", __FUNCTION__);
			dsp->tx_mix = 1;
			dsp_cmx_hardware(dsp->conf, dsp);
			if (dsp_debug & DEBUG_DSP_CMX)
				dsp_cmx_debug(dsp);
		...
		case CMX_MIX_OFF: /* disable mixing of transmit data with conference members */
			if (dsp_debug & DEBUG_DSP_CORE)
				printk(KERN_ERR  "%s: disable mixing of tx-data with conf mebers\n", __FUNCTION__);
			dsp->tx_mix = 0;
			dsp_cmx_hardware(dsp->conf, dsp);
			if (dsp_debug & DEBUG_DSP_CMX)
				dsp_cmx_debug(dsp);
		...
		case BF_ENABLE_KEY: /* turn blowfish on */
			...
			if (dsp_debug & DEBUG_DSP_CORE)
				printk(KERN_ERR  "%s: turn blowfish on (key not shown)\n", __FUNCTION__);
		...
		case BF_DISABLE: /* turn blowfish off */
			if (dsp_debug & DEBUG_DSP_CORE)
				printk(KERN_ERR  "%s: turn blowfish off\n", __FUNCTION__);
			dsp_bf_cleanup(dsp);
			dsp_cmx_hardware(dsp->conf, dsp);
			break;
		default:
			if (dsp_debug & DEBUG_DSP_CORE)
				printk(KERN_ERR  "%s: ctrl req %x unhandled\n", __FUNCTION__, cont);
			ret = -EINVAL;
	}
	return(ret);
}
 ...
 */
static int
dsp_from_up(as_isdn_if_t *hif, struct sk_buff *skb)
{
	dsp_t			*dsp;
	as_isdn_head_t		*hh;
	int			ret = 0;
	...
	switch(hh->prim) {
		...
		case PH_ACTIVATE | REQUEST:
			if (dsp_debug & DEBUG_DSP_CORE)
				printk(KERN_ERR  "%s: activating b_channel %s\n", __FUNCTION__, dsp->inst.name);
			lock_HW(&dsp_lock, 0);
			dsp->tx_pending = 0;
		...
		case PH_DEACTIVATE | REQUEST:
			if (dsp_debug & DEBUG_DSP_CORE)
				printk(KERN_ERR  "%s: releasing b_channel %s\n", __FUNCTION__, dsp->inst.name);
			lock_HW(&dsp_lock, 0);
			dsp->tx_pending = 0;
		...
		default:
			if (dsp_debug & DEBUG_DSP_CORE)
				printk(KERN_ERR  "%s: msg %x unhandled %s\n", __FUNCTION__, hh->prim, dsp->inst.name);
			ret = -EINVAL;
			break;
	}
	if (!ret)
 ...
 */
static int
dsp_from_down(as_isdn_if_t *hif,  struct sk_buff *skb)
{
	dsp_t		*dsp;
	as_isdn_head_t	*hh;
	int		ret = 0;
	...
	switch(hh->prim)
	{
			...
			if (dsp->dtmf.software) {
				...
				if (digits) while(*digits) {
					if (dsp_debug & DEBUG_DSP_DTMF)
						printk(KERN_ERR  "%s: sending software decoded digit(%c) to upper layer %s\n", __FUNCTION__, *digits, dsp->inst.name);
		...
		case PH_CONTROL | INDICATION:
			...
			if (dsp_debug & DEBUG_DSP_DTMFCOEFF)
				printk(KERN_ERR  "%s: PH_CONTROL received: %x (len %d) %s\n", __FUNCTION__, hh->dinfo, skb->len, dsp->inst.name);
			switch (hh->dinfo) {
				case HW_HFC_COEFF: /* getting coefficients */
				if (!dsp->dtmf.hardware) {
					if (dsp_debug & DEBUG_DSP_DTMFCOEFF)
						printk(KERN_ERR  "%s: ignoring DTMF coefficients from HFC\n", __FUNCTION__);
					break;
				}
				if (dsp->inst.up.func) {
					...
					if (digits) while(*digits) {
						...
						struct sk_buff *nskb;
						if (dsp_debug & DEBUG_DSP_DTMF)
							printk(KERN_ERR  "%s: now sending software decoded digit(%c) to upper layer %s\n", __FUNCTION__, *digits, dsp->inst.name);
				...
				default:
				if (dsp_debug & DEBUG_DSP_CORE)
					printk(KERN_ERR  "%s: ctrl ind %x unhandled %s\n", __FUNCTION__, hh->dinfo, dsp->inst.name);
				ret = -EINVAL;
			}
			unlock_HW(&dsp_lock);
			break;
		case PH_ACTIVATE | CONFIRM:
			lock_HW(&dsp_lock, 0);
			if (dsp_debug & DEBUG_DSP_CORE)
				printk(KERN_ERR  "%s: b_channel is now active %s\n", __FUNCTION__, dsp->inst.name);
			/* bchannel now active */
			dsp->b_active = 1;
...
//#endif
			if (dsp_debug & DEBUG_DSP_CORE)
				printk(KERN_ERR  "%s: done with activation, sending confirm to user space. %s\n", __FUNCTION__, dsp->inst.name);
		...
		case PH_DEACTIVATE | CONFIRM:
			lock_HW(&dsp_lock, 0);
			if (dsp_debug & DEBUG_DSP_CORE)
				printk(KERN_ERR  "%s: b_channel is now inactive %s\n", __FUNCTION__, dsp->inst.name);
...
{
	as_isdn_instance_t	*inst = &dsp->inst;
	conference_t *conf;
	...
	if (dsp_debug & DEBUG_DSP_MGR)
		printk(KERN_ERR  "%s: removing conferences %s\n", __FUNCTION__, dsp->inst.name);
	conf = dsp->conf;
	if (conf) {
	...
	if (dsp_debug & DEBUG_DSP_MGR)
		printk(KERN_ERR  "%s: disconnecting instances %s\n", __FUNCTION__, dsp->inst.name);
	if (inst->up.peer) {
	...
	if (dsp_debug & DEBUG_DSP_MGR)
		printk(KERN_ERR  "%s: remove & destroy object %s\n", __FUNCTION__, dsp->inst.name);
	list_del(&dsp->list);
	...
	if (dsp_debug & DEBUG_DSP_MGR)
		printk(KERN_ERR  "%s: dsp instance released\n", __FUNCTION__);
}
 ...
 */
static int
new_dsp(as_isdn_stack_t *st, AS_ISDN_pid_t *pid) 
{
	int err = 0;
	dsp_t *ndsp;

	if (dsp_debug & DEBUG_DSP_MGR)
		printk(KERN_ERR  "%s: creating new dsp instance\n", __FUNCTION__);
	...
	memcpy(&ndsp->inst.pid, pid, sizeof(AS_ISDN_pid_t));
	ndsp->inst.obj = &dsp_obj;
	ndsp->inst.data = ndsp;
	if (!AS_ISDN_SetHandledPID(&dsp_obj, &ndsp->inst.pid)) {
		int_error();
		err = -ENOPROTOOPT;
		free_mem:
		vfree(ndsp);
...
//#endif
	...
	if (dsp_debug & DEBUG_DSP_MGR)
		printk(KERN_ERR  "%s: dsp instance created %s\n", __FUNCTION__, ndsp->inst.name);
	unlock_HW(&dsp_lock);
	return(err);
}
	...
	if (!(dsp_options & DSP_OPT_NOHARDWARE)) {
		...
		if (nskb) {
			if (dsp->inst.down.func(&dsp->inst.down, nskb)) {
				dev_kfree_skb(nskb);
				if (dsp_debug & DEBUG_DSP_MGR)
					printk(KERN_ERR  "%s: no features supported by %s\n", __FUNCTION__, dsp->inst.name);
			} else {
				if (dsp_debug & DEBUG_DSP_MGR)
					printk(KERN_ERR  "%s: features of %s: hfc_id=%d hfc_dtmf=%d hfc_loops=%d pcm_id=%d pcm_slots=%d pcm_banks=%d\n",
					 __FUNCTION__, dsp->inst.name,
					 dsp->features.hfc_id,
...
dsp_manager(void *data, u_int prim, void *arg) {
	...
	switch(prim) {
		...
		ret = AS_ISDN_ConnectIF(inst, arg);
		dsp_feat(dspl);
		break;
	    case MGR_SETIF | REQUEST:
	    case MGR_SETIF | INDICATION:
		ret = AS_ISDN_SetIF(inst, arg, prim, dsp_from_up, dsp_from_down, dspl);
		break;
	    case MGR_DISCONNECT | REQUEST:
	    case MGR_DISCONNECT | INDICATION:
		ret = AS_ISDN_DisConnectIF(inst, arg);
		break;
	    case MGR_UNREGLAYER | REQUEST:
	    case MGR_RELEASE | INDICATION:
		if (dsp_debug & DEBUG_DSP_MGR)
			printk(KERN_ERR  "%s: release_dsp id %x\n", __FUNCTION__, dspl->inst.st->id);
...
static int dsp_init(void)
{
	...
	/* display revision */
	printk(KERN_INFO AS_VERSION_INFO("Audio DSP") );
	...
	/* register object */
	if ((err = as_isdn_register(&dsp_obj))) {
		printk(KERN_ERR "AS_ISDN_dsp: Can't register %s error(%d)\n", DSPName, err);
		return(err);
	}
...
static void dsp_cleanup(void)
{
	...
	if (dsp_debug & DEBUG_DSP_MGR)
		printk(KERN_ERR  "%s: removing module\n", __FUNCTION__);

	if ((err = as_isdn_unregister(&dsp_obj))) {
		printk(KERN_ERR "AS_ISDN_dsp: Can't unregister Audio DSP error(%d)\n", 
			err);
	}
	if (!list_empty(&dsp_obj.ilist)) {
		printk(KERN_WARNING "AS_ISDN_dsp: Audio DSP object inst list not empty.\n");
		list_for_each_entry_safe(dspl, nd, &dsp_obj.ilist, list)
			release_dsp(dspl);
	}
	if (!list_empty(&Conf_list)) {
		printk(KERN_ERR "AS_ISDN_dsp: Conference list not empty. Not all memory freed.\n");
	}
}
