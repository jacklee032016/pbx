/*
 * $Id: dsp_cmx.c,v 1.1.1.1 2006/11/30 16:27:58 lizhijie Exp $
...
extern AS_ISDN_HWlock_t dsp_lock;
LIST_HEAD(Conf_list);
...
dsp_cmx_debug(dsp_t *dsp)
{
	...
	printk(KERN_ERR  "-----Current DSP\n");
	list_for_each_entry(odsp, &dsp_obj.ilist, list)
	{
		printk(KERN_ERR  "* %s echo=%d txmix=%d", odsp->inst.name, odsp->echo, odsp->tx_mix);
		if (odsp->conf)
...
//printk("-----3\n");
		if (dsp_debug & DEBUG_DSP_CMX)
			printk(KERN_ERR  "%s checking dsp %s\n", __FUNCTION__, dsp->inst.name);
//printk("-----a\n");
		one_member:
		/* remove HFC conference if enabled */
		if (dsp->hfc_conf >= 0) {
			if (dsp_debug & DEBUG_DSP_CMX)
				printk(KERN_ERR  "%s removing %s from HFC conf %d because dsp is split\n", __FUNCTION__, dsp->inst.name, dsp->hfc_conf);
		...
		if (!dsp->echo) {
			...
			if (dsp->pcm_slot_tx>=0 || dsp->pcm_slot_rx>=0) {
				if (dsp_debug & DEBUG_DSP_CMX)
					printk(KERN_ERR  "%s removing %s from PCM slot %d (TX) %d (RX) because dsp is split (no echo)\n", __FUNCTION__, dsp->inst.name, dsp->pcm_slot_tx, dsp->pcm_slot_rx);
		...
		if (dsp->pcm_slot_tx>=0) {
			...
			dsp->pcm_bank_rx = 2;
			if (dsp_debug & DEBUG_DSP_CMX)
				printk(KERN_ERR  "%s refresh %s for echo using slot %d\n", __FUNCTION__, dsp->inst.name, dsp->pcm_slot_tx);
		...
		if (i == ii) {
			if (dsp_debug & DEBUG_DSP_CMX)
				printk(KERN_ERR  "%s no slot available for echo\n", __FUNCTION__);
		...
		dsp->pcm_bank_rx = 2;
		if (dsp_debug & DEBUG_DSP_CMX)
			printk(KERN_ERR  "%s assign echo for %s using slot %d\n", __FUNCTION__, dsp->inst.name, dsp->pcm_slot_tx);
		dsp_cmx_hw_message(dsp, HW_PCM_CONN, dsp->pcm_slot_tx, 2, dsp->pcm_slot_rx, 2);
		return;
	}
...
//printk("-----4\n");
	...
	if (dsp_debug & DEBUG_DSP_CMX)
		printk(KERN_ERR  "%s checking conference %d\n", __FUNCTION__, conf->id);
//printk("-----5\n");
	...
	list_for_each_entry(member, &conf->mlist, list) {
		...
		if (member->dsp->tx_mix) {
			if (dsp_debug & DEBUG_DSP_CMX)
				printk(KERN_ERR  "%s dsp %s cannot form a conf, because tx_mix is turned on\n", __FUNCTION__, member->dsp->inst.name);
			conf_software:
			list_for_each_entry(member, &conf->mlist, list) {
				dsp = member->dsp;
				/* remove HFC conference if enabled */
				if (dsp->hfc_conf >= 0) {
					if (dsp_debug & DEBUG_DSP_CMX)
						printk(KERN_ERR  "%s removing %s from HFC conf %d because not possible with hardware\n", __FUNCTION__, dsp->inst.name, dsp->hfc_conf);
					dsp_cmx_hw_message(dsp, HW_CONF_SPLIT, 0, 0, 0, 0);
					dsp->hfc_conf = -1;
				}
				...
				if (dsp->pcm_slot_tx>=0 || dsp->pcm_slot_rx>=0) {
					if (dsp_debug & DEBUG_DSP_CMX)
						printk(KERN_ERR  "%s removing %s from PCM slot %d (TX) slot %d (RX) because not possible with hardware\n", __FUNCTION__, dsp->inst.name, dsp->pcm_slot_tx, dsp->pcm_slot_rx);
					dsp_cmx_hw_message(dsp, HW_PCM_DISC, 0, 0, 0, 0);
					dsp->pcm_slot_tx = -1;
					dsp->pcm_bank_tx = -1;
					dsp->pcm_slot_rx = -1;
		...
		if (member->dsp->echo) {
			if (dsp_debug & DEBUG_DSP_CMX)
				printk(KERN_ERR  "%s dsp %s cannot form a conf, because echo is turned on\n", __FUNCTION__, member->dsp->inst.name);
			goto conf_software;
		}
		...
		if (member->dsp->tx_volume) {
			if (dsp_debug & DEBUG_DSP_CMX)
				printk(KERN_ERR  "%s dsp %s cannot form a conf, because tx_volume is changed\n", __FUNCTION__, member->dsp->inst.name);
			goto conf_software;
		}
		if (member->dsp->rx_volume) {
			if (dsp_debug & DEBUG_DSP_CMX)
				printk(KERN_ERR  "%s dsp %s cannot form a conf, because rx_volume is changed\n", __FUNCTION__, member->dsp->inst.name);
			goto conf_software;
		}
		/* check if encryption is enabled */
		if (member->dsp->bf_enable) {
			if (dsp_debug & DEBUG_DSP_CMX)
				printk(KERN_ERR  "%s dsp %s cannot form a conf, because encryption is enabled\n", __FUNCTION__, member->dsp->inst.name);
			goto conf_software;
		}
		/* check if member is on a card with PCM support */
		if (member->dsp->features.pcm_id < 0) {
			if (dsp_debug & DEBUG_DSP_CMX)
				printk(KERN_ERR  "%s dsp %s cannot form a conf, because dsp has no PCM bus\n", __FUNCTION__, member->dsp->inst.name);
			goto conf_software;
		}
		/* check if relations are on the same PCM bus */
		if (member->dsp->features.pcm_id != same_pcm) {
			if (dsp_debug & DEBUG_DSP_CMX)
				printk(KERN_ERR  "%s dsp %s cannot form a conf, because dsp is on a different PCM bus than the first dsp\n",
					__FUNCTION__, member->dsp->inst.name);
			goto conf_software;
		}
	...
	if (memb == 1) {
		if (dsp_debug & DEBUG_DSP_CMX)
			printk(KERN_ERR  "%s conf %d cannot form a HW conference, because dsp is alone\n", __FUNCTION__, conf->id);
		conf->hardware = 0;
		conf->software = 1;
		member = list_entry(conf->mlist.next, conf_member_t, list);
		dsp = member->dsp;
	...
	if (memb == 2) {
		...
		if (member->dsp->hfc_conf >= 0) {
			if (dsp_debug & DEBUG_DSP_CMX)
				printk(KERN_ERR  "%s removing %s from HFC conf %d because two parties require only a PCM slot\n", __FUNCTION__, member->dsp->inst.name, member->dsp->hfc_conf);
		 ...
		 && member->dsp->features.hfc_id!=nextm->dsp->features.hfc_id) {
			...
			if (member->dsp->pcm_slot_tx>=0
			 ...
			 && nextm->dsp->pcm_bank_tx!=nextm->dsp->pcm_bank_rx) {
				...
				if (dsp_debug & DEBUG_DSP_CMX)
					printk(KERN_ERR  "%s dsp %s & %s stay joined on PCM slot %d bank %d (TX) bank %d (RX) (on different chips)\n", __FUNCTION__,
			...
			if (i == ii) {
				if (dsp_debug & DEBUG_DSP_CMX)
					printk(KERN_ERR  "%s no slot available for %s & %s\n", __FUNCTION__,
						member->dsp->inst.name, nextm->dsp->inst.name);
				/* no more slots available */
				goto conf_software;
			}
			...
			if (dsp_debug & DEBUG_DSP_CMX)
				printk(KERN_ERR  "%s adding %s & %s to new PCM slot %d (TX and RX on different chips) because both members have not same slots\n", __FUNCTION__,
		...
		/* if members have one bank (or on the same chip) */
			...
			if (member->dsp->pcm_slot_tx>=0
			 ...
			 && nextm->dsp->pcm_bank_rx==0) {
				/* all members have same slot */
				if (dsp_debug & DEBUG_DSP_CMX)
					printk(KERN_ERR  "%s dsp %s & %s stay joined on PCM slot %d (TX) %d (RX) on same chip or one bank PCM)\n", __FUNCTION__,
			...
			nextm->dsp->pcm_bank_tx = 0;
			if (dsp_debug & DEBUG_DSP_CMX)
				printk(KERN_ERR  "%s adding %s & %s to new PCM slot %d (TX) %d (RX) on same chip or one bank PCM, because both members have not crossed slots\n", __FUNCTION__,
					member->dsp->inst.name, nextm->dsp->inst.name, member->dsp->pcm_slot_tx,
	...
	if (same_hfc < 0) {
		if (dsp_debug & DEBUG_DSP_CMX)
			printk(KERN_ERR  "%s conference %d cannot be formed, because members are on different chips or not on HFC chip\n",
				__FUNCTION__, conf->id);
		goto conf_software;
	}
	...
	if (current_conf >= 0) {
		...
		list_for_each_entry(member, &conf->mlist, list) {
			...
			if (i == ii) {
				/* no more slots available */
				if (dsp_debug & DEBUG_DSP_CMX)
					printk(KERN_ERR  "%s conference %d cannot be formed, because no slot free\n", __FUNCTION__, conf->id);
				goto conf_software;
			}
			if (dsp_debug & DEBUG_DSP_CMX)
				printk(KERN_ERR  "%s changing dsp %s to HW conference %d slot %d\n", __FUNCTION__, member->dsp->inst.name, current_conf, i);
	...
	if (i == ii) {
		...
		if (dsp_debug & DEBUG_DSP_CMX)
			printk(KERN_ERR  "%s conference %d cannot be formed, because no conference number free\n", __FUNCTION__, conf->id);
		goto conf_software;
	}
	/* join all members */
	current_conf = i;
...
dsp_cmx_conf(dsp_t *dsp, u32 conf_id)
{
	...
	if (dsp->conf_id) {
		if (dsp_debug & DEBUG_DSP_CMX)
			printk(KERN_ERR  "removing us from conference %d\n",
				dsp->conf->id);
		/* remove us from conf */
		conf = dsp->conf;
		...
		if (list_empty(&conf->mlist)) {
			if (dsp_debug & DEBUG_DSP_CMX)
				printk(KERN_ERR  "conference is empty, so we remove it.\n");
	...
	/* now add us to conf */
	if (dsp_debug & DEBUG_DSP_CMX)
		printk(KERN_ERR  "searching conference %d\n",
			conf_id);
	conf = dsp_cmx_search_conf(conf_id);
	if (!conf) {
		if (dsp_debug & DEBUG_DSP_CMX)
			printk(KERN_ERR  "conference doesn't exist yet, creating.\n");
	...
	if (list_empty(&conf->mlist)) {
		if (dsp_debug & DEBUG_DSP_CMX)
			printk(KERN_ERR  "we are alone in this conference, so exit.\n");
		/* update hardware */
		dsp_cmx_hardware(NULL, dsp);
		return(0);
	}
...
#ifdef CMX_DEBUG
	printk(KERN_ERR  "cmx_receive(dsp=%lx): W_rx(dsp)=%05x W_min=%05x W_max=%05x largest=%05x %s\n", dsp, dsp->W_rx, W_min, W_max, dsp->largest, dsp->inst.name);
#endif
...
#warning CMX_CONF_DEBUG is enabled, it causes performance loss with normal 2-party crossconnects
			...
			if (3 <= count_list_member(&conf->mlist)) {
#endif
//printk(KERN_ERR  "cmxing dsp:%s dsp->W_rx=%04x conf->W_max=%04x\n", dsp->inst.name, dsp->W_rx, conf->W_max);
		...
		if (dsp_debug & DEBUG_DSP_CMX)
			printk(KERN_ERR  "CMX: receiving too fast (rx_buff) dsp=%x\n", (u32)dsp);
#ifdef CMX_DEBUG
		printk(KERN_ERR  "W_max=%x-W_min=%x = %d, largest = %d\n", W_max, W_min, (W_max - W_min) & CMX_BUFF_MASK, dsp->largest);
#endif
	}
}
...
*dsp_cmx_send(dsp_t *dsp, int len, int dinfo)
{
	...
	if (!nskb) {
		printk(KERN_ERR "FATAL ERROR in AS_ISDN_dsp.o: cannot alloc %d bytes\n", len);
		return(NULL);
	}
	AS_ISDN_sethead(PH_DATA | REQUEST, dinfo, nskb);
...
#ifdef CMX_DEBUG
	printk(KERN_ERR  "CMX_SEND(dsp=%lx) %d bytes from tx:0x%05x-0x%05x rx:0x%05x-0x%05x echo=%d %s\n", dsp, len, t, tt, r, rr, dsp->echo, dsp->inst.name);
#endif
...
dsp_cmx_transmit(dsp_t *dsp, struct sk_buff *skb)
{
	...
	if (dsp->tx_delay_count >= DELAY_CHECK) {
		...
		if (dsp_debug & DEBUG_DSP_DELAY)
			printk(KERN_ERR  "%s(dsp=0x%x) removing delay of %d bytes\n", __FUNCTION__, (u32)dsp, dsp->tx_delay);
...
#ifdef CMX_DEBUG
	printk(KERN_ERR  "cmx_transmit(dsp=%lx) %d bytes to 0x%x-0x%x. %s\n", dsp, (ww-w)&CMX_BUFF_MASK, w, ww, dsp->inst.name);
#endif
