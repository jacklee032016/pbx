/*
 * $Id: dtmf.c,v 1.1.1.1 2006/11/30 16:27:58 lizhijie Exp $
...
typedef struct _dtmf
{
	...
	char				last;
	int				idx;
	int				buf[DTMF_NPOINTS];	/* int : 4 bytes */
...
static as_isdn_object_t dtmf_obj;
...
static inline void isdn_audio_tlookup(const u_char *table, u_char *buff, unsigned long n)
{
#ifdef __i386__
...
static void isdn_audio_goertzel(dtmf_t *dtmf)
{
	...
	for (k = 0; k < NCOEFF; k++)
	{
		...
		if (sk[k] < -32768 || sk[k] > 32767)
			printk(KERN_ERR  "dtmf goertzel overflow, sk[%d]=%d\n", k, sk[k]);
		if (sk2[k] < -32768 || sk2[k] > 32767)
			printk(KERN_ERR  "isdn_audio: dtmf goertzel overflow, sk2[%d]=%d\n", k, sk2[k]);
	...
	if (dtmf->debug & DEBUG_DTMF_KOEFF)
		printk(KERN_ERR  "DTMF koeff(%d,%d,%d,%d,%d,%d,%d,%d) range(%d-%d)\n",
	...
	if (dtmf->debug & DEBUG_DTMF_DETECT)
		printk(KERN_ERR  "DTMF: last(%c) what(%c)\n",
			dtmf->last, what);
	if ((what != dtmf->last) && (what != ' ') && (what != '.')) {
		if (dtmf->debug & DEBUG_DTMF_TONE)
			printk(KERN_ERR  "DTMF: tone='%c'\n", what);
 ...
 */
static void isdn_audio_calc_dtmf(dtmf_t *dtmf, struct sk_buff *skb)
{
	int len = skb->len;
	u_char	*p = skb->data;
	int i;
...
static void dtmf_reset(dtmf_t *dtmf)
{
	dtmf->last = ' ';
	dtmf->idx = 0;
}

/* upper function interface of a instance : process data from upper layer instance
* eg. command start/stop/reset dtmf etc. 
*/
static int dtmf_from_up(as_isdn_if_t *hif, struct sk_buff *skb)
{
	dtmf_t		*dtmf;
	as_isdn_head_t	*hh;
	int		*data;
		...
		case (PH_CONTROL | REQUEST):
			if ((hh->dinfo == 0) && (skb->len >= sizeof(int)))
			{
				...
				if (dtmf->debug & DEBUG_DTMF_CTRL)
					printk(KERN_ERR  "DTMF: PH_CONTROL REQ data %04x\n",	*data);
			...
			/* Fall through in case of not handled function */
		default:
...
/* down function for interface of a instance: process data from low layer instance
* eg. calculate DTMF phone number
*/
static int dtmf_from_down(as_isdn_if_t *hif,  struct sk_buff *skb)
{
	dtmf_t		*dtmf;
	as_isdn_head_t	*hh;
...
static void release_dtmf(dtmf_t *dtmf)
{
	as_isdn_instance_t	*inst = &dtmf->inst;
...
/* client DTMF request : used DTMF in layer represented by 'pid' in stack */
static int new_dtmf(as_isdn_stack_t *st, AS_ISDN_pid_t *pid)
{
	dtmf_t *n_dtmf;
	int err;
	...
	memcpy(&n_dtmf->inst.pid, pid, sizeof(AS_ISDN_pid_t));
	AS_ISDN_init_instance(&n_dtmf->inst, &dtmf_obj, n_dtmf);

	/* check dtmf_object can be used with this 'pid'(layer + ChannelType+protocol )  */
	if (!AS_ISDN_SetHandledPID(&dtmf_obj, &n_dtmf->inst.pid))
	{
		int_error();
		kfree(n_dtmf);
		return(-ENOPROTOOPT);
	...
	n_dtmf->debug = debug;
	/* added dtmf into dtfm_object */
	list_add_tail(&n_dtmf->list, &dtmf_obj.ilist);

	/* central_manager : create a layer object in stack */
...
#if 0
static int dtmf_status(dtmf_t *dtmf, status_info_dtmf_t *si)
{
...
static int dtmf_manager(void *data, u_int prim, void *arg)
{
	...
	if (debug & DEBUG_DTMF_MGR)
		printk(KERN_ERR  "dtmf_manager data:%p prim:%x arg:%p\n", data, prim, arg);
	if (!data)
		return(ret);
	...
	switch(prim)
	{
		...
		case MGR_SETIF | INDICATION:
			/* instance which contains dtmf_object is used as peer */
			return(AS_ISDN_SetIF(inst, arg, prim, dtmf_from_up, dtmf_from_down, dtmf_l));
		...
		case MGR_RELEASE | INDICATION:
			if (debug & DEBUG_DTMF_MGR)
				printk(KERN_ERR  "release_dtmf id %x\n", dtmf_l->inst.st->id);
			release_dtmf(dtmf_l);
			break;
...
/* only used in B channel, layer 2*/
static int dtmf_init(void)
{
	int err;

	printk(KERN_INFO AS_VERSION_INFO("DTMF module") );
#ifdef MODULE
	...
	if ((err = as_isdn_register(&dtmf_obj)))
	{
...
static void dtmf_cleanup(void)
{
	...
	if ((err = as_isdn_unregister(&dtmf_obj)))
	{
