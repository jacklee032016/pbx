/*
 * $Id: core.c,v 1.1.1.1 2006/11/30 16:27:58 lizhijie Exp $
 */
...
static int AS_ISDNd(void *data)
{
	...
	for (;;) 
	{
		...
		if (test_and_clear_bit(AS_ISDN_TFLAGS_TEST, &hkt->Flags))
			printk(KERN_DEBUG  "kISDNd: test event done\n");
	}
...
as_isdn_object_t *get_object(int id)
{
...
static as_isdn_object_t *find_object(int protocol)
{
	as_isdn_object_t	*obj;
	int		err;
...
#if  __WITH_KERNEL_2_6_		
/* meanful in 2.6 */
static as_isdn_object_t *find_object_module(int protocol)
{
#ifdef CONFIG_KMOD
	int		err;
#endif
...
static void remove_object(as_isdn_object_t *obj)
{
	as_isdn_stack_t *st, *nst;
...
static int dummy_if(as_isdn_if_t *hif, struct sk_buff *skb)
{
	if (!skb)
	{
...
as_isdn_instance_t *get_next_instance(as_isdn_stack_t *st, AS_ISDN_pid_t *pid)
{
	int		err;
	as_isdn_instance_t	*next;
	int		layer, proto;
	as_isdn_object_t	*obj;
...
static int sel_channel(as_isdn_stack_t *st, channel_info_t *ci)
{
	int	err = -EINVAL;
...
static int disconnect_if(as_isdn_instance_t *inst, u_int prim, as_isdn_if_t *hif)
{
	int	err = 0;
...
static int add_if(as_isdn_instance_t *inst, u_int prim, as_isdn_if_t *hif)
{
	as_isdn_if_t *myif;
...
static int debugout(as_isdn_instance_t *inst, logdata_t *log)
{
	char *p = tmpbuf;
...
static int get_hdevice(as_isdn_device_t **dev, int *typ)
{
	if (!dev)
		return(-EINVAL);
	...
	{
		printk(KERN_ERR "RAW DEVICE is not provide in protocol stack\n");
#if 0		
...
static int set_stack_req(as_isdn_stack_t *st, AS_ISDN_pid_t *pid)
{
	struct sk_buff	*skb;
	AS_ISDN_headext_t	*hhe;
	AS_ISDN_pid_t	*npid;
...
static int new_entity(as_isdn_instance_t *inst)
{
	int	entity;
	int	ret;
...
int as_isdn_register(as_isdn_object_t *obj)
{
	u_long	flags;
...
int as_isdn_unregister(as_isdn_object_t *obj)
{
	u_long	flags;
...
int  as_isdn_init(void)
{
	...
	printk(KERN_ERR AS_VERSION_INFO("Core Engine") );
