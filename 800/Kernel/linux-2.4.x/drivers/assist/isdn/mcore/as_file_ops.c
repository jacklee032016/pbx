/*
* $Id: as_file_ops.c,v 1.1.1.1 2006/11/30 16:27:58 lizhijie Exp $
*/
...
static  __inline__ ssize_t  as_rawdev_do_write(struct file *file, const char *buf, size_t count, loff_t * off)
{
	...
	if(count > RAWDEV_PENDING_SKB_SIZE)
		count = RAWDEV_PENDING_SKB_SIZE;

	skb = dev->pending_tx_skb;
	if(skb == NULL)
	{
		printk(KERN_ERR "pending skb is null\n");
		return (-ENOMEM);
	}

	if( (dev->currentPosition +count) < RAWDEV_PENDING_SKB_SIZE)
	{
		/* skb_put : return data pointer of skbuff and change skb->len at the same time */
		if (copy_from_user(skb_put(skb, count), buf, count))
		{
//			dev_kfree_skb(skb);
			return(-EFAULT);
		}
		dev->currentPosition += count;
		return count;
	}

	dev->pending_tx_skb = dev_alloc_skb(RAWDEV_PENDING_SKB_SIZE +PORT_SKB_RESERVE );
	if(!dev->pending_tx_skb )
	{
		dev->pending_tx_skb = skb;
		return (-ENOMEM);
	}	
	
	if( copy_from_user( skb_put(dev->pending_tx_skb, count), buf, count ) )
	{
//		dev_kfree_skb(dev->pending_tx_skb);
		return (-EFAULT);
	}
	dev->currentPosition = count;
	...
	if(skb_queue_len(&dev->wport.queue) >= dev->wport.maxqlen)
	{
			...
			printk(KERN_WARNING "write port is full for device %d\n", dev->minor);
...
static __inline__ ssize_t _as_isdn_do_raw_read(struct file *file, char *buf, size_t count, loff_t * off)
{
	as_isdn_rawdev_t	*dev = file->private_data;
	size_t			len =0;
	size_t			copyLength = 0;
	u_long			flags;
	struct sk_buff	*skb = dev->last_skb;
	...
	if(count < BCHANNEL_READ_LENGTH )
	{
		printk(KERN_ERR "BChannel buffer size %d is too small, it should be %d\n", count, BCHANNEL_READ_LENGTH);
		return (-ENOSPC);
	}
	...
	while (skb_queue_empty(&dev->rport.queue))
	{
		...
		if (file->f_flags & O_NONBLOCK)
		{
			printk(KERN_DEBUG " no read buffer can be read now\n");
			return(-EAGAIN);
		}	
	...
	if( skb)
	{
		copyLength = BCHANNEL_READ_LENGTH> skb->len ? skb->len: BCHANNEL_READ_LENGTH-len;
		if (copy_to_user(buf, skb->data, copyLength) )
		{
			return(-EFAULT);
		}
		len += copyLength;
		...
		*off += copyLength;
		...
		if(copyLength < skb->len)
		{
			skb_pull( skb, copyLength);
			return len;
		}
		else
		{
	...
	printk(KERN_DEBUG" get next skb in read queue\n");

	while( len < BCHANNEL_READ_LENGTH )
	{
		skb = skb_dequeue(&dev->rport.queue);
		spin_unlock_irqrestore(&dev->rport.lock, flags);

		copyLength = (BCHANNEL_READ_LENGTH-len) > skb->len ? skb->len: (BCHANNEL_READ_LENGTH-len);
		if (copy_to_user(buf, skb->data, copyLength) )
		{
			spin_lock_irqsave(&dev->rport.lock, flags);
...
static __inline__ ssize_t _as_isdn_do_core_read(struct file *file, char *buf, size_t count, loff_t * off)
{
...
//	TRACE();
	...
	if (minor != AS_ISDN_CORE_DEVICE)
	{
		...
		if (rawDev)
		{
			...
			{
				printk(KERN_ERR " minor(%d) is busy\n", minor);
				return(-EBUSY);
			}
			else
			{
	...
	else if (minor== AS_ISDN_CORE_DEVICE) /* mainDev*/
	{
		dev = &mainDev;
