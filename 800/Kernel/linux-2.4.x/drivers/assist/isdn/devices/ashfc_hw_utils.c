/*
 * $Id: ashfc_hw_utils.c,v 1.1.1.1 2006/11/30 16:27:58 lizhijie Exp $
 */
...
#define DATA_DIRECTION_ST_2_FIFO			0x80
...
/************** remove old slot for this channel ******************/
	...
	switch (protocol)
	{
		case (ISDN_PID_NONE):
			...
			HFC_wait(hc);
			HFC_outb(hc, A_CON_HDLC, DATA_DIRECTION_ST_2_FIFO | 0x00 | V_IFF);	/* Inter frame fill is 1s */
			...
			HFC_wait(hc);
			HFC_outb(hc, A_CON_HDLC, DATA_DIRECTION_ST_2_FIFO | 0x00);
			HFC_outb(hc, A_SUBCH_CFG, 0);
			HFC_outb(hc, A_IRQ_MSK, 0);
			HFC_outb(hc, R_INC_RES_FIFO, V_RES_F);
			HFC_wait(hc);
...
//			HFC_outb(hc, R_FIFO, (ch<<1) );		/* PCM should be in MSB ??, refer to P.168 */
			...
			HFC_wait(hc);
			HFC_outb(hc, A_CON_HDLC, DATA_DIRECTION_ST_2_FIFO | 0x00 | V_HDLC_TRP | V_IFF);		/* transparent mode selected, not HDLC mode */
...
//			HFC_outb(hc, R_FIFO, (ch<<1)|ASHFC_DATA_RX);
			...
			HFC_wait(hc);
			HFC_outb(hc, A_CON_HDLC, DATA_DIRECTION_ST_2_FIFO | 0x00 | V_HDLC_TRP);
			HFC_outb(hc, A_SUBCH_CFG, 0);
			HFC_outb(hc, A_IRQ_MSK, 0);
			HFC_outb(hc, R_INC_RES_FIFO, V_RES_F);
			HFC_wait(hc);
		...
		case (ISDN_PID_L1_NT_S0):
			...
			HFC_outb(hc, R_FIFO, ch<<1);
			HFC_wait(hc);
			HFC_outb(hc, A_CON_HDLC, DATA_DIRECTION_ST_2_FIFO | 0x04 | V_IFF);
			...
			HFC_outb(hc, R_FIFO, (ch<<1)|1);
			HFC_wait(hc);
			HFC_outb(hc, A_CON_HDLC, DATA_DIRECTION_ST_2_FIFO | 0x04);
