/*
 * $Id: ashfc_isr.c,v 1.1.1.1 2006/11/30 16:27:58 lizhijie Exp $
...
*/
		...
		while(i < 4)
		{
			...
			if (state)
			{
				if (state==active)
				{/* L1 is active */
					if (hc->activity[i])					/* running Tx or Rx */
					{
						led[i] = LED_COLOR_GREEN; 	/* led green */
						hc->activity[i] = 0;			/* set to 1 by Rx/Tx function */
					}
					else
						led[i] = LED_COLOR_RED; 		/* led red */
				}
				else if (hc->ledcount>>11) 			/* divided by 2048; deactivated, red blink */
					led[i] = LED_COLOR_RED; 			/* led red */
				else
					led[i] = LED_COLOR_OFF; 			/* led off */
			}
			else										/* no state has value of 0, so this statement is useless */
				led[i] = LED_COLOR_OFF; 				/* led off */
#endif
			if (state)
			{
...
led[i]=1, GPIO pin output 1, led green; led[i]=2, GPIO pin output 0,led red */
		...
		HFC_outb(hc, R_GPIO_OUT1, ((led[0]&LED_COLOR_GREEN)<<0) | ((led[1]&LED_COLOR_GREEN)<<1) 
			|((led[2]&LED_COLOR_GREEN)<<2) | ((led[3]&LED_COLOR_GREEN)<<3));
#if 0
