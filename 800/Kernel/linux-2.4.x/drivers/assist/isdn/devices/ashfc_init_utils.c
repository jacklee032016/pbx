/*
 * $Id: ashfc_init_utils.c,v 1.1.1.1 2006/11/30 16:27:58 lizhijie Exp $
 */
...
//	AS_ISDN_TE_DEV_LAYERS, AS_ISDN_TE_DEV_LAYERS 
	AS_ISDN_NT_L2_DEV_LAYERS, AS_ISDN_NT_L2_DEV_LAYERS 
};
...
int __init ashfc_init_object(as_isdn_object_t	*obj)
{
	memset(obj, 0, sizeof(as_isdn_object_t));
#ifdef MODULE
...
static int __init _ashfc_init_port(as_isdn_object_t *obj, ashfc_t *hc, int port)
{
	int err, i;
	...
	bchannel_t *bch;
	u_int _protocol = 0;
	if( port_protocol & (1<<port) )
		
		_protocol = ASISDN_TE_D_PROTOCOL_STACK;
	else
		_protocol = ASISDN_NT_D_PROTOCOL_STACK;
...
//			goto free_channels;
		...
		bch->channel = ch2;
		bch->file_ioctl = ashfc_bch_ioctl;
		bch->hw_bh = ashfc_bch_bh;
		bch->private = hc;
#if 0		
...
//			goto free_channels;
		...
		if( as_isdn_register_bchannel( bch) )
			return -ENODEV;
...
static int __init _ashfc_init_stack4port(as_isdn_object_t *obj, ashfc_t *hc, int port)
{
	int err;//, i;
//	int bchperport =2;	
	dchannel_t *dch;
//	bchannel_t *bch;
	as_isdn_stack_t *dst = NULL; /* make gcc happy */
	AS_ISDN_pid_t pid;
	u_int _protocol = 0;
	int _layermask;
	...
	{
		_protocol = ASISDN_NT_D_PROTOCOL_STACK;
		_layermask = AS_ISDN_NT_L2_DEV_LAYERS;	
	}
	else
	{
		_protocol = ASISDN_TE_D_PROTOCOL_STACK;
		_layermask = AS_ISDN_TE_DEV_LAYERS;	
	}
...
//	AS_ISDN_set_dchannel_pid(&pid, ashfc_protocol[port], ashfc_layermask[port]);
	AS_ISDN_set_dchannel_pid(&pid, _protocol, _layermask );
...
int __init ashfc_init_ashfc(as_isdn_object_t *obj, ashfc_t *hc)
{
	int err;
//	as_isdn_stack_t *dst = NULL; /* make gcc happy */
	int pt;
...
//	while (pt < hc->type) /* number of port/interface is 4 or 8 */
	while (pt < port_used) /* number of port/interface is 4 or 8 */
	{
