/*
 * $Id: ashfc_init.c,v 1.1.1.1 2006/11/30 16:27:58 lizhijie Exp $
...
#define HFC_MODULE_NAME		"ASHFC driver"

#include "as_isdn_version.h"

//static int HFC_cnt;
as_isdn_object_t	ASHFC_obj;
...
#define 	PORT_NT(i)			(1<<i)

#define ASHFC_PROTOCOLS 	\
		(AS_HFC_PORT_4_NT<<3) |\
		(AS_HFC_PORT_3_NT<<2) |\
		(AS_HFC_PORT_2_NT<<1) |\
		(AS_HFC_PORT_1_NT<<0)  
...
int port_protocol = ASHFC_PROTOCOLS;


#ifdef AS_HFC_PORTS
	int port_used = AS_HFC_PORTS;
#else
	int port_used = 4;
#endif
...
//TRACE();
			...
			if (hc->chan[i].bch)
			{
				...
				as_isdn_unregister_bchannel(hc->chan[i].bch );
...
int __init  ashfc_init(void)
{
	...
	printk(KERN_ERR AS_VERSION_INFO(HFC_MODULE_NAME));

	if( port_used<0 || port_used > 4)
	{
		printk(KERN_ERR "Port Number that used is not validate\n");
		return (-EINVAL);
	}
...
//	TRACE();
	...
	printk(KERN_INFO "Release " AS_VERSION_INFO(HFC_MODULE_NAME) );
	return;
}
...
MODULE_PARM(port_protocol, "1i");
MODULE_PARM(port_used, "1i");
