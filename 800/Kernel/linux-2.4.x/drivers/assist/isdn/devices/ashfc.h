/*
 * $Id: ashfc.h,v 1.1.1.1 2006/11/30 16:27:58 lizhijie Exp $
 */
 
#ifndef  __AS_HFC_H__
#define __AS_HFC_H__

#define WITH_ASHFC_DEBUG_INIT			0
#define WITH_ASHFC_DEBUG_CTRL			0
#define WITH_ASHFC_DEBUG_DATA		0
#define WITH_ASHFC_DEBUG_STATE		0

#define WITH_ASHFC_DEBUG_DCHANNEL	1

#define ASHFC_PORT_COUNT				4

#define ASHFC_MSB_FIRST				0x80
#define ASHFC_DATA_RX					0x01

/* see notice in ashfc.c  */
#define ASHFC_DEBUG_FIFO				0x0001
#define ASHFC_DEBUG_CRC				0x0002
#define ASHFC_DEBUG_INIT				0x0004
#define ASHFC_DEBUG_MGR				0x0008
#define ASHFC_DEBUG_MODE				0x0010
#define ASHFC_DEBUG_MSG				0x0020
#define ASHFC_DEBUG_STATE				0x0040
#define ASHFC_DEBUG_SYNC				0x0100
#define ASHFC_DEBUG_DTMF				0x0200
#define ASHFC_DEBUG_LOCK				0x8000
...
/* channel structure for a port(S/T interface) of HFC chips */
struct hfc_chan
{
	...
	struct sk_buff_head 	dtmfque;	/* only for B channel, skb queue for DTMF coefficients  */
...
typedef struct ashfc_hw	ashfc_hw_t;
...
/* this is the object attached in AS_ISDNobject->ilist */
struct ashfc
{
...
#endif
	ashfc_hw_t		hw;				/* remember data of write-only-registers */
	u_char			e1_switch; 		/* last state of the switches */

	u_long			chip;			/* chip configuration */
	int				masterclk;		/* port(ST interface) that provides master clock -1=off */
	...
	u_int			ledcount; 		/* used to animate leds with color red when deactivated */
	u_int			activity[8]; 		/* green blink with green when Tx/Rx if there is any action on this port (set by Rx/Tx function and will be cleared after showing led-states) */

	AS_ISDN_HWlock_t  lock;			/* the lock */
#ifdef SPIN_DEBUG
	void				*lock_adr;
#endif
	 ...
	 */
	hfc_chan_t		chan[32];		/* for 4S, only first 16 channels are used and there are 4 null channels for 4S card */
	u_char			created[8]; 		/* D channel AS_ISDNSTack has been created */
	signed char		slot_owner[256]; /* owner channel of slot */
};

typedef struct ashfc	ashfc_t;
...
#define R_RAM_ADDR1		0x09	/* address bit 8--15 */
#define R_RAM_ADDR2		0x0A
#define R_FIRST_FIFO		0x0B	/* First FIFO if the FIFO sequence(FSM mode)*/
#define R_RAM_SZ			0x0C	/* R_RAM_MISC */
#define R_FIFO_MD			0x0D	/* FIFO mode */
#define R_INC_RES_FIFO		0x0E	/* Increment and Reset FIFO, p.167 */
#define R_FSM_IDX			0x0F
#define R_FIFO				0x0F
#define R_SLOT				0x10
#define R_IRQMSK_MISC		0x11
#define R_SCI_MSK			0x12	/* State Change Irq Mask register of ST interfaces */
...
#define R_ST_SEL				0x16	/* ST interface selection register */
#define R_ST_SYNC			0x17	/* ST interface synchronization source */
#define R_CONF_EN			0x18
#define R_TI_WD				0x1A	/* Timer and WatchDog Control Register p.294 */
...
#define A_ST_RD_STATE		0x30	/* ST state machine register */
#define A_ST_WR_STATE		0x30
#define R_RX_OFF			0x30
#define A_ST_CTRL0			0x31	/* Control register of the selected SY interface, P.210 */
...
#define A_ST_CLK_DLY		0x37	/* clock registe for ST module, p.213 */
#define R_PWM0				0x38
#define R_PWM1				0x39
...
#define A_SL_CFG			0xD0		/*HFC channel assign for PCM slot and PCM output buffer configuration,p.244 */
#define A_CONF				0xD1		/* Conference Param, p.259 */
#define A_CH_MSK			0xF4
#define A_CON_HDLC			0xFA		/* HDLC and Connection setting for selected FIFO, p.172 */
...
#define R_IRQ_STATECH		0x12	/* state chane register of S/T interface ,P.215 */
#define R_CONF_OFLOW		0x14
#define R_RAM_USE			0x15
#define R_CHIP_ID			0x16	/* Chip ID, 1100 :4S 1000:8S */
#define R_BERT_STA			0x17
#define R_F0_CNTL			0x18	/* F0IO(Frame Sync 125us) pulse counter */
#define R_F0_CNTH			0x19
#define R_BERT_EC			0x1A
#define R_BERT_ECL			0x1A
#define R_BERT_ECH			0x1B
#define R_STATUS			0x1C	/* STATUS register, P.298 */
#define R_CHIP_RV			0x1F	/* Chip Revision: */	
#define R_STATE				0x20	
...
#define R_IRQ_FIFO_BL0		0xC8		/* IRQ for FIFO block 0 : FIFO 0--3(both Tx/Rx),p.299*/
...
/* R_CIRM 0x00, page.101 */
#define V_IRQ_SEL		0x01	/* bit0--2, for ISA PnP only */
#define V_SRES			0x08	/* soft reset : group 0 */
#define V_HFCRES		0x10	/* HFC reset: group 1*/
#define V_PCMRES		0x20	/* PCM reset: group 2 */
#define V_STRES			0x40	/* S/T reset:group 3*/
#define V_ETRES			0x40
#define V_RLD_EPR		0x80	/*EEPROM reload */

/* R_CTRL */
#define V_FIFO_LPRIO	0x02	/* Low Priority */
#define V_SLOW_RD		0x04	/* additional wait cycle for PCI read access */
#define V_EXT_RAM		0x08	/* use external RAM */
#define V_CLK_OFF		0x20	/* CLK oscillator is switch off */
#define V_ST_CLK			0x40	/* S/T clock selection */

/* R_RAM_ADDR0, 0x0A, P.103 */
#define V_RAM_ADDR2	0x01	/* bit 0--3(in R_RAM_ADDR2) : representing address 16--19*/
#define V_ADDR_RES		0x40	/* address reset */
#define V_ADDR_INC		0x80	/* address automatic increase after read/write */
...
#define V_REV			0x80
/* R_SLOT */
#define V_SL_DIR			0x01		/* Rx direction */
...
/* A_CON_HDLC */
#define V_IFF			0x01		/* write 1s as Inter Frame Fill */
#define V_HDLC_TRP		0x02		/* Transparent mode : PCM data */
...
#define V_DTMF_STOP		0x08	/* stop after N steps */
#define V_CHBL_SEL			0x10	/* HFC CHannle BLock Select */
#define V_RST_DTMF			0x40	/* restart */
...
/* R_IRQ_MISC p.297 */
#define V_STA_IRQ			0x01	/* not support in 4/8S */
#define V_TI_IRQ				0x02	/* Timer Interrrupt */
#define V_IRQ_PROC			0x04	/* Processing transition */
#define V_DTMF_IRQ			0x08	/* DTMF */
#define V_IRQ1S				0x10	/* not support is 4/8S */
#define V_SA6_IRQ			0x20	/* V_EXT_IRQ */
#define V_RX_EOMF			0x40	/* not support */
#define V_TX_EOMF			0x80	/* not support */

/* R_STATUS */
#define V_BUSY				0x01	
#define V_PROC				0x02
#define V_DTMF_STA			0x04	/* reserved in Page.298 ????*/
#define V_LOST_STA			0x08	/* LOST error(frame has been lost)*/
#define V_SYNC_IN			0x10	/* SYNC_1 onput pin*/
#define V_EXT_IRQSTA		0x20	/* External IRQ */
#define V_MISC_IRQSTA		0x40	/* any MISC IRQ */
#define V_FR_IRQSTA			0x80	/* any FIFO IRQ */
...
static unsigned char _HFC_outb(ashfc_t *a, unsigned char b, unsigned char c, char *function, int line)
{
	...
	bits[1] = '0'+(!!(c&64));
	bits[0] = '0'+(!!(c&128));
	printk(KERN_ERR  "HFC_outb(\"%s\", %02x=%s, 0x%02x=%s); in %s() line %d\n", a->name, b, regname, c, bits, function, line);
	return(*(((volatile u_char *)a->pci_membase)+b) = c);
}
#define HFC_inb(a,b) _HFC_inb(a, b, __FUNCTION__, __LINE__)
static unsigned char _HFC_inb(ashfc_t *a, unsigned char b, char *function, int line)
{
	char regname[256]="", bits[9]="xxxxxxxx";
	...
	bits[1] = '0'+(!!(c&64));
	bits[0] = '0'+(!!(c&128));
	printk(KERN_ERR  "HFC_inb(\"%s\", %02x=%s) = 0x%02x=%s; in %s() line %d\n", a->name, b, regname, c, bits, function, line);
	return(c);
}
#define HFC_inw(a,b) _HFC_inw(a, b, __FUNCTION__, __LINE__)
static unsigned short _HFC_inw(ashfc_t *a, unsigned char b, char *function, int line)
{
	char regname[256]="";
	...
	printk(KERN_ERR  "HFC_inw(\"%s\", %02x=%s) = 0x%04x; in %s() line %d\n", a->name, b, regname, c, function, line);
	return(c);
}
#define HFC_wait(a) _HFC_wait(a, __FUNCTION__, __LINE__)
static void _HFC_wait(ashfc_t *a, char *function, int line)
{
	printk(KERN_ERR  "HFC_wait(\"%s\"); in %s() line %d\n", a->name, function, line);
...
/* usage: HFC_outX(card,register,value); */
static inline void HFC_outb(ashfc_t *a, u_char b, u_char c)
{
	outb(b,(a->pci_iobase)+4);
	outb(c,a->pci_iobase);
}
static inline void HFC_outl(ashfc_t *a, u_char b, u_long c)
{
	outb(b,(a->pci_iobase)+4);
	outl(c,a->pci_iobase);
}
...
static inline u_char HFC_inb(ashfc_t *a, u_char b)
{
	outb(b,(a->pci_iobase)+4);
	return (inb((volatile u_int)a->pci_iobase));
}
static inline u_short HFC_inw(ashfc_t *a, u_char b)
{
	outb(b,(a->pci_iobase)+4);
	return (inw((volatile u_int)a->pci_iobase));
}
static inline u_long HFC_inl(ashfc_t *a, u_char b)
{
	outb(b,(a->pci_iobase)+4);
...
/* usage: HFC_wait(card); */
static inline void HFC_wait(ashfc_t *a)
{
...
/* no debug */
#define HFC_outl_(a,b,c) 		( a+1)
#define HFC_inl_(a,b) 			( a+1)
#define HFC_inw_(a,b) 		( a+1)
#define HFC_outb_(a,b,c) 		( a+1)
#define HFC_inb_(a,b) 		( a+1)
#define HFC_wait_(a) 			( a+1)

#define HFC_outb(a,b,c) 		( a+1)

#define HFC_inb(a,b) 			( a+1)
#define HFC_inw(a,b)	 		( a+1)

#define HFC_wait(a) 			( a+1)

#endif

#include "ashfc_macro.h"
