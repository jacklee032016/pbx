/*
 * $Id: i4l_mISDN.c,v 1.1.1.1 2006/11/30 16:27:58 lizhijie Exp $
...
#include <linux/AS_ISDNif.h>
#include "fsm.h"
#include "helper.h"
#include "dss1.h"
#include "debug.h"

static char *i4lcapi_revision = "$Revision: 1.1.1.1 $";
...
static as_isdn_object_t	I4Lcapi;
...
sendqueued(i4l_channel_t *ch)
{
	...
	if (!test_bit(I4L_FLG_BCONN, &ch->Flags)) {
		if (ch->drv->debug & 0x40)
			printk(KERN_ERR  "%s: bc%d not ready\n", __FUNCTION__, ch->nr);
		return(0);
	}
	while ((skb = skb_dequeue(&ch->sendq))) {
		...
		if (ch->drv->debug & 0x800)
			printk(KERN_ERR  "bc%d sent skb(%p) %d(%d)\n", ch->nr, skb, ret, len);
		if (ret == len) {
			continue;
		} else if (ret > 0) {
...
i4l_dhup(struct FsmInst *fi, int event, void *arg)
{
	...
	skb = AS_ISDN_alloc_l3msg(8, MT_RELEASE);
	if (!skb)
		return;
...
i4l_icall(struct FsmInst *fi, int event, void *arg)
{
	...
	if ((skb = alloc_skb(sizeof(int *) + 8, GFP_ATOMIC))) {
		...
		*idp = &ch->l4id;
		...
		if (ch->drv->debug & 0x2)
			printk(KERN_ERR  "%s: l4id(%x) ch(%p)->nr %d\n", __FUNCTION__, ch->l4id, ch, ch->nr);
	} else
		return;
	...
	if (setup->phone[0]) {
		...
		tmp[0] = i-1;
		AS_ISDN_AddIE(skb, IE_CALLING_PN, tmp);
		if (setup->phone[j] == '.') {
			i = 1;
			tmp[i++] = 0x80;
			j++;
			while (setup->phone[j])
				tmp[i++] = setup->phone[j++];
			tmp[0] = i-1;
			AS_ISDN_AddIE(skb, IE_CALLING_SUB, tmp);
		}
	}
	if (setup->eazmsn[0]) {
		i = 1;
		...
		tmp[0] = i-1;
		AS_ISDN_AddIE(skb, IE_CALLED_PN, tmp);
		if (setup->eazmsn[j] == '.') {
			i = 1;
			tmp[i++] = 0x80;
			j++;
			while (setup->eazmsn[j])
				tmp[i++] = setup->eazmsn[j++];
			tmp[0] = i-1;
			AS_ISDN_AddIE(skb, IE_CALLED_SUB, tmp);
		}
	}
	p = tmp;
	*p++ = IE_LLC;
...
i4l_dconn_out(struct FsmInst *fi, int event, void *arg)
{
	...
	skb = AS_ISDN_alloc_l3msg(4, MT_CONNECT);
	if (!skb)
		return;
...
stackready(struct FsmInst *fi, int event, void *arg)
{
	...
	fsm_change_state(fi, ST_BREADY);
	test_and_set_bit(I4L_FLG_BREADY, &ch->Flags);
	if (test_bit(I4L_FLG_BCONN, &ch->Flags))
		as_isdn_fsm_event(&ch->i4lm, EV_I4L_BCONN, NULL);
}
...
capi_ocall(struct FsmInst *fi, int event, void *arg)
{
	...
	if (ch->drv->debug & 0x4)
		printk(KERN_ERR  "ocall from %s, si(%d/%d) -> %s\n", ctrl.parm.setup.eazmsn,
...
capi_connect(struct FsmInst *fi, int event, void *arg)
{
	...
	fsm_change_state(fi, ST_WAITDCONN);
...
static int
Dchannel_i4l(as_isdn_if_t *hif, struct sk_buff *skb)
{
	int		i, ret = -EINVAL;
	as_isdn_head_t	*hh;
	i4l_capi_t	*ic;
	...
	switch(hh->prim) {
		case CC_SETUP | REQUEST:
			ret = as_isdn_fsm_event(&ch->i4lm, EV_CAPI_OCALL, skb);
			break;
		case CC_ALERTING | REQUEST:
			ret = as_isdn_fsm_event(&ch->i4lm, EV_CAPI_ALERT, skb);
			break;
		case CC_CONNECT | REQUEST:
			ret = as_isdn_fsm_event(&ch->i4lm, EV_CAPI_DCONNECT, skb);
			break;
		case CC_DISCONNECT | REQUEST:
		case CC_RELEASE | REQUEST:
			ret = as_isdn_fsm_event(&ch->i4lm, EV_CAPI_DISCONNECT, skb);
			break;
		case CC_RELEASE_COMPLETE | REQUEST:
			ret = as_isdn_fsm_event(&ch->i4lm, EV_CAPI_RELEASE, skb);
			break;
		default:
			if (debug)
				printk(KERN_ERR  "%s: ch%x prim(%x) id(%x) not handled\n",
...
static int
Bchannel_i4l(as_isdn_if_t *hif, struct sk_buff *skb)
{
	i4l_channel_t	*ch;
	int		ret = -EINVAL;
	as_isdn_head_t	*hh;
	...
	switch(hh->prim) {
		...
		case DL_ESTABLISH | REQUEST:
			as_isdn_fsm_event(&ch->i4lm, EV_CAPI_ESTABLISHB, NULL);
			skb_trim(skb, 0);
			ret = if_newhead(&ch->inst.up, hh->prim | CONFIRM, 0, skb);
			break;
		case PH_DEACTIVATE | REQUEST:
		case DL_RELEASE | REQUEST:
			as_isdn_fsm_event(&ch->i4lm, EV_CAPI_RELEASEB, NULL);
			skb_trim(skb, 0);
			ret = if_newhead(&ch->inst.up, hh->prim | CONFIRM, 0, skb);
			break;
...
i4l_sent_pkt(i4l_capi_t *drv, isdn_ctrl *c)
{
	...
	if (drv->debug & 0x800)
		printk(KERN_ERR  "bc%ld ack skb(%p)\n", c->arg, skb);
	if (skb_queue_len(&ch->sendq))
		sendqueued(ch);
	skb_trim(skb, 0);
	hhe = AS_ISDN_HEADEXT_P(skb);
...
I4Lcapi_status_callback(isdn_ctrl *c)
{
	...
	ch = drv->ch;
	if (drv->debug & 0x8)
		printk(KERN_ERR  "drv%d cmd(%d) arg(%ld)\n",
	...
	switch (c->command) {
		...
		case ISDN_STAT_ICALL:
			if (c->arg < 0)
				return -1;
			ch += c->arg;
			ret = as_isdn_fsm_event(&ch->i4lm, EV_I4L_ICALL, &c->parm.setup);
			break;
		case ISDN_STAT_CINF:
			if (c->arg < 0)
				return -1;
		...
		case ISDN_STAT_CAUSE:
			...
			ch->cause_val |= 0x80;
			if (drv->debug & 0x1)
				printk(KERN_ERR  "isdn: ch%ld cause: %s %02x%02x\n",
		...
		case ISDN_STAT_DCONN:
			if (c->arg < 0)
				return -1;
			ch += c->arg;
			ret = as_isdn_fsm_event(&ch->i4lm, EV_I4L_DCONN, NULL);
			break;
		case ISDN_STAT_DHUP:
			if (c->arg < 0)
				return -1;
			ch += c->arg;
			ret = as_isdn_fsm_event(&ch->i4lm, EV_I4L_DHUP, NULL);
			break;
		case ISDN_STAT_BCONN:
			if (c->arg < 0)
				return -1;
			ch += c->arg;
			ret = as_isdn_fsm_event(&ch->i4lm, EV_I4L_BCONN, NULL);
			break;
		case ISDN_STAT_BHUP:
			if (c->arg < 0)
				return -1;
			ch += c->arg;
			ret = as_isdn_fsm_event(&ch->i4lm, EV_I4L_BHUP, NULL);
			break;
		...
		case ISDN_STAT_L1ERR:
			if (c->arg < 0)
				return -1;
			ch += c->arg;
			ret = as_isdn_fsm_event(&ch->i4lm, EV_I4L_L1ERR, NULL);
			break;
...
I4Lcapi_manager(void *data, u_int prim, void *arg) {
	...
	if (debug & 0x100)
		printk(KERN_ERR  "%s: data:%p prim:%x arg:%p\n",
			__FUNCTION__, data, prim, arg);
	if (prim == (MGR_HASPROTOCOL | REQUEST))
		return(AS_ISDN_HasProtocolP(&I4Lcapi, arg));
	if (!data) {
	...
	switch(prim) {
		...
		if (nr_ch == -1)
			return(AS_ISDN_SetIF(inst, arg, prim, Dchannel_i4l, NULL, card));
		else
			return(AS_ISDN_SetIF(inst, arg, prim, Bchannel_i4l, NULL, channel));
	    ...
	    case MGR_SETSTACK | CONFIRM:
			...
			as_isdn_fsm_event(&channel->i4lm, EV_STACKREADY, NULL);
		}
		break;
	    default:
		if (debug)
			printk(KERN_ERR  "I4Lcapi_manager prim %x not handled\n", prim);
		return(-EINVAL);
	}
	return(0);
}
...
I4Lcapi_register(isdn_if *iif)
{
	...
	AS_ISDN_init_instance(&drvmap[drvidx]->inst, &I4Lcapi, drvmap[drvidx]);
	sprintf(drvmap[drvidx]->inst.name, "Fritz%d", drvidx+1);
	AS_ISDN_set_dchannel_pid(&drvmap[drvidx]->pid, 2, 0);
...
I4Lcapi_init(void)
{
	...
	as_isdn_fsm_new(&i4lfsm_s, I4LFnList, I4L_FN_COUNT);
	
	if ((err = as_isdn_register(&I4Lcapi)))
	{
		printk(KERN_ERR "Can't register I4L CAPI error(%d)\n", err);
		as_isdn_fsm_free(&i4lfsm_s);
		return(err);
	}
...
I4Lcapi_cleanup(void)
{
	...
	int err;
	if ((err = as_isdn_unregister(&I4Lcapi))) {
