/*
 * $Id: faxl3.c,v 1.1.1.1 2006/11/30 16:27:58 lizhijie Exp $
 * Linux ISDN subsystem, Fax Layer 3
 */

#include <linux/config.h>
#include <linux/module.h>
/* added by lizhijie, 2005.08.02 */
...
static as_isdn_object_t faxl3_obj;

static char *AS_ISDN_faxl3_revision = "$Revision: 1.1.1.1 $";
...
print_Dxx(faxl3_t *fl3, int typ)
{
	...
	v2 = (p[2] >> 2) & 3;
	v3 = (p[2] >> 4) & 7;
	printk(KERN_ERR  "%s: width(%s) plength(%s) MinRow(%s)\n", ts,
	...
	if (typ == Dxx_TYPE_DCS)
		printk(KERN_ERR  "%s:%s%s%s BS(%s)%s\n", ts,
...
send_data_down(faxl3_t *fl3, struct sk_buff *skb) {
	int		ret = 0;
	as_isdn_if_t	*down = &fl3->inst.down;

	if (test_and_set_bit(FAXL3_STATE_DATABUSY, &fl3->state)) {
		skb_queue_tail(&fl3->downq, skb);
	} else {
		AS_ISDN_sethead(PH_DATA_REQ, data_next_id(fl3), skb);
...
set_new_modulation(struct FsmInst *fi, int event, void *arg)
{
	...
	if ((fl3->pending_mod < 0) || (fl3->pending_rate <0)) {
		if (event == EV_MOD_READY)
			fsm_change_state(fi, ST_MOD_IDLE);
		else
			int_error();
		return;
	}
	fsm_change_state(fi, ST_MOD_WAITCONNECT);
...
mod_activ(struct FsmInst *fi, int event, void *arg)
{
	...
	fsm_change_state(fi, ST_MOD_CONNECTED);
	fl3->current_mod = fl3->pending_mod;
	fl3->pending_mod = -1;
	fl3->current_rate = fl3->pending_rate;
	fl3->pending_rate = -1;
	as_isdn_fsm_event(&fl3->main, EV_MODEM_ACTIV, NULL);	
}

static void
mod_disconnect(struct FsmInst *fi, int event, void *arg)
{
	fsm_change_state(fi, ST_MOD_WAITDISCONNECT);
}
...
mod_error(struct FsmInst *fi, int event, void *arg)
{
	...
	fsm_change_state(fi, ST_MOD_IDLE);
	as_isdn_fsm_event(&fl3->main, EV_MODEM_ERROR, NULL);	
}
...
mod_nocarrier(struct FsmInst *fi, int event, void *arg)
{
	...
	fsm_change_state(fi, ST_MOD_IDLE);
	as_isdn_fsm_event(&fl3->main, EV_MODEM_IDLE, NULL);	
}
...
{
	fsm_change_state(fi, ST_L3_WAIT_RECVDIS);
}

static void
l3m_activ_dis(struct FsmInst *fi, int event, void *arg)
{
	fsm_change_state(fi, ST_L3_RECV_DIS);
}
...
l3m_finish_dis(struct FsmInst *fi, int event, void *arg)
{
	...
	if (fl3->current_rate_idx > 0) {
		...
		fl3->pending_rate = 3;
		fsm_change_state(fi, ST_L3_WAIT_SENDDCS);
		as_isdn_fsm_event(&fl3->mod, EV_MOD_NEW, NULL);
	} else // ABORT
		fsm_change_state(fi, ST_L3_IDLE);
}
...
l3m_send_dcs(struct FsmInst *fi, int event, void *arg)
{
	...
	fsm_change_state(fi, ST_L3_SEND_DCS);
...
l3m_finish_dcs(struct FsmInst *fi, int event, void *arg)
{
	...
	fsm_change_state(fi, ST_L3_WAIT_SENDTRAIN);
	/* wait 75 ms */
	fsm_restart_timer(&fl3->deltimer, 75, EV_DELAYTIMER, NULL, 2);
}
...
l3m_send_train(struct FsmInst *fi, int event, void *arg)
{
	...
	fsm_change_state(fi, ST_L3_SEND_TRAIN);
...
l3m_finish_train(struct FsmInst *fi, int event, void *arg)
{
	...
	fl3->pending_rate = 3;
	fsm_change_state(fi, ST_L3_WAIT_TRAINSTATE);
	as_isdn_fsm_event(&fl3->mod, EV_MOD_NEW, NULL);
}
...
{
	fsm_change_state(fi, ST_L3_RECV_TRAINSTATE);
}
...
l3m_receive_trainstate(struct FsmInst *fi, int event, void *arg)
{
	...
	switch(*p) {
		case 0x84: // CFR
			...
			printk(KERN_ERR  "training successfull\n");
		...
		case 0x44: // FTT
			...
			printk(KERN_ERR  "training failed\n");
			break;
		default:
...
l3m_finish_trainstate(struct FsmInst *fi, int event, void *arg)
{
	...
	if (fl3->result == FAXL3_RESULT_FTT) {
		...
		if (fallback_rate(fl3)) {
			...
			fsm_change_state(fi, ST_L3_WAIT_SENDDCN);
		}
	} else {
		fsm_change_state(fi, ST_L3_WAIT_SENDPAGE);
		fsm_restart_timer(&fl3->deltimer, 75, EV_DELAYTIMER, NULL, 2);
	}
}
...
l3m_ready_sendpage(struct FsmInst *fi, int event, void *arg)
{
	...
	fsm_change_state(fi, ST_L3_SEND_PAGE);
...
l3m_finish_page(struct FsmInst *fi, int event, void *arg)
{
	...
	fl3->pending_mod = HW_MOD_FTH;
	fl3->pending_rate = 3;
	fsm_change_state(fi, ST_L3_WAIT_SENDEOP);
	as_isdn_fsm_event(&fl3->mod, EV_MOD_NEW, NULL);
}
...
l3m_send_endofpage(struct FsmInst *fi, int event, void *arg)
{
	...
	fsm_change_state(fi, ST_L3_SEND_EOP);
...
l3m_finish_eop(struct FsmInst *fi, int event, void *arg)
{
	...
	fl3->pending_mod = HW_MOD_FRH;
	fl3->pending_rate = 3;
	fsm_change_state(fi, ST_L3_WAIT_RECVMCF);
	as_isdn_fsm_event(&fl3->mod, EV_MOD_NEW, NULL);
}
...
{
	fsm_change_state(fi, ST_L3_RECV_MCF);
}
...
l3m_receive_mcf(struct FsmInst *fi, int event, void *arg)
{
	...
	switch(*p) {
		case 0x8C: // MCF
			...
			printk(KERN_ERR  "got MCF\n");
			break;
		case 0xCC: // RTP
			fl3->result = FAXL3_RESULT_RTP;
			printk(KERN_ERR  "got RTP\n");
			break;
		case 0x4C: // RTN
			fl3->result = FAXL3_RESULT_RTN;
			printk(KERN_ERR  "got RTN\n");
			break;
		default:
			fl3->result = FAXL3_RESULT_NONE;
			int_errtxt("unhandled FCF (%02X) len %d", *p, skb->len);
...
l3m_finish_dcn(struct FsmInst *fi, int event, void *arg)
{
	...
	fsm_change_state(fi, ST_L3_CLEARING);
}
...
prepare_page_data(faxl3_t *fl3)
{
	...
	if (!test_and_set_bit(FAXL3_STATE_SFFHEADER, &fl3->state)) {
		...
		printk(KERN_ERR  "SFFHEADER(%x,%x,%x,%x)\n", tmp32, ver, len8, tmp16);
		...
		printk(KERN_ERR  "SFFHEADER pages %d ofP(%x) o_lpage(%x) o_dend(%x)\n",
	...
	while (fl3->data_skb->len > 1) {
		...
		if (fl3->lasttyp == 255) {
			...
			printk(KERN_ERR  "current page end: %d lines\n", fl3->line_cnt); 
			if (len8 == 0) {
				// doc end
				printk(KERN_ERR  "SFF doc end found\n");
			...
			printk(KERN_ERR  "SFF page header len %d\n", len8);
			if (len8 < 10) {
				int_error();
				return(-EINVAL);
			}
			...
			printk(KERN_ERR  "SFF page header: vres(%x) hres(%x) code(%x) resrv(%x)\n",
				fl3->page_vres, fl3->page_hres, fl3->page_code, fl3->page_rsv1);
			printk(KERN_ERR  "SFF page header: llen(%d) plen(%d) op(%x) on(%x)\n",
...
data_b3_req(faxl3_t *fl3, struct sk_buff *skb)
{
	...
	/* we save DataHandle and Flags in a area after normal AS_ISDN_HEAD */ 
	hh++;
...
connect_b3_req(faxl3_t *fl3, u_int di, u32 addr, struct sk_buff *skb)
{
	...
	fl3->pending_rate = 3;
	as_isdn_fsm_event(&fl3->mod, EV_MOD_NEW, NULL);
	as_isdn_fsm_event(&fl3->main, EV_CALL_OUT, NULL);
...
static int
faxl3_from_up(as_isdn_if_t *hif, struct sk_buff *skb)
{
	faxl3_t		*faxl3;
	as_isdn_head_t	*hh;
	__u32		addr;
	...
	if (faxl3->debug & DEBUG_FAXL3_FUNC)
		printk(KERN_ERR  "%s: prim(%x) dinfo(%x) len(%d)\n", __FUNCTION__, hh->prim, hh->dinfo, skb->len);
	if (skb->len < 4) {
...
ph_status_ind(faxl3_t *fl3, int status)
{
	switch(status) {
		case HW_MOD_READY:
			as_isdn_fsm_event(&fl3->mod, EV_MOD_READY, NULL);
			break;
		case HW_MOD_CONNECT:
			as_isdn_fsm_event(&fl3->mod, EV_MOD_CONNECT, NULL);
			break;
		case HW_MOD_OK:
		case HW_MOD_NOCARR:
			as_isdn_fsm_event(&fl3->mod, EV_MOD_NOCARRIER, NULL);
			break;
		default:
...
ph_data_cnf(faxl3_t *fl3, int status)
{
	...
	if (skb)  {
		...
		as_isdn_fsm_event(&fl3->main, EV_NEXT_DATA, NULL);
	}
}

static int
faxl3_from_down(as_isdn_if_t *hif,  struct sk_buff *skb)
{
	faxl3_t		*faxl3;
	as_isdn_head_t	*hh;
	int		err = 0;
	...
	switch(hh->prim) {
		...
		case PH_DATA | INDICATION:
			as_isdn_fsm_event(&faxl3->main, EV_DATA, skb);
			break;
...
release_faxl3(faxl3_t *faxl3) {
	as_isdn_instance_t	*inst = &faxl3->inst;
	...
	fsm_del_timer(&faxl3->deltimer, 99);
	fsm_del_timer(&faxl3->timer1, 99);
	fsm_del_timer(&faxl3->modtimer, 99);
	if (faxl3->entity != AS_ISDN_ENTITY_NONE)
...
static int
new_faxl3(as_isdn_stack_t *st, AS_ISDN_pid_t *pid) {
	faxl3_t *n_faxl3;
	u8	*p;
	int	err;
	...
	n_faxl3->entity = AS_ISDN_ENTITY_NONE;
	n_faxl3->next_id = 1;
	spin_lock_init(&n_faxl3->lock);
	memcpy(&n_faxl3->inst.pid, pid, sizeof(AS_ISDN_pid_t));
	...
	if (debug & DEBUG_FAXL3_CFG) {
		printk(KERN_ERR  "%s opt(%x) fmt(%x) id=%s head=%s\n",
		...
		if (n_faxl3->inst.pid.param[1]) {
			...
			printk(KERN_ERR  "B1 param len %d rate %d\n", *p,
				(*p > 1) ? *((u16 *)(p+1)) : -1);
		}
	}
	AS_ISDN_init_instance(&n_faxl3->inst, &faxl3_obj, n_faxl3);
	if (!AS_ISDN_SetHandledPID(&faxl3_obj, &n_faxl3->inst.pid)) {
		int_error();
		kfree(n_faxl3);
		return(-ENOPROTOOPT);
	}
	...
	fsm_init_timer(&n_faxl3->main, &n_faxl3->deltimer);
	fsm_init_timer(&n_faxl3->main, &n_faxl3->timer1);
	...
	fsm_init_timer(&n_faxl3->mod, &n_faxl3->modtimer);
	...
	if (err) {
		printk(KERN_WARNING "AS_ISDN %s: MGR_NEWENTITY REQUEST failed err(%x)\n",
			__FUNCTION__, err);
	}
	...
	if (err) {
		...
		if (debug)
			printk(KERN_ERR  "%s:mlen(%d) hup(%d) hdown(%d)\n", __FUNCTION__,
...
faxl3_manager(void *data, u_int prim, void *arg) {
	...
	if (debug & DEBUG_FAXL3_MGR)
		printk(KERN_ERR  "faxl3_manager data:%p prim:%x arg:%p\n", data, prim, arg);
	if (!data)
		return(err);
	...
	switch(prim) {
		...
		return(AS_ISDN_SetIF(inst, arg, prim, faxl3_from_up, faxl3_from_down, faxl3_l));
		...
		if (debug & DEBUG_FAXL3_MGR)
			printk(KERN_ERR  "release_faxl3 id %x\n", faxl3_l->inst.st->id);
...
static int faxl3_init(void)
{
	...
	printk(KERN_INFO "%s modul version %s\n", MName, AS_ISDN_getrev(AS_ISDN_faxl3_revision));
#ifdef MODULE
	...
	if ((err = as_isdn_register(&faxl3_obj))) {
	...
	as_isdn_fsm_new(&faxl3fsm, FaxL3FnList, FAXL3_FN_COUNT);
	...
	as_isdn_fsm_new(&modfsm, ModFnList, MOD_FN_COUNT);
	return(err);
}
...
static void faxl3_cleanup(void)
{
	...
	if ((err = as_isdn_unregister(&faxl3_obj))) {
	...
	as_isdn_fsm_free(&faxl3fsm);
	as_isdn_fsm_free(&modfsm);
}
