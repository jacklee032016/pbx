 ...
 *
 * $Id: as_zl_init.c,v 1.1.1.1 2006/11/30 16:28:00 lizhijie Exp $
...
{
#if AS_HW_EC_50235
	name 			:	 "Assist Voice Echo Canceller(16 Channels)",

	group_num		:	8,
	chan_num		:	16,
#else
	name 			:	 "Assist Voice Echo Canceller(8 Channels)",
...
#define  AS_ZL_PROC_DIR_NAME	"astel/ec"		/* ZarLink Echo Canceller */
...
static int as_zarlink_proc_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	...
	if ( zr->name ) 
	{
		...
		len += sprintf(page + len,  AS_VERSION_INFO("AssistEchoCanceler") );
	}
...
static void __exit as_zarlink_cleanup(void) 
{
	...
	*ZARLINK_CS_REGISTER = 0;//|= ~(ZARLINK_CONTROL_STATUS);
	...
	printk(KERN_INFO "Assist Echo Canceler Driver Unloaded\r\n" );
}
...
static int __init as_zarlink_init(void) 
{
	...
	*ZARLINK_CS_REGISTER |= ZARLINK_CONTROL_STATUS;
	...
	if(!zarlink.resource) 
	{
		printk(KERN_ERR "ASEC : Could not request mem region.\n" );
		res = -ENOMEM;
		goto Error;
	}
	...
	if (!zarlink.map_private) 
	{
		printk(KERN_ERR "ASEC : Failed to map IO region. (ioremap)\n");
		res = -EIO;
		goto Error;
	}
	...
	if( res <0)
	{
		printk(KERN_ERR "ASEC : Device failed when initialized\n");
		goto Error;
	}
...
#if AS_HW_EC_50235
	printk(KERN_INFO "Assist 16 Channels Echo Canceler Driver loaded\r\n" );
#else
	printk(KERN_INFO "Assist 8 Channels Echo Canceler Driver loaded\r\n" );
#endif

	printk(KERN_INFO  AS_VERSION_INFO("AssistEchoCanceler") );
	return res;
