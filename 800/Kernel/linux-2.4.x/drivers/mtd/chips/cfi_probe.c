/* 
   ...
   $Id: cfi_probe.c,v 1.1.1.1 2006/11/30 16:28:18 lizhijie Exp $
*/
...
/* comment this line. lizhijie 2004.07.22 
#define DEBUG_CFI 
*/
...
{
	int osf = cfi->interleave * cfi->device_type;/*1; 	// scale factor */

#if 0
	printk("cfi_read result=%lx ought ot be =%lx\n", cfi_read(map,base+osf*0x10),cfi_build_cmd('Q',map,cfi));
			  ...
			  struct flchip *chips, struct cfi_private *cfi)
{
	int i;
/*
	printk("cfi->interleave=%d device type =%d\n", cfi->interleave, cfi->device_type);
*/	
	if ((base + 0) >= map->size) {
		printk(KERN_NOTICE
...
/*	change  this line as following. Refer to 'Intel Advanced+ Boot Block Flash Memory(C3) Datasheet
page 46.  lizhijie 2004.07.22
