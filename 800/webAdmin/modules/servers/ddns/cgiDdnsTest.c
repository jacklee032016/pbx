/*
* $Id: cgiDdnsTest.c,v 1.1 2007/05/31 20:39:35 lizhijie Exp $
*/
...
static char *__ddns_test_status(DDNS_INFO *info)
{
	char buf[4096];
	int length = 0;
	...
	ip = DdnsTestRegisterResult(info->fqdn);
...
int cgiDdnsTest(DDNS_INFO *info)
{
	...
	if(pid <= 0)
	{
		CGI_SPRINTF(buf, 0, "%s %s %s!", gettext(CGI_STR_DDNS_NAME), gettext(CGI_STR_DAEMON), gettext("is not running now") );
		return cgi_error_page(NULL, gettext(CGI_STR_WARN), buf);
	}
	
	CGI_SPRINTF(buf, 0, "%s DDNS %s", gettext(CGI_STR_DDNS_NAME), gettext(CGI_STR_TEST_RESULT) );
