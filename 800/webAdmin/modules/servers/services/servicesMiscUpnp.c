...
#else
	length += CGI_SPRINTF(buf,length, "<tr><td align=\"center\" bgcolor=\"#cccccc\"><strong>%s</strong></td><td align=\"center\">%s</td><td align=\"center\">%s</td></TR>",
		gettext("Net Neighbour"), cgi_service_get_state_name(SERVICE_NAME_UPNP_NEIGHBOUR), cgi_button(gettext(CGI_STR_MODIFY), WEB_URL_SERVICES ) );
	length += CGI_SPRINTF(buf,length, "<tr><td align=\"center\" bgcolor=\"#cccccc\"><strong>%s</strong></td><td align=\"center\">%s</td><td align=\"center\">%s</td></TR>",
		gettext("Port Forward"), cgi_service_get_state_name(SERVICE_NAME_UPNP_FORWARD), cgi_button(gettext(CGI_STR_MODIFY), WEB_URL_SERVICES ) );
#endif
...
{
#if 1
	cgi_info_page(gettext("UPNP Configuration"), __servicesMisc_upnp_status(), "" );
#else
	char		*action;
