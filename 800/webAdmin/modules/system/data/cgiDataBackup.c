...
static int __backup_packaging(char *opName)
{
	char		msg[1024];
	int		len = 0;
	char 	cmd[256] = "\0";
	...
	chdir(path);
	sprintf(cmd,"/bin/tar czf /tmp/%s -C %s", dataFilename, CGIDATA_BACKUP_DIRS);
...
//		error_flag=ERROR_TAR;
	...
	sprintf(cmd,"ln -s ../../data/%s /tmp/%s", dataFilename, dataFilename);
...
static int __backupdata_to_server(char *serverip)
{
	...
	if(__backup_packaging(CGIDATA_OP_BACK_SERVER))
	{
		return -1;
	}
	ret = -1;
...
int cgiData_BackupServer(DATA_INFO *info)
{
	...
	{
		cgi_info_page(gettext("Data Backup to TFTP Server"), "", cgidata_serverip_button(info, CGIDATA_OP_BACK_SERVER) );
		return 0;
	}
...
static char *__backup_local_button(char *_fileName)
{
	...
	length += sprintf(buf+length, "<TR><TD align=\"center\"><strong>%s:</strong></TD>", 
		gettext("SONiX IP Camera Configuration Backup") );
...
#else
	length += CGI_SPRINTF(buf,length, "<TD align=\"center\"><img height=10 src=\"/sp.gif\" width=10><br><a href='/system/%s'  target='mainframe'> <IMG SRC='/pd.gif' BORDER=0>%s</a><br> </TD></TR></FORM>\r\n",
		 _fileName, _fileName );
#endif
	return strdup(buf);
}
...
int cgiData_BackupLocal(DATA_INFO *info)
{
	...
	{
		char target[128];
		if(__backup_packaging(CGIDATA_OP_BACK_LOCAL))
		{
			return -1;
		}
