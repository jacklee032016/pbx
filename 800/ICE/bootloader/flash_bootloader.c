...
#define BLOCK_COUNT_OF_BOOTLOADER	2
#define OPTION_COUNT_OF_BOOTLOADER	1
...
void flash_program()
{
	...
	ulong32 dst = 0;
	ulong32 i = 0;
	ulong32 Temp_BlockAddress = 0;


	for(i = 0;i< OPTION_COUNT_OF_BOOTLOADER;i++ )
	{
	...
	BlockAddress = FLASH_START;
	status = 0;
	while(BlockAddress < (FLASH_START + (MAIN_BLOCK_SIZE_BY_WORD * OPTION_COUNT_OF_BOOTLOADER)))
	{
		i = 0;
