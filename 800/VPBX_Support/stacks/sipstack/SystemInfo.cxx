...
*
* $Id: SystemInfo.cxx,v 1.1.1.1 2006/11/30 16:27:10 lizhijie Exp $
*/
...
void
Assist::parseSystemInfo(char* tag, char* type, char* value)
{
    if (strcmp(tag, "REGISTER") == 0)
    {
        ...
        {
            Assist::theSystem.setUrlToRegister(value);
        }
...
SystemInfo::updateHost(const Data *destHost)
{
	if(destHost)
	{
		...
		if(dHostent->h_addrtype!=AF_INET)return;
...
const char * const
SystemInfo::gethostAddress( const Data *destHost)
{
	struct addrinfo hints;

	hints.ai_family = NetworkConfig::instance().getAddrFamily();
	if(hints.ai_family != PF_INET)return hostIP;
...
const char* const 
SystemInfo::gethostAddress () //const
{
	struct addrinfo hints;
