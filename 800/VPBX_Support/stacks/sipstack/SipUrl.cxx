...
*
* $Id: SipUrl.cxx,v 1.1.1.1 2006/11/30 16:27:10 lizhijie Exp $
*/
...
SipUrl::SipUrl( const Data& data ) throw(SipUrlParserException&)
    :BaseUrl(),
     schemeName("sip"),
     user(),
     passwd(),
...
SipUrl::SipUrl(bool lr)
    :BaseUrl(),
     ...
     otherName(),
     headers(),
     lrParam(lr), // !jf! - should be true when it is a looser router
     ipv6Addr(false),
     EscObj()
{
}
...
bool SipUrl::operator ==(const SipUrl& srcUrl) const
{
    ...
    if (user != 0)
    {
	cpDebug(LOG_DEBUG_STACK, "this.user= %s", user->encode().logData());
    }
    if (srcUrl.user != 0)
    {
	cpDebug(LOG_DEBUG_STACK, "other.user= %s", srcUrl.user->encode().logData());
    }
    cpDebug(LOG_DEBUG_STACK, "this.passwd= %s" , passwd.logData());
    cpDebug(LOG_DEBUG_STACK, "other.passwd= %s", srcUrl.passwd.logData());
    
    cpDebug(LOG_DEBUG_STACK, "this.host= %s" , host.logData());
    cpDebug(LOG_DEBUG_STACK, "other.host= %s", srcUrl.host.logData());
    
    cpDebug(LOG_DEBUG_STACK, "this.ttlParam= %s", ttlParam.logData() );
    cpDebug(LOG_DEBUG_STACK, "other.ttlParam= %s" , srcUrl.ttlParam.logData());
    
    cpDebug(LOG_DEBUG_STACK, "this.maddrParam= %s" , maddrParam.logData());
    cpDebug(LOG_DEBUG_STACK, "other.maddrParam= %s" , 
	                                    srcUrl.maddrParam.logData());
    
    cpDebug(LOG_DEBUG_STACK, "this.userParam= %s " , userParam.logData());
    cpDebug(LOG_DEBUG_STACK, "other.userParam= %s" , srcUrl.userParam.logData());

    cpDebug(LOG_DEBUG_STACK, "this.methodParam= %s" , methodParam.logData());
    cpDebug(LOG_DEBUG_STACK, "other.methodParam= %s" , srcUrl.methodParam.logData());
    cpDebug(LOG_DEBUG_STACK, "this.otherParam= %s", otherParam.logData());
    cpDebug(LOG_DEBUG_STACK, "other.otherParam= %s" , 
	                      srcUrl.otherParam.logData());
    
    cpDebug(LOG_DEBUG_STACK, "this.headers = %s" , headers.logData());
    cpDebug(LOG_DEBUG_STACK, "other.headers = %s" , srcUrl.headers.logData());
    ...
    cpDebug(LOG_DEBUG_STACK, "sipUrl comparison: , returning: %s", (equal == true ? "true" : "false" ));
    return equal;
...
SipUrl::decode( const Data& urlstr )
{
    ...
    //cpDebug(LOG_DEBUG_STACK, "completed decoding in the SipUrl");
}    // SipUrl::decode
...
SipUrl::fastDecode( Data myData )
{
    ...
    if((keyPos = workstring.find_first_of("?")) != string::npos){
        ...
        EscObj = new EmbeddedObj(escData);
        myData = tmpmyData;   
    }
    Data userInfo;
    ...
    if(parseFailed)
    {
	cpDebug(LOG_DEBUG, "parse failed: could not find ':' separating URL type from body: %s", myData.logData());
	return false;
    }

    if  (!isEqualNoCase(urlType, "sip") )
    {
	cpDebug(LOG_DEBUG, "parse failed: scheme not SIP: %s", myData.logData());
	return false;
    }
    //set it.
    schemeName = urlType;
...
SipUrl::getUrlParam() const
{
    ...
    if (userParam.length() )
    {
        cpDebug(LOG_DEBUG_STACK, "SipUrl::encode : %s", userParam.logData());
        urlparam1 += SEMICOLON;
