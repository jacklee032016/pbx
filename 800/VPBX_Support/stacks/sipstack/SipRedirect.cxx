...
*
* $Id: SipRedirect.cxx,v 1.1.1.1 2006/11/30 16:27:10 lizhijie Exp $
*/
...
{
    cpDebug(LOG_DEBUG_STACK, "SipRedirect ::operator ==");
    bool equal = (( rurl == sredirect.rurl) &&
                  ( rcounter == sredirect.rcounter) &&
                  ( rlimit == sredirect.rlimit) &&
                  ( rreason == sredirect.rreason));
    bool sel = (rurl == sredirect.rurl);
    cpDebug(LOG_DEBUG_STACK, " url equal is : %d", sel);

    sel = sel && ( rcounter == sredirect.rcounter);
    cpDebug(LOG_DEBUG_STACK, " counter1 equal is : %d", sel);

    sel = sel && ( rlimit == sredirect.rlimit);
    cpDebug(LOG_DEBUG_STACK, " limit1 equal is : %d", sel);

    sel = sel && ( rreason == sredirect.rreason);
    cpDebug(LOG_DEBUG_STACK, " reason1 equal is : %d", sel);
