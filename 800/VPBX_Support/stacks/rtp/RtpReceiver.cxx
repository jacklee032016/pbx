...
*
* $Id: RtpReceiver.cxx,v 1.1.1.1 2006/11/30 16:27:09 lizhijie Exp $
*/
...
//printf("inPos =%d\n", inPos);
		...
		else
		{
            ...
            cpDebug(LOG_DEBUG_STACK, "insert middle packet: %d prevSeq %d",
...
void RtpReceiver::initSource (RtpPacket* p)
{
    ...
    cpDebug(LOG_DEBUG_STACK, "InitSource %u with sequence %d and rtp time %u",
                                ...
                                RtpPacket* p, bool print)
{
    ...
    switch (newtype)
    {
        case rtpPayloadPCMU:
        case rtpPayloadPCMA:
        if (print) cpDebug(LOG_DEBUG, "Setting api format to: PCMU %d", no_samples);
        apiFormat_clockRate = 8000;
        break;
        case rtpPayloadL16_mono:
        if (print) cpDebug(LOG_DEBUG, "Setting api format to: L16 %d", no_samples);
        apiFormat_clockRate = 44100;
        apiFormat_perSampleSize = 2;
        break;
        case rtpPayloadG729:
        if (print) cpDebug(LOG_DEBUG, "Setting api format to: G729 %d", no_samples);
        apiFormat_clockRate = 8000;
        break;
        default:
                                    ...
                                    RtpPacket* p, bool print)
{
    ...
    switch (newtype)
    {
        case rtpPayloadPCMU:
        case rtpPayloadPCMA:
        if (print) cpDebug(LOG_DEBUG, "Setting network format to: PCMU %d", no_samples);
        networkFormat_clockRate = 8000;
        break;
        case rtpPayloadL16_mono:
        if (print) cpDebug(LOG_DEBUG, "Setting network format to: L16 %d", no_samples);
