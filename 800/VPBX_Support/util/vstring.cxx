/*
* $Id: vstring.cxx,v 1.1.1.1 2006/11/30 16:27:10 lizhijie Exp $
*/
...
const size_t    Assist::MAX_LINE_LENGTH = 1024;
const string    Assist::SEPARATORS(":= \t");
const string    Assist::COMMENT_CHARS("#!;");
const string    Assist::QUOTE_CHARS("\"'`");

const char      Assist::NL('\n');
const string    Assist::TAB1(" \t");
const string    Assist::TAB2(" \t\t");
const string    Assist::TAB3(" \t\t\t");


string &
Assist::upper(string & src)
{
...
{
    char    buffer[Assist::MAX_LINE_LENGTH];
    
    while ( true )
    {
        memset(buffer, 0, Assist::MAX_LINE_LENGTH);

        if ( in.getline(buffer, Assist::MAX_LINE_LENGTH).eof() == true )
        {
            return ( false );
        }
