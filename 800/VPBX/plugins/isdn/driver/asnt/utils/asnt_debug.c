/*
* $Id: asnt_debug.c,v 1.1.1.1 2006/11/30 16:26:29 lizhijie Exp $
*/
#include <stdarg.h>
#include "asnt_debug.h"


static unsigned int	debug_mask = 0xFFFFFFFF;
static FILE		*debug_file = NULL;
static FILE		*warn_file = NULL;
static FILE		*error_file = NULL;

int debug_init(unsigned int mask, char *dfile, char *wfile, char *efile)
{
	if (dfile)
	{
		...
		{
			debug_file = stdout;
			fprintf(debug_file,"%s: cannot open %s for debug log, using stdout\n",	__FUNCTION__, dfile);
		}
	}
	else
	{
...
//				"%s: using stdout for debug log\n", __FUNCTION__);
	...
	if (wfile)
	{
		...
		{
			warn_file = stderr;
			fprintf(warn_file,	"%s: cannot open %s for warning log, using stderr\n",__FUNCTION__, wfile);
		}
	}
	else
	{
...
//				"%s: using stderr for warning log\n", __FUNCTION__);
	...
	if (efile)
	{
		...
		{
			error_file = stderr;
			fprintf(error_file,	"%s: cannot open %s for error log, using stderr\n",__FUNCTION__, efile);
		}
	}
	else
	{
...
int dprint(unsigned int mask, const char *fmt, ...)
{
	int	ret = 0;
	va_list	args;
...
int dhexprint(unsigned int mask, char *head, unsigned char *buf, int len)
{
	int	ret = 0;
	char	*p,*obuf;
