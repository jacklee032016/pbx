...
SubscribeManager::SubscribeManager( Sptr <SipTransceiver> sipstack )
{
	...
	sipExpires.setDelta( Data(config->getSubscribeExpires() ));
	subscribeMsg->setExpires( sipExpires );

	delay = 1000 * config->getSubscribeExpires();
...
void SubscribeManager::subscribeMain()
{
...
bool SubscribeManager::handleSubscribeResponse(const StatusMsg& statusMsg)
{
	bool ret = true;
...
void SubscribeManager::updateSubscribeMsg( const StatusMsg& statusMsg )
{
	// for now, just update expire if status is 200 to be done
...
bool SubscribeManager::handleNotify( const NotifyMsg& msg )
{
	if ( subscribeMsg == 0 )
	{
