...
/*$Id: DigitCollector.cxx,v 1.1.1.1 2006/11/30 16:26:29 lizhijie Exp $*/
#include "global.h"
#include <cassert>
...
LocalDialMapType& DigitCollector::localDialMap = PbxConfiguration::instance()->getLocalDials();
RegularContainer DigitCollector::Regulars;
SpeedDialMapType& DigitCollector::speedDialMap = PbxConfiguration::instance()->getSpeedDials();
DialingPlanListType& DigitCollector::dialingPlanList = PbxConfiguration::instance()->getDialPlans();
Sptr < Fifo < Sptr < SipProxyEvent > > > DigitCollector::callProcessingQueue;
OthsDialMapType& DigitCollector::recallDialMap = PbxConfiguration::instance()->getOthsDials(0);
OthsDialMapType& DigitCollector::specialKeyDialMap = PbxConfiguration::instance()->getOthsDials(1);
...
{
	//dial = 
    if (dialMethod == URL_DIAL) return myDialedUrl;
    switch ( dialMethod )
    {
    case SPEED_DIAL:
		getUrlFromSpeedDial();
...
const string& DigitCollector::getUrl(const string&dstr, DialMethodsType dtype)
{
	dialMethod = dtype;
	dial = dstr;
	return getUrl();
	...
	if(!isPbxDial)
	{
		...
		if(buf[1] == '0')
		{
			if(buf[2] == '0')
				...
				url = url+"000"+PbxConfiguration::instance()->getLocalCountry()+buf.substr(2);
		}
		else
		{
			url = url+"000"+PbxConfiguration::instance()->getLocalCountry()+PbxConfiguration::instance()->getLocalDistrict()+\
				PbxConfiguration::instance()->getLocalDevice()+buf.substr(1);
		}

		

		static string local = "000"+PbxConfiguration::instance()->getLocalCountry()+PbxConfiguration::instance()->getLocalDistrict()+PbxConfiguration::instance()->getLocalDevice();
		...
		{
			url += string(":");
			url += PbxConfiguration::instance()->getLocalSipPort();
		}
		#endif
	}	
	else
...
void DigitCollector:: getUrlFromLocalDial()
{
	...
	{
		myDialedUrl += string(":");
		myDialedUrl += PbxConfiguration::instance()->getLocalSipPort ();
	}
...
//PbxConfiguration::instance()->lock_config(); 
/*add by  Tangbiao 2005-4-4*/
...
/*add by Tangbiao 2005-4-4*/
//PbxConfiguration::instance()->unlock_config(); 
/*add by  Tangbiao 2005-4-4*/
