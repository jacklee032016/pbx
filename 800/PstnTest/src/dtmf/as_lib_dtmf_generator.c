...
typedef struct 
{
	unsigned char buf[AS_DTMF_DATA_SIZE]; /* fsk write audio buffer */
	int bufp;
}AS_DIAL_DATA;
...
int __as_put_audio_sample(int fd,float y,int flag)
{
	...
	if (__dtmf_data.bufp== AS_DTMF_DATA_SIZE) 
	{
		__dtmf_data.bufp=0;
		
		ret = write(fd, __dtmf_data.buf, AS_DTMF_DATA_SIZE);
		if(ret != AS_DTMF_DATA_SIZE)
		{
			printf("write error : length %d\n", ret );
			if (errno == AS_ELAST) 
				return(-2);
			return(-1);
		}
...
static int __as_finish_audio(int fd,int flag)
{
	...
	{
		while(__dtmf_data.bufp < AS_DTMF_DATA_SIZE)   /* pad with silence */
		{		
		...
		if(ret != AS_DTMF_DATA_SIZE)
		{
			printf("Error in finish audio, length %d\n", ret);
			if (errno == AS_ELAST) 
				return(-2); /* hangup */
			return(-1);
		}
...
int as_lib_dial(int fd , char *digitstring ,int length )
{
	...
	if ( (!length) || (length <AS_DEFAULT_DTMFLENGTH) ) 
