...
#else
		if(strlen(hostname) )
			length += CGI_SPRINTF(msg, length, "<tr><td align=\"center\"><font size=2>%s&nbsp </font></td>",hostname);
		else
			length += CGI_SPRINTF(msg, length, "<tr><td align=\"center\"><font size=2>%s&nbsp </font></td>",cgi_host_name_read() );
		
		length += CGI_SPRINTF(msg, length, "<td align=\"center\"><font size=2>%s </font></td>" ,fqdn );
		length += CGI_SPRINTF(msg, length, "<td align=\"center\"><font size=2>%s&nbsp </font></td></tr>" ,ip); 
#endif
...
static char *__cgi_host_add_buttons()
{
	...
	length += CGI_SPRINTF(buf, length, "<form name=\"hostnameForm\" method=\"post\" action=\"%s\">\n", WEB_URL_SYSCONFIG );
	...
	length += CGI_SPRINTF(buf, length, "<TD  align=\"left\"><input name=\"%s\" type=\"text\" size=\"15\" value=\"%s\" maxlength=\"50\"></TD></TR>\r\n",
	...
	length += CGI_SPRINTF(buf, length, "<TR><TD align=\"center\" colspan=2>%s %s</TD></TR></TABLE>\r\n\r\n",
...
int sysconfig_hostname(SYSCFG_INFO *info)
{
	...
	if(action!= NULL && !strcasecmp(action, CGI_ACTION_ACTIVATE) )
	{
