...
/*
    ...
    *If backup to local,then:	  name = cmn_back_yyyy_mm_dd.tar
    *If backup to TFTP server,then:  name = cmn_back_deviceID.tar 
...
int cgidata_get_package_filename(char *filename, const char *command, char *devID)
{
	...
	if(!strcmp(command,CGIDATA_OP_BACK_LOCAL) )
	{
		...
		sprintf(filename,"%s_%02d-%02d-%02d.tar.gz",
	...
	{
		sprintf(filename,"%s_%s.tar.gz", CGIDATA_DATA_BACKUP_PREFIX, devID);
	}
	else if(!strcmp(command, CGIDATA_OP_AUTO_UPDATE ) )
	{
		sprintf(filename,"%s_%s.tar.gz", CGIDATA_SOFT_UPLOAD_PREFIX, devID);
	}
...
#define CGIDATA_IS_BACKUP(opname) \
	...
	char 	buf[8192*5];
	int 		length = 0;
	length += CGI_SPRINTF(buf,length, "<tr>\n\t<td><strong>%s %s<p></strong></TD></TR>\n", gettext("Input IP Address of TFTP Server which"), (CGIDATA_IS_BACKUP(opName))? gettext("save your backup"):gettext("get your backup") );
